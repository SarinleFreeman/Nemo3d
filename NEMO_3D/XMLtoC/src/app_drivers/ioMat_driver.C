...
int app_input_driver(int argC, char* argV[])
{
    ...
    if (!errorsOccured && !errReporter->getSawErrors())
    {
        ...
        char *pyFuncsFile = "funcs";

#ifndef NOPYTHON
        // Initialize embedded Python
        Py_Initialize();
        initExtension();
        PyObject* pFile = PyString_FromString(pyFuncsFile);
        PyObject *pModule = PyImport_Import(pFile);
        if (!pModule)  cerr << "Cannot find file \"" << pyFuncsFile << ".py\"" << endl;
        PyObject *pDict = PyModule_GetDict(pModule);

        // Setup RExec restrictions to the enviroment
        PyObject *v;
        v = PyRun_String("import rexec", Py_single_input, pDict, pDict);
        v = PyRun_String("renv=rexec.RExec()", Py_single_input, pDict, pDict);

        top_struct ts(doc,pDict);
        cout << " --------------------- " << endl;
        cout << "ts.toString()" << endl << ts.toString(0) << endl;
