...
LOCAL void usage_short(argv_t *args, int flag) {
  ...
  for (argp = args; argp->ar_short_arg != ARGV_LAST; argp++) {
    ...
    if (!mark) {
      ...
      if (colc + len > SCREEN_WIDTH - 2) {
        (void)fprintf(argv_error_stream, "\n%*.*s", (int)USAGE_LABEL_LENGTH,
                      (int)USAGE_LABEL_LENGTH, "");
  ...
  for (argp = args; argp->ar_short_arg != ARGV_LAST; argp++) {
    ...
    if (argp->ar_short_arg == ARGV_MAND) {
      ...
      (void)fprintf(argv_error_stream, "%s%s%c", prefix, SHORT_PREFIX,
                    argp->ar_short_arg);
      colc += len;
 ...
 * print a short-format usage message
 */
LOCAL void display_arg(FILE *stream, argv_t *argp, int max, int *colc) {
  int var_len, len;
...
LOCAL void display_option(FILE *stream, argv_t *argp, int *colc) {
  ...
  if (argp->ar_short_arg != ARGV_MAYBE) {
    (void)fprintf(stream, "%s%c", SHORT_PREFIX, argp->ar_short_arg);
...
LOCAL void usage_long(argv_t *args) {
  ...
  for (argp = args; argp->ar_short_arg != ARGV_LAST; argp++) {
    ...
    if (argp->ar_short_arg == NULLC) {
      ...
      if (colc < LONG_COLUMN) {
        (void)fprintf(argv_error_stream, "%*.*s", LONG_COLUMN - colc,
                      LONG_COLUMN - colc, "");
        colc = LONG_COLUMN;
      }
    }
    ...
    if (argp->ar_long_arg != NULL) {
      ...
      (void)fprintf(argv_error_stream, "%s%-.*s", LONG_PREFIX, len,
                    argp->ar_long_arg);
    ...
    if (argp->ar_comment != NULL) {
      ...
      if (colc < COMMENT_COLUMN) {
        (void)fprintf(argv_error_stream, "%*.*s", COMMENT_COLUMN - colc,
                      COMMENT_COLUMN - colc, "");
        colc = COMMENT_COLUMN;
      }

      len = SCREEN_WIDTH - colc - COMMENT_LABEL_LENGTH;
      (void)fprintf(argv_error_stream, "%s%-.*s", COMMENT_LABEL, len,
                    argp->ar_comment);
    }
...
LOCAL void do_usage(argv_t *args, int flag) {
  ...
  if (flag == GLOBAL_USAGE_ALL) {
    ...
    (void)fprintf(argv_error_stream,
                  ...
                  SHORT_COLUMN, SHORT_COLUMN, "", LONG_PREFIX, USAGE_ARG);
    (void)fprintf(argv_error_stream,
                  "%*.*sUse '%s%s' for short usage information.\n",
                  SHORT_COLUMN, SHORT_COLUMN, "", LONG_PREFIX, USAGE_SHORT_ARG);
    (void)fprintf(argv_error_stream,
                  "%*.*sUse '%s%s' for long usage information.\n", SHORT_COLUMN,
                  SHORT_COLUMN, "", LONG_PREFIX, USAGE_LONG_ARG);
    (void)fprintf(argv_error_stream,
                  "%*.*sUse '%s%s' for all usage information.\n", SHORT_COLUMN,
                  SHORT_COLUMN, "", LONG_PREFIX, USAGE_ALL_ARG);
    (void)fprintf(argv_error_stream,
                  "%*.*sUse '%s%s' to display the help message.\n",
                  SHORT_COLUMN, SHORT_COLUMN, "", LONG_PREFIX, HELP_ARG);
    (void)fprintf(argv_error_stream,
                  "%*.*sUse '%s%s' to display the version message.\n",
                  SHORT_COLUMN, SHORT_COLUMN, "", LONG_PREFIX, VERSION_ARG);
    (void)fprintf(argv_error_stream,
                  "%*.*sUse '%s%s' to display the options and their values.\n",
                  SHORT_COLUMN, SHORT_COLUMN, "", LONG_PREFIX, DISPLAY_ARG);
  }
}
...
LOCAL int preprocess_array(argv_t *args, int num_args) {
  ...
  for (argp = args; argp < args + num_args; argp++) {
    ...
    if (argp->ar_variable == NULL && argp->ar_short_arg != ARGV_OR &&
 ...
 */
LOCAL void translate_value(char *arg, ARGV_PNT var, short type) {
  argv_array_t *arrp;
  argv_type_t *typep;
  ...
  if (type & ARGV_ARRAY) {
    ...
    if (arrp->aa_entryn == 0)
      ...
      arrp->aa_entries = (char *)realloc(arrp->aa_entries,
                                         (arrp->aa_entryn + ARRAY_INCR) * size);
  ...
  case ARGV_BOOL:
    ...
    if (arg == NULL)
      *(char *)var = ARGV_TRUE;
             ...
             *(char *)arg == 'y' || *(char *)arg == 'Y' || *(char *)arg == '1')
      *(char *)var = ARGV_TRUE;
    else
      *(char *)var = ARGV_FALSE;
    break;
  ...
  case ARGV_BOOL_ARG:
    if (*(char *)arg == 't' || *(char *)arg == 'T' || *(char *)arg == 'y' ||
        *(char *)arg == 'Y' || *(char *)arg == '1')
      *(char *)var = ARGV_TRUE;
    else
      *(char *)var = ARGV_FALSE;
    break;
...
LOCAL int check_or(argv_t *args, argv_t *which) {
  ...
  if (matchp == NULL) {
    ...
    for (argp = which + 2; argp->ar_short_arg != ARGV_LAST &&
  ...
  for (;;) {
    if (matchp->ar_long_arg == NULL)
      ...
      (void)fprintf(argv_error_stream, "     %s%c (%s%s)\n", SHORT_PREFIX,
                    matchp->ar_short_arg, LONG_PREFIX, matchp->ar_long_arg);
...
LOCAL int check_xor(argv_t *args) {
  ...
  for (argp = startp;; argp += 2) {
     ...
     * something below and above...
     */
    (void)fprintf(argv_error_stream, "     %s%c", SHORT_PREFIX,
                  (argp - 1)->ar_short_arg);
    if ((argp - 1)->ar_long_arg != NULL)
      (void)fprintf(argv_error_stream, " (%s%s)", LONG_PREFIX,
                    (argp - 1)->ar_long_arg);
    (void)fprintf(argv_error_stream, "\n");
...
LOCAL int check_mand(argv_t *args) {
  ...
  for (argp = args; argp->ar_short_arg != ARGV_LAST; argp++)
    if (argp->ar_short_arg == ARGV_MAND &&
        (!(argp->ar_type & ARGV_FLAG_USED)) && !(argp->ar_type & ARGV_ARRAY))
      slotc++;
  ...
  if (slotc > 0) {
    if (argv_error_stream != NULL)
      (void)fprintf(argv_error_stream,
                    ...
                    argv_program, USAGE_ERROR_NAME, slotc,
                    (slotc == 1 ? "" : "s"));
    return ERROR;
  }
  ...
  if (queuec > 0) {
    if (argv_error_stream != NULL)
      (void)fprintf(argv_error_stream,
                    ...
                    argv_program, USAGE_ERROR_NAME, queuec,
                    (queuec == 1 ? "" : "s"));
    return ERROR;
  }
 ...
 * marker.
 */
LOCAL void do_list(argv_t *grid, int argc, char **argv, char *okayp) {
  argv_t *gridp, *matchp;
  ...
  for (argp = argv; argp < argv + argc; argp++) {
    ...
    if (!last_arg && strncmp(LONG_PREFIX, *argp, LONG_PREFIX_LENGTH) == 0) {
      ...
      for (gridp = grid; gridp->ar_short_arg != ARGV_LAST; gridp++) {
        ...
        if (strncmp(*argp + LONG_PREFIX_LENGTH, gridp->ar_long_arg, len) == 0) {
          if (matchp != NULL) {
            if (argv_error_stream != NULL)
              (void)fprintf(argv_error_stream,
            ...
            if (argv_error_stream != NULL) {
              if (argv_version_string == NULL)
                ...
                (void)fprintf(argv_error_stream, "%s: %s%s\n", argv_program,
                              VERSION_LABEL, argv_version_string);
            }
            (void)exit(0);
          }
          continue;
  ...
  if (unwantc > 0 && argv_error_stream != NULL)
    (void)fprintf(argv_error_stream,
                  ...
                  USAGE_ERROR_NAME, unwantc, (unwantc == 1 ? "" : "s"));
}
...
LOCAL int process_args(argv_t *args, int argc, char **argv) {
  ...
  /* was there an error? */
#ifndef MPI_NEGF /* some cluster versions of mpi append other command line     \
 ...
 */
EXPORT int argv_web_process_string(argv_t *args, char *arg0, char *string) {
  char *strp;
