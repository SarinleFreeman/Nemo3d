...
*****************************************************************************
$Header: /repo/nemo3d/src/util/util_stringops.c,v 1.5 2004/10/27 19:43:10 gekco Exp $
*****************************************************************************/
...
/*
 * Allocates new memory space, removes trailing white spaces from the string, and
 * then copies the string into the memory space.
 */
...
/*
 * Contactenates two or more character strings. It allocates space for the resulting
 * string and NULL terminates it. Since it uses str_calloc() to allocate space,
 * the calling routing should call str_free() to free up the string.
 */
char *n3d_strcat(char *firstToken, ...) {
   ...
   if (firstToken) {
     ...
     /* Allocate new memory to hold all the tokens and set the initial value to be firstToken */
...
char ** n3d_StringListAdd(char ***str_list_ptr, const char *new_string) {
   ...
   if (str_list) {
	 ...
	 str_new[i] = str_list[i];
      ...
      str_new[numStrings] = (new_string) ? n3d_strdup_n(new_string) : n3d_strdup_n(" ");
      str_free(str_list);
   }
   else {
