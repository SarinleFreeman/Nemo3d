...
*****************************************************************************
$Header: /repo/nemo3d/src/base/StrainParam.c,v 1.12 2005/01/14 20:24:38 marek Exp $
*****************************************************************************/
 ...
 * @param anh is an integer 0/1/2 specifying the choice of the anharmonicity corrections
 * @param pr is aboolean value, specifying either to print the wornings at truncation
     ...
     //Get the effective value of alpha scaled with strain
{
  ...
  if(anh==1){
    ...
    if(pr && !mpi_n3d_id){
      printf("\nOption anh=%d has not been implemented yet!!!\n"
             "Use anharmonicity corrections for better strain distribution instead (anh=0).",
             anh);
    }
    A=this->aa_VFF[m_atom_type][n_atom_type];
  }
  ...
  if(r2<(d2*(1+delt*2/(3*A))))
    alpha=(this->alpha[m_atom_type][n_atom_type]*
           (1-A*(r2/d2-1)));
    ...
    if(pr && !mpi_n3d_id){
      printf("!!!%d-%d bond : sqrt(r2/d2)=%g/%g=%g : alpha=%g\n",
             m_atom_type,n_atom_type,
             sqrt(r2),sqrt(d2),sqrt(r2/d2),alpha);
    }
 ...
 */
real StrainParam::beta_eff(int m_atom_type,//type of the m-th atom
     ...
     //Get the effective value of beta scaled with strain
{
  ...
  real be0=sqrt(this->beta[m_atom_type][n1_atom_type]*this->beta[m_atom_type][n1_atom_type]);
  real beta=be0;
  real B,C;
  if(anh==1){
  ...
  if(tt>delt/3.0*(2*(1/B-1/3.0)-1/(3*rd))){
    ...
    beta=be0*((1.0-2*delt/3.0)+B/3.0*(delt/3.0*(1/rd+2)-1))*(1-C*(rd-1));
    flag=false;
#ifdef PR_WARNING_ON_VFFC_TRUNCATION
    if(pr && !mpi_n3d_id){
      printf("  !!!r1/d1=%g/%g=%g r2/d2=%g/%g=%g cos(tt)=%g => !!angle!!: beta=%g\n",
...
#ifdef PR_WARNING_ON_VFFC_TRUNCATION
      if(pr && !mpi_n3d_id){
        printf("  !!!r1/d1=%g/%g=%g r2/d2=%g/%g=%g cos(tt)=%g => !!length!!: beta=%g\n",
               r10,d10,r10/d10,r20,d20,r20/d20,tt,beta);
      }
#endif//PR_WARNING_ON_VFFC_TRUNCATION
    }
...
void StrainParam::SetMatParm(vector<MaterialHandle> sMatList) 
{
   ...
   this->aa_VFF =  Rmatrix(EL_TOTAL,EL_TOTAL);//Coefficients of alpha strain dependence at low energy
   this->ba_VFF =  Rmatrix(EL_TOTAL,EL_TOTAL);//Coefficients of beta strain dependence at low energy
   this->ca_VFF =  Rmatrix(EL_TOTAL,EL_TOTAL);//Coefficients of beta strain dependence at low energy
   this->ao_VFF =  Rmatrix(EL_TOTAL,EL_TOTAL);//Coefficients of alpha strain dependence at high energy
   this->bo_VFF =  Rmatrix(EL_TOTAL,EL_TOTAL);//Coefficients of beta strain dependence at high energy
   this->co_VFF =  Rmatrix(EL_TOTAL,EL_TOTAL);//Coefficients of beta strain dependence at high energy
   ...
   for (vector<MaterialHandle>::iterator sm = sMatList.begin();
      ...
      if (smh->unstrnd_cubic_cell_length <= 0.0 && mpi_n3d_id==mpi_n3d_masterid){
      ...
      this->alpha[anion][cation] = this->alpha[cation][anion] = 
         smh->strain_alpha;
      this->beta[anion][cation] = this->beta[cation][anion] = 
         smh->strain_beta;
...
void StrainParam::SetOptions(const top_struct& opt) 
{
   ...
   if (keating.MinWrtLatt == Keating_struct::x) {
      MinWrtX=true;  MinWrtY=false;  MinWrtZ=false;
   }
   else if (keating.MinWrtLatt == Keating_struct::y) {
      MinWrtX=false;  MinWrtY=true;  MinWrtZ=false;
   }
   else if (keating.MinWrtLatt == Keating_struct::z) {
      MinWrtX=false;  MinWrtY=false;  MinWrtZ=true;
   }
   else if (keating.MinWrtLatt == Keating_struct::xy) {
      MinWrtX=true;  MinWrtY=true;  MinWrtZ=false;
   }
   else if (keating.MinWrtLatt == Keating_struct::xz) {
      MinWrtX=true;  MinWrtY=false;  MinWrtZ=true;
   }
   else if (keating.MinWrtLatt == Keating_struct::yz) {
      MinWrtX=false;  MinWrtY=true;  MinWrtZ=true;
   }
   else if (keating.MinWrtLatt == Keating_struct::xyz) {
      MinWrtX=true;  MinWrtY=true;  MinWrtZ=true;
   }
   else {
      MinWrtX=false;  MinWrtY=false;  MinWrtZ=false;
   }
   ...
   if (keating.Periodicity_strain==Keating_struct::SameAsGlobal) {
      ...
      MinWrtX = MinWrtX &&
         (bc==ElGeo_struct::Periodic_x || 
          bc==ElGeo_struct::Periodic_xy ||
          bc==ElGeo_struct::Periodic_xz || 
          bc==ElGeo_struct::Periodic_xyz);

      MinWrtY = MinWrtY &&
         (bc==ElGeo_struct::Periodic_y || 
          bc==ElGeo_struct::Periodic_xy ||
          bc==ElGeo_struct::Periodic_yz || 
          bc==ElGeo_struct::Periodic_xyz);
   
      MinWrtZ = MinWrtZ &&
         (bc==ElGeo_struct::Periodic_z || 
          bc==ElGeo_struct::Periodic_yz ||
          bc==ElGeo_struct::Periodic_xz || 
          bc==ElGeo_struct::Periodic_xyz);
