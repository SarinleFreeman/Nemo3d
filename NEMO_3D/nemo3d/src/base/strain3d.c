...
*****************************************************************************
$Header: /repo/nemo3d/src/base/strain3d.c,v 1.47 2008/07/23 16:49:37 nkharche
Exp $
*****************************************************************************/
      ...
      for (n = 0; n < d->geo.Neighbors(Za); n++) {
        ...
        if (Zc_nbr >= 0) {
          ...
          if (AtomType_nbr != 12 && AtomType_nbr != 10 && AtomType_nbr != 14) {
            printf("ACHTUNG(%d)!!!: nbr atom type = %d  atom type = %d  "
                   "(indx=%d %d)\n",
...
/*! Comment_N: This is like a main() function in strain calculation. It is
   called from 'setup_hamiltonian' function in ham3d_final.c */
...
//! If first_or_second is 2, min_estrain is called the second time.
{
  if (d->opt.ExecParam.Strain.StrainModel == Strain_struct::Read) {
    /*! Comment_N: Read displacements from a file. File name is provided from
     * the input deck. Displacements read from this file are stored in
     * l_m_2_atomdsp[cell_indx][atom_indx][coord_indx]. coord_indx: 0,1,2 for
     * x,y,z respectively. If strain calculation is turned off (from the input
     * deck) then these will be the final displacements of relaxed structure
     * from original structure. If Strain calculation is on then these
     * displacements will be used as initial guess. */
    ...
    return;
  } else if ((d->opt.ExecParam.Strain.StrainModel ==
              Strain_struct::VFF_keating) ||
             (d->opt.ExecParam.Strain.StrainModel ==
              Strain_struct::VFF_keating_strained)) {
    /*! Comment_N: VFF_Keating minimization from scratch i.e. no initial guess
     * is provided. */
    ...
    const bool do_min = d->strain.doPeriodMinimization();
    /*! Comment_N: doPeriodMinimization() is coded in file StrainParam.h.
Relevent portion of the input deck is as follows: <name>Minimize wrt lattice
const</name> <cTag>MinWrtLatt</cTag> <desc>allow the periodicity to relax</desc>
                        <value>z</value>
                        <option>none</option>
                        <option>x</option>
                        <option>y</option>
                        ...
                        <option>yz</option>
                        <option>xyz</option>
In the above calculation a boolean called d->strain.MinWrtZ will be true and
lattice constant in z-direction will be allowed to vary. a_x and a_y will remain
fixed. Initial values of a_x, a_y and a_z are specified in the begining of the
input deck (Just below the band model). */
    int Npad = (!mpi_n3d_id && do_min ? 3 : 0);
    int n_loc = 3 * d->cell_ln[mpi_n3d_id] * d->geo.AtomsPerCellMax() + Npad;
    int n_tot = 3 * d->n_atom_tot + Npad;
    rvectr p = Rvectr(n_loc);
    /*! Comment_N: p - is the vector of independent variables which will be
      interatively modified in the Conjugate Gradient (CG) routine. Total length
      of this vecot is n_tot = n_loc[0] + n_loc[1] + ... + n_loc[#CPUs-1]. For
      all CPUs except the mater CPU (CPU #0): n_loc =
      3*d->cell_ln[mpi_n3d_id]*d->geo.AtomsPerCellMax(). 3 - (x,y,z).
      d->cell_ln[mpi_n3d_id] - # of unit cells on proc #mpi_n3d_id and
      AtomsPerCellMax() = # atoms in each unit cell For master CPU: n_loc =
      3*d->cell_ln[mpi_n3d_id]*d->geo.AtomsPerCellMax() + Npad. Npad = 3 if
      lattice constant minimization is turned on. Lattice constants are stored
      on the master CPU and they are broadcasted to all other CPUs for
      calculation of gradient for next iteration. */
    macopt_args amac;
    ...
    /*! Comment_N: macoptII_par is a CG routine. And above are the arguments
     * passed to this routine. Their values are set from the input deck. */
    ...
    if (d->opt.ExecParam.Strain.StrainOut.EnergyCGiter) {
      ...
      if (d->strain.first_or_second == 1)
        n3d_FileTypeSet(&filename, "nd_estrain_iter_first", TRUE);
      else
        n3d_FileTypeSet(&filename, "nd_estrain_iter_second", TRUE);
    ...
    if (d->opt.ExecParam.Strain.StrainOut.EnergyFinal) {
      ...
      if (d->strain.first_or_second == 1)
        n3d_FileTypeSet(&filename, "nd_estrain_first", TRUE);
      else
        n3d_FileTypeSet(&filename, "nd_estrain_second", TRUE);

      amac.filename_last = filename;
    }

    d->strain.dp = Rvectr(n_loc);
    /*! Comment_N: strain.dp is the gradient of vector p. It is passed to
     * macoptII_par for calculation of next approximation to p. Note that the
     * length of strain.dp on every processor are same as that of p which is
     * equal to n_loc for that processor. */
    d->strain.debugp = 0;
    /*! Comment_N: If debugp == 1. Then code runs in debug mode. In this mode it
     * prints out some important values which could ease the dibugging process.
     */

    if (d->strain.first_or_second == 1)
      readDisplacement(d, p);
    //! In the first strain calculation, initialize both p and
    //! d->geo.l_m_2_atomdsp from a file, if the input file exists.
    else {
...
d->geo.l_m_2_atomdsp[Icell][j][0] ; d->geo.l_m_2_atomdsp_onedomain[i][j][1] =
d->geo.l_m_2_atomdsp[Icell][j][1] ; d->geo.l_m_2_atomdsp_onedomain[i][j][2] =
d->geo.l_m_2_atomdsp[Icell][j][2] ;
        }}

        rm_f3tensor(&d->geo.l_m_2_atomdsp,0) ;
        nml_memory_report("REPLACED THE STRAIN ARRAY WITH A TEMPORARY COPY - FOR
MACOPT") ;
}
...
*/
    ...
    initStrainComm(d);
    /*! Comment_N: This function initializes the communication arrays to
     * communicate calculated strain results among participating CPUs. */
    ...
    if (!do_min) {
      ...
      /*! Comment_N: To remove minimization wrt lattice constant this part
       * should be executed always. */
      macoptII_par(p, n_loc, n_tot, (calc_estrain_macopt), (void *)d, &amac);
      /*! Comment_N: macoptII_par is the actual CG routine. It is coded in the
       * file NEMO_3D/eigen/src/macopt_par.c */

      E = amac.track_one_value;
    } else { /*! do try to minimize energy with respect to lattice constant */
      /*! Comment_N: We want to remove the following part which minimizes energy
       * wrt lattice constant. */
      /*
      ...
      */
      ...
      if (Npad) {
        /*! Comment_N: Npad is non-zero only for the master proc. Last 3
        elements of p on the master proc are used to store (variable) lattice
        constants. SCALE_FCTR is provided from the input deck. Relevant portion
        of the input deck is as follows: <param type="real"> <name>What is the
        scaling factor for dE/da?</name> <cTag>SCALE_FCTR</cTag> <desc>What is
        the scaling factor (S) for dE/da? Use number between 1e-2 to 1e-5
        depending on the number of atoms (N) in strain calculation. An ideal S
        is about 1.0/sqrt(N).
                                    </desc>
                                    <value>1e-1</value>
                                </param>
        ...
        /*! Comment_N: Above three lines assign initial values of lattice
         * constants to appropriate elements of p. */
      }
      ...
      if (!_new_minimize) {
        /*! Comment_N: _new_minimize appears to be always one. Its value is set
         * on line #52 in the same (this) file. So te following code is not
         * executed. */
        latt_max = 0.62;
        ...
        while (fabs(latt_max - latt_min) > 1e-8) {
          ...
          macoptII_par(p, n_loc, n_tot, (calc_estrain_macopt), (void *)d,
                       &amac);
      ...
      /*! Comment_N: Minimization is done. Print out new lattice constants on
       * the master CPU. */
      cout_master << "a=(" << d->geo.lattice_x << "," << d->geo.lattice_y << ","
                  << d->geo.lattice_z << ")\n";
    }
...
/*
{
    ...
    for ( int i=0; i < d->cell_ln[mpi_n3d_id]; i++, Icell++ ) {
            for ( int j=0; j < d->geo.AtomsPerCellMax(); j++){
                d->geo.l_m_2_atomdsp[Icell][j][0] =
d->geo.l_m_2_atomdsp_onedomain[i][j][0] ; d->geo.l_m_2_atomdsp[Icell][j][1] =
d->geo.l_m_2_atomdsp_onedomain[i][j][1] ; d->geo.l_m_2_atomdsp[Icell][j][2] =
d->geo.l_m_2_atomdsp_onedomain[i][j][2] ;
        }}

        rm_f3tensor(&d->geo.l_m_2_atomdsp_onedomain,0) ;
        nml_memory_report("REPLACED THE TEMPORARY COPY WITH THE STRAIN ARRAY -
AFTER MACOPT") ;
}
...
*/
    ...
    if (d->strain.first_or_second == 1) {
      write_nbr(d,
                1); /*! write if d->opt.ExecParam.Strain.StrainOut.Neighborhood
                       is on */
      writePos(d, "strain_first"); /*! write if
                                      d->opt.ExecParam.Strain.StrainOut.DisplacementData
                                      is on */
      //! cout_master <<" Printing Surface Atomic Position.
      //! "<<d->strain.first_or_second<<endl; writePos(d, "surface_atom_pos");
    } else {
      write_nbr(d,
                2); /*! write if d->opt.ExecParam.Strain.StrainOut.Neighborhood
                       is on */
      writePos(d, "strain_second"); /*! write if
                                       d->opt.ExecParam.Strain.StrainOut.DisplacementData
                                       is on */
    }
...
void initStrainComm(qd_struct d) {
  /*! Comment_N: Hopefully the same communication arrays will also work after we
   * eliminate lattice constant minimization. We will still be using same data
   * structures, so this function should work just fine. I didn't go through
  ...
  int mpi_left =
      (mpi_n3d_id ? mpi_n3d_id - 1
                  : (d->geo.isPeriodicX() ? mpi_n3d_numprocs - 1 : -1));
  int mpi_right =
      (mpi_n3d_id < mpi_n3d_numprocs - 1 ? mpi_n3d_id + 1
                                         : (d->geo.isPeriodicX() ? 0 : -1));
  int left_min = (mpi_left >= 0 ? d->seg_s[mpi_left] : 9999);
  int left_max =
      (mpi_left >= 0 ? d->seg_s[mpi_left] + d->seg_ln[mpi_left] : -9999);
  int right_min = (mpi_right >= 0 ? d->seg_s[mpi_right] : 9999);
  int right_max =
      (mpi_right >= 0 ? d->seg_s[mpi_right] + d->seg_ln[mpi_right] : -9999);

  int szDatStr = sizeof(int) + sizeof(real);
  int szCommL =
      (mpi_left >= 0 ? 6 * maxNbr * d->commsize[mpi_n3d_id][mpi_left] : 1);
  int szCommR =
      (mpi_right >= 0 ? 6 * maxNbr * d->commsize[mpi_n3d_id][mpi_right] : 1);
...
#define MEMCHECK
  ...
  printf("\n strain memory allocation\nmemory in MB strain_memory=%g "
         "tl_memory=%g\n",
         strain_memory, tl_memory),
      fflush(stdout);
#endif

  if (mpi_n3d_numprocs == 1)
    return; //! only set up if there will be communication

  /*! it's simplest to send equal sized-messages; set sz_msg to be max among
   * procs */
  sz_msg = 1;
  ...
  /*! at this point, we have a temporary index mapping files
     {l,m}_{tmp_snd,tmp_rcv}{L,R}. we need to send this mapping to neighboring
     processors so that they know what to do with the actual displacement data
     they will receive. */
  if (mpi_n3d_id % 2) {
    if (mpi_left >= 0) {
      MPI_Sendrecv(&tmp_sndL[0], sz_msg, MPI_INT, mpi_left, 1, &tmp_rcvL[0],
                   sz_msg, MPI_INT, mpi_left, 0, MPI_COMM_WORLD,
                   &mpi_n3d_status);
    }

    if (mpi_right >= 0) {
      MPI_Sendrecv(&tmp_sndR[0], sz_msg, MPI_INT, mpi_right, 0, &tmp_rcvR[0],
                   sz_msg, MPI_INT, mpi_right, 1, MPI_COMM_WORLD,
                   &mpi_n3d_status);
    }
  } else {
    if (mpi_right >= 0) {
  ...
  if ((mpi_left >= 0 ? d->commsize[mpi_n3d_id][mpi_left] : 0) != tmp_rcvL[0] ||
    ...
    printf("%d => %d <= %d      %d <= %d => %d   (%d,%d)  (%d,%d)\n",
           (mpi_left >= 0 ? d->commsize[mpi_n3d_id][mpi_left] : 0), mpi_n3d_id,
           (mpi_right >= 0 ? d->commsize[mpi_n3d_id][mpi_right] : 0),
           (mpi_left >= 0 ? d->commsize[mpi_left][mpi_n3d_id] : 0), mpi_n3d_id,
           (mpi_right >= 0 ? d->commsize[mpi_right][mpi_n3d_id] : 0),
           tmp_sndL[0], tmp_sndR[0], tmp_rcvL[0], tmp_rcvR[0]);
  }
   ...
   *    the position of the corresponding entry in lm_rcvL and lm_rcvR,
   * respectively
   */

  /*! first rcvL */
  if (countL != 0) {
    d->strain.cellLMin = d->strain.cellLMax =
        d->strain.lm_rcvL[0] / d->geo.AtomsPerCellMax();
    d->strain.atomLMin = d->strain.atomLMax =
        d->strain.lm_rcvL[0] % d->geo.AtomsPerCellMax();
    ...
    for (ii = 0; ii < nml_iv_extent(d->strain.lm_rcvL); ii++) {
      ...
      if (l < d->strain.cellLMin)
        d->strain.cellLMin = l;
      if (l > d->strain.cellLMax)
        d->strain.cellLMax = l;
      if (m < d->strain.atomLMin)
        d->strain.atomLMin = m;
      if (m > d->strain.atomLMax)
        d->strain.atomLMax = m;
    }

    if (d->strain.cellLMax != d->strain.cellLMin) {
      d->strain.rcvL_lm =
          Imatrix((d->strain.cellLMax - d->strain.cellLMin + 1),
  ...
  /*! now rcvR */
  if (countR != 0) {
    d->strain.cellRMin = d->strain.cellRMax =
        d->strain.lm_rcvR[0] / d->geo.AtomsPerCellMax();
    d->strain.atomRMin = d->strain.atomRMax =
        d->strain.lm_rcvR[0] % d->geo.AtomsPerCellMax();
    ...
    for (ii = 0; ii < nml_iv_extent(d->strain.lm_rcvR); ii++) {
      ...
      if (l < d->strain.cellRMin)
        d->strain.cellRMin = l;
      if (l > d->strain.cellRMax)
        d->strain.cellRMax = l;
      if (m < d->strain.atomRMin)
        d->strain.atomRMin = m;
      if (m > d->strain.atomRMax)
        d->strain.atomRMax = m;
    }

    if (d->strain.cellRMax != d->strain.cellRMin) {
      d->strain.rcvR_lm =
          Imatrix((d->strain.cellRMax - d->strain.cellRMin + 1),
...
void calc_estrain_macopt(rvectr p, int sz, rvectr xi, void *data, int itercount,
                         real *onevalue) {
  /*! Comment_N: This function is called from macoptII_par(). macoptII_par is
  called as follows:             macoptII_par(p, n_loc, n_tot,
  (calc_estrain_macopt), (void*) d, &amac); Corresponding argument in
  macoptII_par() is void (*dfunc)(nml_dvector*, int, nml_dvector*, void*, int,
  double*). calc_estrain_macopt() calculates gradient. Actually this function
  calls function calc_estrain() which does final gradient calculation and stores
  it in d->strain.dp. calc_estrain_macopt() takes care of boundary conditions.
  */
  ...
  /*! Comment_N: Apply boundary conditions to vector p. Vector p has values
   * calculated in previous iteration. Only those components which are allowed
   * by the boundary conditions are used. All remaining elements are not used
   * for updating displacements and/or lattice constants. */
  ...
  case Keating_struct::AllFree:
    /*! Comment_N: All atoms are allowed to displace from their original
     * positions. i.e. all d->geo.l_m_2_atomdsp[ioff][j][k] are allowed to
     * change as per p[id] calculated from CG routine macoptII_par. */
    for (i = 0; i < d->cell_ln[mpi_n3d_id]; i++, ioff++) {
      for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
        if (d->geo.AtomType[ioff][j]) {
          for (k = 0; k < 3; k++) {
  ...
  case Keating_struct::FixSurfaceAll:
    /*! Comment_N: All atoms except those on the surface are allowed to
     * displace. */
    for (i = 0; i < d->cell_ln[mpi_n3d_id]; i++, ioff++) {
      for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
  ...
  case Keating_struct::FixZmin:
    /*! Comment_N: All atoms except those in the bottom-most (in zidirection)
     * unit cell are allowed to displace. */
    for (i = 0; i < d->cell_ln[mpi_n3d_id]; i++, ioff++) {
      for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
      ...
      for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
        if (d->geo.AtomType[ioff][j]) {
          for (k = 0; k < 3; k++) {
            ...
            /*               fout<< id<< "
             * "<<d->geo.l_m_2_atomdsp_onedomain[i][j][k]<<endl; */
#else  /*! DISPL_SINGLE_PRECISION */
            fout << id << " " << d->geo.l_m_2_atomdsp[ioff][j][k] << endl;
#endif /*! DISPL_SINGLE_PRECISION */
          }
  ...
  if (_new_minimize && do_min) {
    /*! Comment_N: If do_min is true (i.e. lattice const minimization is tured
     * on) then lattice constant latt[0-2] is allowed to vary. It may appear
     * that all three lattice constants are allowed to vary. latt[0-2] has
     * values of new lattice constants but only those lattice constants which
     * are permitted to vary according to boundary conditions are updated. This
     * is done below, please scroll down. */
    real latt[3];
    ...
    /*! if we are running the special case of Fixed interior pts, we
       ...
       of lattice constant */
    if (d->opt.ExecParam.Strain.Keating.BoundCond ==
        Keating_struct::FixInterior) {
      ioff = d->cell_s[mpi_n3d_id];
      for (i = 0; i < d->cell_ln[mpi_n3d_id]; i++, ioff++) {
        for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
          if (!d->geo.AtomType[ioff][j] && !d->geo.isOnSurface(ioff, j))
    ...
    /*! Comment_N: Impose boundary conditions on lattice constant. Only those
     * lattice constants which are permitted to vary accoring to the boundary
     * condtions are updated. Variables MinErtX, Y or Z are set from the input
     * deck. */
    if (d->strain.MinWrtX)
      d->geo.lattice_x = latt[0];
    if (d->strain.MinWrtY)
...
#endif /*! DISPL_SINGLE_PRECISION */
       /*! Note that if defined DISPL_SINGLE_PRECISION, displacements are only
        * stored    in an array encompassing only the atoms on current CPU, so it is
        * not possible    to store values from rcvL and rcvR */
  ...
  *onevalue = es = calc_estrain(d, p, rcvL, rcvR);
  /*! Comment_N: Function calc_estrain() does the actual gradient and strain
   * energy calculation. */
  ...
  if (d->opt.ExecParam.Strain.StrainOut.EnergyCGall &&
  ...
  if (mpi_n3d_id == mpi_n3d_masterid) {
    ...
    fp = fopen(filename, "a");
    str_free(filename);
    fprintf(fp, "%d %e %e %e \n", itercount, d->geo.lattice_x, d->geo.lattice_y,
            d->geo.lattice_z);
    fclose(fp);
  }

  /*! Comment_N: Now gradient and strain energy calculations are done. We update
   * only those components of vector xi (which is the gradient vector supplied
   * to CG routine) with calculated components of strain.dp vector which are
   * allowed to be used in minimization (as per the boundary condtions). */
  ioff = d->cell_s[mpi_n3d_id];
  switch (d->opt.ExecParam.Strain.Keating.BoundCond) {
  case Keating_struct::FixOne:
    first = (mpi_n3d_id == mpi_n3d_masterid ? 1 : 0);
  ...
  if (itercount && d->opt.ExecParam.Strain.Keating.strainCheckPointInterval &&
      (itercount % d->opt.ExecParam.Strain.Keating.strainCheckPointInterval) ==
          0) {
...
/*! Comment_N: This function does the actual gradient and strain energy
 * calculation. */
  ...
  int sz_loc =
      3 * d->cell_ln[mpi_n3d_id] * d->geo.AtomsPerCellMax() +
      (_new_minimize && !mpi_n3d_id && d->strain.doPeriodMinimization() ? 3
                                                                        : 0);
#ifdef INTEL_MODIFIED
  ...
  int szCommL =
      1 + (mpi_left >= 0 ? 6 * maxNbr * d->commsize[mpi_n3d_id][mpi_left] : 0);
  int szCommR =
      1 +
      (mpi_right >= 0 ? 6 * maxNbr * d->commsize[mpi_n3d_id][mpi_right] : 0);
...
#pragma message("INTEL_MODIFIED: Strain nnv[] allign ON")
  ...
  /*! Comment_N: Current value of the lattice constant. Note that we updated
   * d->geo.lattice_x etc in calc_estrain_macopt() */
  ...
  for (l = cell0; l < cellF; l++) {
    ...
    for (m = 0; m < d->geo.AtomsPerCellMax(); m++) {
      ...
      /*! Loop through the atom's nearest neighbors */
      for (n = 0; n < d->geo.Neighbors(m); n++) {
        /*! Comment_N: This loop calculated nearest neighbor vectors (nnv's).
         * nnv's calculated in this loop are used in later loops over nearest
         * neighbors. */
        /*! Get relative position of unit cell where this neighbor lives. */
        int i_nbr = i + d->geo.NbrCell(m, n, 0);
        int j_nbr = j + d->geo.NbrCell(m, n, 1);
        ...
        /*! Initialize neighbor status to zero.  This vector is used to insure
           ...
           upper triangular and  -1 if the matrix element is lower triangular.
           By including only upper or lower triangular contributions we insure
           that bond length distortion contributions are only counted once in
           the strain energy sum. */
        nstat[n] = 0;
        ...
        if (cindx >= 0) {
          ...
          if (AtomType_cindx_aindx) {
#ifdef DISPL_SINGLE_PRECISION
            /*! Comment_N: DISPL_SINGLE_PRECISION is defined in geo_struct.c.
             * Therefore following code will be executed. */
            /*! Here displacements are transmitted via displacements, rcvL , and
             * rcvR. We need to assign the atoms appropriately We know that the
             * cell number l is within the domain on this processor but we do
             * not know about the cell number cindx and we need to check it
             */

            /*! First deal with the atom [l][m] */
            int cellIndexInThisDomain = l - cell0;
            int entryNumber =
                cellIndexInThisDomain * d->geo.AtomsPerCellMax() * 3 + m * 3;
            ...
            int theEntry;
            if ((cindx >= cell0) &&
                (cindx < cellF)) { /*! this neighbor is on this CPU and we find
                                      it in displacements */

              int cellIndexInThisDomain = cindx - cell0;
              int entryNumber =
                  cellIndexInThisDomain * d->geo.AtomsPerCellMax() * 3 +
                  aindx * 3;
              rv1 = &(displacements[entryNumber]);
              /*! Comment_N: *rv1 = l_m_2_atomdsp for nbr [cindx][aindx] */

            } else if ((cindx >= d->strain.cellLMin) &&
                       (cindx <=
                        d->strain.cellLMax)) { /*! this neighbor is on the left
                                                  neighboring CPU and we find it
                                                  in rcvL */

              theEntry = d->strain.rcvL_lm[cindx - d->strain.cellLMin]
                                          [aindx - d->strain.atomLMin];
              if ((theEntry >= 0) && (d->strain.rcvL_lm != NULL)) {
                rv1 = &(rcvL[theEntry * 3]);
                /*! Comment_N: *rv1 = l_m_2_atomdsp for nbr [cindx][aindx] */
              } else {
                die("Problem with finding neighbors when calculating estrain, "
                    "rcvL");
              }
            } else if ((cindx >= d->strain.cellRMin) &&
                       (cindx <=
                        d->strain.cellRMax)) { /*! this neighbor is on the right
                                                  neighboring CPU and we find it
                                                  in rcvR */

              theEntry = d->strain.rcvR_lm[cindx - d->strain.cellRMin]
                                          [aindx - d->strain.atomRMin];
...
#else  /*! DISPL_SINGLE_PRECISION */
            /*! Comment_N: DISPL_SINGLE_PRECISION is defined in geo_struct.c.
             * Therefore these two statements will not be executed. */
            real *rv1 = d->geo.l_m_2_atomdsp[cindx][aindx];
            real *rv2 = d->geo.l_m_2_atomdsp[l][m];
#endif /*! DISPL_SINGLE_PRECISION */
            real *rv3 = d->geo.PositionInCell(aindx);
            /*! Comment_N: *rv3 = Position of nbr in its unit cell (i.e. cell
             * with index cindx.) PositionInCell have values like (0,0,0),
             * (0,-0.5,-0.5), (-0.25,-0.25,-0.25) etc */
            real *rv4 = d->geo.PositionInCell(m);
            /*! Comment_N: Same as rv3 but for atom m in cell l. */

            int *iv1 = d->geo.NbrCell(m, n);
            /*! Comment_N: *iv1 = relative position of unit cell of nbr
            (cindx,aindx) wrt unit cell of atom (l,m). NbrCell is a vecor of
            length 3 with elements as either 0 or 1. e.g.
            1. If nbr lies in the same unit cell as that of an atom then NbrCell
            = (0,0,0)
            2. If nbr lies in the unit cell which is just above (in z-direction)
            the unit cell of that atom then NbrCell = (0,0,1). */
            ...
            /*! Comment_N: Unstrained lattice constant of a atom-nbr combination
             * e.g. GaAs, SiGe, SiSi etc. These values are provided through the
             * input deck. If material is pure Si then alat = Si lattice
             * constant. */

            /*! Get relative real-space location for this neighbor.  There are
               two terms inside the brackets.  The first gives the relative
               location of the neighbor's unit cell.  The second gives the
               relative location of the neighbor's atom within the unit cell.
               The term disp is the atom's displacement due to strain */
            nnv_n = nnv[n];
            nnv0_n = nnv0[n];
            ...
            for (p = 0; p < 3; p++, rv1++, rv2++, rv3++, rv4++, iv1++) {
              ...
              /* Addition for ZB_110 */
              real alat_temp = alat;
              if (d->geo.isCellZB_110() || d->geo.isCellZB_111() ||
                  d->geo.isCellUserDef()) {
                alat_temp *= d->geo.get_a_latt_rot_norm(p);
              }
              /* End Addition for ZB_110 */

              /*! Comment_N: R_ij's in Keating energy expression. Note that
               * a_lattice has current values of lattice_x,y,z which are allowed
               * to vary depending on boundary conditions. */
            ...
            if (nnv0[n][0] == 0.0 && nnv0[n][1] == 0.0 && nnv0[n][2] == 0.0) {
              ...
              printf("W CellAtom_2_ijk[m]=(%e %e %e)\n",
                     d->geo.PositionInCell(m, 0), d->geo.PositionInCell(m, 1),
                     d->geo.PositionInCell(m, 2));
            }
            /////////////////////////////////////////////////////////////////////////////
            if ((d->opt.ExecParam.Strain.StrainModel ==
                 Strain_struct::VFF_keating_strained) &&
                1) {
              dssq = nnv_n[0] * nnv_n[0] + nnv_n[1] * nnv_n[1] +
                     nnv_n[2] * nnv_n[2];
              d0sq = nnv0_n[0] * nnv0_n[0] + nnv0_n[1] * nnv0_n[1] +
                     nnv0_n[2] * nnv0_n[2];

              alpha[n] = 0;
              beta[n] = 0;
              if (d0sq == 0.0)
                printf("ERROR:  d0sq vanishes!!!\n"), fflush(stdout);
              alpha[n] = d->strain.alpha_eff(AtomType_l_m, AtomType_cindx_aindx,
                                             dssq, d0sq, 1, false);
...
#pragma message("INTEL_MODIFIED: Strain, move invariant (part1) is ON")
      ...
      for (n = 0; n < d->geo.Neighbors(m); n++) {
        if (nstat[n] == 0)
          continue; //!  Check if neighbor exists
...
#define _CORRECTION_BOYKIN
        ...
        bldl = (dssq - d0sq);
        bld += bldc * bldl * bldl;
        /*! Comment_N: bld is bond length distortion contribution to the strain
         * energy. */
        /*! Derivative for minimization */
#ifdef INTEL_MODIFIED
#pragma message(                                                               \
    "INTEL_MODIFIED: Strain, move invariant, [n] prefetch (part2) is ON")
//! move invariant
#else
...
/*! Comment_N: strain.dp = dE/dR_ij, dE_dlatt = dE/da */
#else
        /*! Comment_N: INTEL_MODIFIED is defined. So following code will not be
         * executed. */
        for (p = 0; p < 3; p++)
        ...
        for (p = n + 1; p < d->geo.Neighbors(m); p++) {
          ...
          /*! d0dot == dot product between vectors pointing to undistorted atoms
             n and p dsdot == dot product between vectors pointing to strained
             atoms n and p den == product of undistorted distances to atoms n
             and p */
#ifdef INTEL_MODIFIED
#pragma message("INTEL_MODIFIED: Strain move invariant (part4) is ON")
          ...
          nnv_p = nnv[p];
          nnv0_p = nnv0[p];
          d0dot = nnv0_n[0] * nnv0_p[0] + nnv0_n[1] * nnv0_p[1] +
                  nnv0_n[2] * nnv0_p[2];
          dsdot =
              nnv_n[0] * nnv_p[0] + nnv_n[1] * nnv_p[1] + nnv_n[2] * nnv_p[2];
          den = nnv0_p[0] * nnv0_p[0] + nnv0_p[1] * nnv0_p[1] +
                nnv0_p[2] * nnv0_p[2];

          if (den == 0.0)
            printf("ERROR:  den vanishes!!!\n"), fflush(stdout);
          ...
          if ((d->opt.ExecParam.Strain.StrainModel ==
            ...
            Beta = d->strain.beta_eff(
                (int)d->geo.AtomType[l][m], //! type of the m-th atom
                (int)d->geo.AtomType[indxmat[n][0]]
                                    [indxmat[n][1]], //! type of the n1-th atom
                (int)d->geo.AtomType[indxmat[p][0]]
                                    [indxmat[p][1]], //! type of the n2-th atom
                nnv0[n],
                nnv0[p], //! vectors of the relaxed bonds of the V doublet
                nnv[n],
                nnv[p], //! vectors of the strained bonds of the V doublet
                1, //! use anharmonicity corrections for strain and acoustical
                   //! phonons
                false);
...
#pragma message("INTEL_MODIFIED: Strain arithmetic opt's is ON")
          ...
          //                nnv_p = nnv[p];
          //                nnv_n = nnv[n];
          /*! Factor of 2 added in the following 3 lines. Without factor of two
             these expressions are not correct. refer eqn 14 in C. Pryor et al.
             JAP vol 83, p 2548, Mar 1 1998. Factor of 3/8 is ignored in the
             code because it will not affect minimization procedure. */
          dE_dlatt[0] +=
              2 * bb_mul * (eta[n][0] * nnv_p[0] + eta[p][0] * nnv_n[0]);
          dE_dlatt[1] +=
              2 * bb_mul * (eta[n][1] * nnv_p[1] + eta[p][1] * nnv_n[1]);
          dE_dlatt[2] +=
              2 * bb_mul * (eta[n][2] * nnv_p[2] + eta[p][2] * nnv_n[2]);
#else
            Beta = sqrt(beta[n] * beta[p]);
          bbdc = Beta /*sqrt(beta[n]*beta[p])*/ / sqrt(den * d0sq);
          if (d->strain.debugp)
...
#define KLUDGE1                                                                \
  1.0 /*! Above line was a bug.                                                \
         For expressions of strain energy, refer eqn 14 in C. Pryor et al. JAP \
         vol 83, p 2548, Mar 1 1998. Factor of 3/8 is ignored in the code                                          \
         because it will not affect minimization procedure. */

          dE_dlatt[0] += KLUDGE1 * bbdc_bbdl_2 *
                         (eta[n][0] * nnv_p[0] + eta[p][0] * nnv_n[0]);
          dE_dlatt[1] += KLUDGE1 * bbdc_bbdl_2 *
                         (eta[n][1] * nnv_p[1] + eta[p][1] * nnv_n[1]);
          dE_dlatt[2] += KLUDGE1 * bbdc_bbdl_2 *
                         (eta[n][2] * nnv_p[2] + eta[p][2] * nnv_n[2]);
#endif
...
#pragma message("INTEL_MODIFIED: Strain, move invariant (part3) is ON")
          ...
          if (cn_this >= 0 && cn_this < d->cell_ln[mpi_n3d_id]) {
            int offset =
                3 * (cn_this * d->geo.AtomsPerCellMax() + indxmat[n][1]);
#ifdef INTEL_MODIFIED
/*! Comment_N: INTEL_MODIFIED is defined. So following code will be executed.
 * Note that following code and code in #else is same. The following code is
 * optimized to run faster. Same is true for all #ifdef INTEL_MODIFIED loops
 * below. */
#pragma message("INTEL_MODIFIED: Strain, small loop (1) vectorization is ON")
...
#else
            /*! Comment_N: INTEL_MODIFIED is defined. So following code will not
             * be executed. */
            for (q = 0; q < 3; q++) {
            ...
            die("invalid range: %d (%d %d) %d\n", mpi_n3d_id, mpi_left,
                mpi_right, indxmat[n][0]);
          }

          if (cp_this >= 0 && cp_this < d->cell_ln[mpi_n3d_id]) {
            int offset =
                3 * (cp_this * d->geo.AtomsPerCellMax() + indxmat[p][1]);
#ifdef INTEL_MODIFIED
#pragma message("INTEL_MODIFIED: Strain, small loop (2) vectorization is ON")
  ...
  if (!(Eloc <= 0) && !(Eloc > 0)) {
    printf("ERROR:  Eloc is Nan!!!! (ID=%d) . . .   ", mpi_n3d_id),
        fflush(stdout);
    if (!(bld <= 0) && !(bld > 0)) {
  ...
  if (!mpi_n3d_id) {
    ...
    for (ii = 0; ii < 3 * d->cell_ln[mpi_n3d_id] * d->geo.AtomsPerCellMax();
         ii++)
      nrm2 += d->strain.dp[ii] * d->strain.dp[ii];

    if (_new_minimize && d->strain.doPeriodMinimization()) {
      /*! Comment_N: last 3 entries of strain.dp on the master processor are
       * derivative dE/da. */
      ...
      d->strain.dp[sz_loc - 3] =
          (dE_dlatt_tot[0] + dE_dlatt_tot[1] + dE_dlatt_tot[2]) / 3.0 *
          SCALE_FCTR;
      d->strain.dp[sz_loc - 2] =
          (dE_dlatt_tot[0] + dE_dlatt_tot[1] + dE_dlatt_tot[2]) / 3.0 *
          SCALE_FCTR;
      d->strain.dp[sz_loc - 1] =
          (dE_dlatt_tot[0] + dE_dlatt_tot[1] + dE_dlatt_tot[2]) / 3.0 *
          SCALE_FCTR;
#endif
    }
    printf("\n  STRAIN:   E=%6.3e  a=(%f,%f,%f)", Etot, a_lattice[0],
           a_lattice[1], a_lattice[2]);
  ...
  if (mpi_n3d_numprocs > 1) {
    ...
    if (mpi_n3d_id % 2) {
      if (mpi_left >= 0) {
        MPI_Sendrecv(&Nsleft, 1, MPI_INT, mpi_left, 1, &Nrleft, 1, MPI_INT,
                     mpi_left, 0, MPI_COMM_WORLD, &mpi_n3d_status);
      }
      if (mpi_right >= 0) {
        MPI_Sendrecv(&Nsright, 1, MPI_INT, mpi_right, 0, &Nrright, 1, MPI_INT,
                     mpi_right, 1, MPI_COMM_WORLD, &mpi_n3d_status);
      }
    } else {
      if (mpi_right >= 0) {
        MPI_Sendrecv(&Nsright, 1, MPI_INT, mpi_right, 0, &Nrright, 1, MPI_INT,
    ...
    printf("%d <= [%d] => %d   %d => [%d] <= %d   (%d)\n", Nsleft, mpi_n3d_id,
           Nsright, Nrleft, mpi_n3d_id, Nrright,
    ...
    if (mpi_n3d_id % 2) {
      if (mpi_left >= 0) {
        MPI_Sendrecv(d->strain.sndL.data, Nsleft * szDatStr, MPI_BYTE, mpi_left,
                     1, d->strain.rcvL.data, Nrleft * szDatStr, MPI_BYTE,
                     mpi_left, 0, MPI_COMM_WORLD, &mpi_n3d_status);
      }
      if (mpi_right >= 0) {
      ...
      for (jj = 0; jj < d->geo.AtomsPerCellMax(); jj++) {
        if (d->geo.AtomType[II][jj]) {
          ...
          if (fabs(d->strain.dp[id]) > 1e-10) {
            printf("%d %d    ", II, jj);
            printf(" %e %e %e\n", d->strain.dp[id], d->strain.dp[id + 1],
                   d->strain.dp[id + 2]);
            fflush(stdout);
          }
        }
