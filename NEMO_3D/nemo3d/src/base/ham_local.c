...
*****************************************************************************
$Header: /repo/nemo3d/src/base/ham_local.c,v 1.24 2008/07/23 16:49:14 nkharche Exp $ 
*****************************************************************************/
                           ...
                           real phase5, real phase6, real phase7) 
{
   ...
   H_DiagStrainCorr_OffDiag_Wrapper(d, l, m, (int) d->geo.AtomType[l][m], n, l_nbr, m_nbr, 
                          nnv, nnv0, a_lattice);
   for (int ii=4; ii<8; ii++) {
      for (int r=0;r<d->NBasisStates;r++){
      for (int c=0;c<d->NBasisStates;c++){
         h[r+ii*d->NBasisStates][c+ii*d->NBasisStates].r += d->hd[r][c].r;
         h[r+ii*d->NBasisStates][c+ii*d->NBasisStates].i += d->hd[r][c].i;
      }}
   }
   ...
   H_DiagStrainCorr_OffDiag_Wrapper(d, l_nbr, m_nbr, (int) d->geo.AtomType[l_nbr][m_nbr], 
                          n_nbr, l, m, nnv, nnv0, a_lattice);
                                                ...
                                                int map_mb_to_m[8][4]) 
{
	...
	l_nbr = d->geo.ssmap[l][m][n];
      ...
      */
      ...
      for (int p=0; p < sp3d5s_nparam; p++)
         d->param[p] = 
            d->parmat[AtomType_nbr][AtomType_this][p];
      ...
      if (d->opt.ExecParam.ElCalc.include_strain_in_Hamiltonian) {
         real StrainShift = 
            d->strnmat[AtomType_this][AtomType_nbr][eta_an_shift];
         for (int r=0;r<d->NBasisStates;r++) {
   ...
   local_Hoff_zincBlende(d, h, l, m, map_mm_to_n, map_mb_to_m, a_lattice,
                         0, 2, 1, 3, d->kxL + d->kyL, d->kxL + d->kzL, 
                         d->kyL + d->kzL);
   local_Hoff_zincBlende(d, h, l, m, map_mm_to_n, map_mb_to_m, a_lattice,
                         1, 3, 0, 2, d->kyL, 0.0, d->kyL);
   local_Hoff_zincBlende(d, h, l, m, map_mm_to_n, map_mb_to_m, a_lattice,
                         2, 0, 3, 1, 0.0, d->kzL, d->kzL);
   local_Hoff_zincBlende(d, h, l, m, map_mm_to_n, map_mb_to_m, a_lattice,
                         3, 1, 2, 0, d->kxL, d->kxL, 0.0);
...
void local_bandstruct_zincBlende_cubic(qd_struct d) 
{
   ...
   for (int m=0; m < d->geo.AtomsPerCellMax() / 2; m++) {
      ...
      if (x < d->opt.ExecParam.Subdomain.xMin || 
          x > d->opt.ExecParam.Subdomain.xMax) continue;
      if (y < d->opt.ExecParam.Subdomain.yMin || 
          y > d->opt.ExecParam.Subdomain.yMax) continue;
      if (z < d->opt.ExecParam.Subdomain.zMin || 
          z > d->opt.ExecParam.Subdomain.zMax) continue;
	 ...
	 if (d->geo.isOnSurface(l,m))
      ...
      if (flag) {
         compute_local_hamiltonian_at_cation(h, d, l, m, a_lattice, 
                                             map_mm_to_n, map_mb_to_m);
      }
   ...
   if (d->_bandstruct_on_local_domain && fpLocal)  fclose(fpLocal);
   ...
   if (d->_bandstruct_on_full_domain) {
	 ...
	 char *filename=n3d_strdup_n(d->inputfile);
         ...
         sprintf(filetag, "nd_Ek_vca_kx=%.3f_ky=%.3f_kz=%.3f", d->kxL, d->kyL, d->kzL);
	 n3d_FileTypeSet(&filename, filetag, TRUE);
	 ...
	 if (cindx < 0 && ( cindx + d->geo.N_Cell+1 ) >= 0  ) {
      ...
      if (d->geo.isCellWurtzite()){
         ...
         if(pz_addition_energy !=0.0)
            h3d_diag_pz_addition_for_wurtzite_crystal_splitting
               (d, pz_addition_energy);
      }
      ...
      for (int n = 0; n < d->geo.Neighbors(m); n++ ) {
         ...
         if ( !d->geo.AtomType[cindx][aindx])  continue;
         ...
         d->geo.getRelativePosVec(nnv0, nnv, a_lattice_unstrnd, cindx, 
                                  aindx, l, m, n);
