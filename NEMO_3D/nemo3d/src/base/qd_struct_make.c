...
*****************************************************************************
$Header: /repo/nemo3d/src/base/qd_struct_make.c,v 1.39 2007/04/23 23:30:35 musman Exp $ 
*****************************************************************************/
...
cout_master <<"\nInitial check: reading version # from file "<< f_in<< endl;
cout_master <<"\nCurrently using version ID '"<< VERSION_ID <<"' (line 24 of"<<f_in<<"file)" << endl;
    FILE *fp;
...
//******* End check VERSION_ID *******
   if (!mpi_n3d_id) 
      cout << "\nParsing input file:  '" << f_xml << "' ('" << f_in << "')" << endl;
   ...
   try {
      parser->parse(f_xml);
      if (parser->getErrorCount() > 0)  errorsOccured = true;
   }
   ...
   if (!errorsOccured && !errReporter->getSawErrors()) {
      ...
      if (!pFile) 
         cerr << "UNABLE TO LOCATE PYTHON FILE:  " 
              << pyFuncFile << endl;
      ...
      if (!pDict) cerr << "pDict is NULL!!!" << endl;
...
/*
      {
      die("!!! INVALID INPUT !!!\n"\
"The version of the input file you are using (%s) is invalid and probably outdated."\
"You should update your input to version %s.  One can update the grammar to the latest "\
"version by using the script $HOME_NEMO3D/nemo3d/misc/update-nemo3d-input.pl.  "\
"Please see the commented lines at the top for usage instructions.  After applying the "\
"script one should carefully examine the newly generated input to make sure there are "\
"no unintended alterations to the physics or to the execution flow.\n",
          this->opt.Version.c_str(),VERSION_ID);
      cout_master  << "====================================================== \n"
      << "****************************************************** \n"
      << "The version of the input file you are using (this->opt.Version.c_str()) \n"
...
void QD_struct::checkInput() 
{
   ...
   if (this->opt.Dev.band_model==Dev_struct::Bands_1_s_nospin)
      cout_master << "WARNING:  check MPI communication for single orbital model.  Also look at d->n_atom_tot, d->geo.AtomsPerCellMax()";
   ...
   for (unsigned int i=0; i<this->opt.material_list.size(); i++) {
      ...
      if (m.mb_strain.size() != sp3d5s_nstrain) {
         die("size of (%dth) material %s strain parameter list (%d) != %d", 
             i, m.material.c_str(), m.mb_strain.size(), sp3d5s_nstrain);
      }
      
      if      (m.band_model=="Bands_20_sp3d5ss_spin" && m.mb_pv.size() != sp3d5s_nparam)
         die("Incorrect length of TB parameter array %d != %d\n", 
             m.mb_pv.size(), sp3d5s_nparam);
      else if (m.band_model=="Bands_10_sp3ss_spin" && m.mb_pv.size() != sp3s_nparam)
         die("Incorrect length of TB parameter array %d != %d\n", 
             m.mb_pv.size(), sp3s_nparam);
      else if (m.band_model=="Bands_10_sp3d5ss_nospin" && m.mb_pv.size() != sp3d5s_nparam)
         die("Incorrect length of TB parameter array %d != %d\n", 
             m.mb_pv.size(), sp3d5s_nparam);
   ...
   if (this->opt.ExecParam.Phys.MagneticFieldOn)
      masterPrint("%sMagnetic field is on  B=(%f,%f,%f) T", bullet,
                  this->opt.ExecParam.Phys.Bx,
                  this->opt.ExecParam.Phys.By,
