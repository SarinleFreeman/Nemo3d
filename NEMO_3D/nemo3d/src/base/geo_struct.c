...
*****************************************************************************
$Header: /repo/nemo3d/src/base/geo_struct.c,v 1.38 2008/07/23 16:47:14 nkharche Exp $ 
*****************************************************************************/
...
//   this->tiltdir[0] = tiltdir[0]; this->tiltdir[1] = tiltdir[1]; this->tiltdir[2] = tiltdir[2];
...
#define MEMCHECK
#ifdef MEMCHECK
   double int2_memory=0.0, ijk_cell_memory=0.0, dspl_memory=0.0, ssmap_memory=0.0, tl_memory=0.0;;
   ijk_cell_memory = Nx*Ny*Nz *4 / 1.0e6;      
   tl_memory += ijk_cell_memory;
   int2_memory  = N_Cell*N_MaxAtomCell *sizeof(int) *5 /1.0e6;  // 5 tensors of I2tensor
   tl_memory += int2_memory;
   ssmap_memory += N_Cell* N_MaxAtomCell* cellgeom.N_MaxNeighbor  *sizeof(int) /1.0e6;                // 1 tensor of I3tensor
   tl_memory += ssmap_memory;
   dspl_memory  = N_Cell*N_MaxAtomCell*3  *8/1.0e6;    // 1 r3tensor
   tl_memory += dspl_memory;
   printf("\n set_geo_struct memory allocation in MB ijk_cell_memory=%g int2_memory=%g dspl_memory=%g ssmap_mem=%g tl_memory=%g\n",ijk_cell_memory, int2_memory, dspl_memory, ssmap_memory, tl_memory),fflush(stdout);
#endif
   ...
   //NoNeed_isDiffNbr this->isDiffNbr             = B2tensor(0,N_Cell-1,0,N_MaxAtomCell-1);
   ...
   //NoNeed_isDiffNbr this->isDiffNbr             = I2tensor(0,N_Cell-1,0,N_MaxAtomCell-1);
...
void geo_struct::readStepRoughness(int Nxx, int Nyy) /* tilt_fix */
{
   ...
   printf("\n tilt_dir = [%d %d %d] NaddALZ = %d\n",this->dx,this->dy,this->dz,this->NaddALZ);
   printf("\n\nNxx = %d Nyy = %d\n\n",Nxx,Nyy);

   int No_steps2 = int(double(Nxx)/(double(this->dx)/2))+1;
//   printf("\n\nNo_steps2 = %d\n\n",No_steps2);
   this->delta_step = I2tensor(0,4*Nyy,0,No_steps2-1); /* This tensor has step roughness information */
...
//   this->adjThicknessY = this->lattice_y*(this->dy*NoTiltedUnitCellsInX/* + double(4-this->NaddALY)/4*/);
   this->adjThicknessZ = this->lattice_z*(this->dz*NoTiltedUnitCellsInX + double(4-this->NaddALZ)/4);
                            /* subtract these numbers from Ly and Lz entered in the input deck */
//printf("\n\nadjThicknessZ = %12.10le\n\n",this->adjThicknessZ);
   
/* tilt_fix
   if (this->dx==1 && this->dy==0 && this->dz==0 && this->NaddALZ==0) this->is_tilted = 1;
   else this->is_tilted = 1;
end tilt_fix */
...
#define MEMCHECK
   ...
   tl_memory += ijk_cell_memory;
   int2_memory  = N_Cell*N_MaxAtomCell *sizeof(int) *5 /1.0e6;  // 6 tensors of I2tensor
   int2_memory += N_Cell*2 *sizeof(int) /1.0e6;                // 1 tensor of I2tensor
   tl_memory += int2_memory;
   dspl_memory  = N_Cell*N_MaxAtomCell*3  *sizeof(double)/1.0e6;    // 1 r3tensor
   tl_memory += dspl_memory;
   ssmap_memory  = N_Cell*N_MaxAtomCell*N_MaxNbr  *sizeof(int)/1.0e6;    // 1 i3matrix
   tl_memory += ssmap_memory;
   printf("\n geometry memory allocation in MB ijk_cell_memory=%g int2_memory=%g dspl_memory=%g ssmap_memory=%g tl_memory=%g\n",ijk_cell_memory, int2_memory, dspl_memory, ssmap_memory, tl_memory),fflush(stdout);
   printf("\n memory datasize  sizeof(long)=%d sizeof(double)=%d, sizeof(float)=%d sizeof(int)=%d, sizeof(short)=%d, sizeof(char)=%d\n", (int) sizeof(long), (int) sizeof(double), (int) sizeof(float), (int) sizeof(int), (int) sizeof(short), (int) sizeof(char));fflush(stdout);
   ...
   printf("c_store128=%c c_store64=%c  testequal1=%d testnonequal1=%d testequal2=%d\n",
           c_store128,c_store64, 
          (((int) c_store128_64) & 128), 
          (((int) c_store128_64) & 64), 
          (((int) c_store64) & 64) ); fflush(stdout); 
#endif
   ...
   if (!veryfirst){
	...
	this->ijk__cell[i][j][k] = -1 - (this->N_Cell) - 1 ;
      ...
      this->l_m_2_offset_ham_tot  = I2tensor(0,N_Cell-1, 0,N_MaxAtomCell-1);  // This array is only used in the second geometry assignment


#ifdef  MEMORY_SAVE_NEWSTRUCT 
      //NoNeed_isDiffNbr this->isDiffNbr             = B2tensor(0,N_Cell-1, 0,N_MaxAtomCell-1);  // This array is only used in the second geometry assignment
      this->surfaceInfo           = B2tensor(0,N_Cell-1, 0,N_MaxAtomCell-1);   // This array is only used in the second geometry assignment   --- will be assigned values of 4,8,16,32,64,128 to indicate the position of the surface position


#else   /* MEMORY_SAVE_NEWSTRUCT  */

      //NoNeed_isDiffNbr this->isDiffNbr             = I2tensor(0,N_Cell-1, 0,N_MaxAtomCell-1);  // This array is only used in the second geometry assignment
      this->surfaceInfo           = I2tensor(0,N_Cell-1, 0,N_MaxAtomCell-1);   // This array is only used in the second geometry assignment   --- will be assigned values of 4,8,16,32,64,128 to indicate the position of the surface position
#endif  /* MEMORY_SAVE_NEWSTRUCT  */
...
void geo_struct::Allocate_NOSTRAIN(int N_Cell, int Nx, int Ny, int Nz, int veryfirst, 
                          const Cell& cellgeom)
{
   const int LARGE_INT = 9999999;
   const int N_MaxAtomCell = cellgeom.AtomsPerCellMax();
...
bool geo_struct::InvalidateStrainOnly() 
{
   ...
   for ( int l=0; l < g.N_Cell; l++ ) {
	 ...
	 if ((g.AtomType[l][m] != 0) && (!g.atom_strain_only[l][m])) {
	    ...
	    indx_map[Zcell] = l;
	    Zcell++;
            if (g.cell__ijk[l][0] > xMax)  xMax=g.cell__ijk[l][0];
            if (g.cell__ijk[l][1] > yMax)  yMax=g.cell__ijk[l][1];
            if (g.cell__ijk[l][2] > zMax)  zMax=g.cell__ijk[l][2];
            if (g.cell__ijk[l][0] < xMin)  xMin=g.cell__ijk[l][0];
            if (g.cell__ijk[l][1] < yMin)  yMin=g.cell__ijk[l][1];
            if (g.cell__ijk[l][2] < zMin)  zMin=g.cell__ijk[l][2];
	    break;
	 }
   }
   ...
   for ( int l=0; l < this->N_Cell; l++ ) {
      ...
      for ( int m=0; m < g.cellgeom.AtomsPerCellMax(); m++ ) {
         ...
         if(this->atom_strain_only[l][m]) this->AtomType[l][m] = 0;
         else this->AtomType[l][m] = g.AtomType[ indx_map[l] ][m];
...
/* Additional variables for tilting the lattice */
   this->dx = di->dx; this->dy = di->dy; this->dz = di->dz;
...
/* End: Additional variables for tilting the lattice */
   ...
   for ( int l=0; l < this->N_Cell; l++ ) {
      ...
      for ( int m=0; m < di->AtomsPerCellMax(); m++ ) {
       ...
       /* MEMCHECKclip  atom_strain_only has not been assigned in earlier initializations! */
...
/* Addition for tilt */
void geo_struct::isNbrOut(int l, int m, int n,
                          bool *_isNotInXnegtv, bool *_isNotInXpostv,
                          bool *_isNotInYnegtv, bool *_isNotInYpostv,
                          bool *_isNotInZnegtv, bool *_isNotInZpostv)
{
   ...
   double posnbr[3];
   double* Ratom = this->PositionInCell(aindx); /* position of this neighbor in the unit cell */
   ...
   if(this->isCellZB_111()) {
      xminn = this->lattice_x * (cell_xmin + this->PositionInCell(/*aindx=*/3,0));
      xmaxx = this->lattice_x * (cell_xmax + this->PositionInCell(/*aindx=*/0,0));
     }
   else {
      xminn = this->lattice_x * (cell_xmin + this->PositionInCell(/*aindx=*/6,0));
      xmaxx = this->lattice_x * (cell_xmax + this->PositionInCell(/*aindx=*/0,0));
     }

   /* End Neerav's addition for tilting ZB_111 */

   double yminn = ((posnbr[0]/this->lattice_x)*this->lattice_y)*(double(this->dy)/double(this->dx)) + delta;
                  /* exclude all atoms on lower boundary */
   double ymaxx;
   ymaxx = yminn + this->lattice_y*(this->cell_ymax-this->cell_ymin+1+this->dy);// - this->adjThicknessY;
                  /* include all atoms on upper boundary */

   double zminn = ((posnbr[0]/this->lattice_x)*this->lattice_z)*(double(this->dz)/double(this->dx)) + delta;
                  /* exclude all atoms on lower boundary */

//   if(shape_rank==2) { // If this line is uncommented roughness is introduced in Si QW only
      // adjust step edges for roughness
/*      double stepLen = (this->lattice_x/4)/(double(this->dz)/double(this->dx));
      int delta_indx=int(posnbr[1]/(this->lattice_y/4)); // row index in delta_step
      double Delta;
...
//printf("\n\nstepNo = %d stepNo2 = %d delta_indx = %d this->Ny_Cell = %d\n\n",stepNo,stepNo2,delta_indx,this->Ny_Cell);

      Delta = delta_step[delta_indx-1][stepNo2]*this->lattice_x/4;

//printf("\ndelta_indx = %d stepNo = %d stepNo2 = %d Delta = %12.10le stepLen = %12.10le",delta_indx,stepNo,stepNo2,Delta,stepLen);

      if(posnbr[0]>(double(stepNo2))*2*stepLen && posnbr[0]<(double(stepNo2)+1)*2*stepLen) {
         if(Delta>0 && stepNo==(2*stepNo2+1)) {
            if(posnbr[0]>(double(stepNo))*stepLen && posnbr[0]<(double(stepNo))*stepLen+Delta)
            zminn = (stepNo)*this->lattice_z/4 - delta;
           }
         if(Delta<0 && stepNo==2*stepNo2) {
            if(posnbr[0]>(stepNo+1)*stepLen+Delta && posnbr[0]<(stepNo+1)*stepLen)
            zminn = (stepNo+2)*this->lattice_z/4 - delta;
           }
        }
//     } /* if shape_rank */

   double zmaxx;
      zmaxx = zminn + this->lattice_z*(this->cell_zmax-this->cell_zmin+1+this->dz) - this->adjThicknessZ;
                  /* include all atoms on upper boundary */
...
#undef MEMCHECK
   ...
   ssmap_memory = this->N_Cell*this->AtomsPerCellMax()*this->NeighborsMax() *4 / 1.0e6;      
   tl_memory += ssmap_memory;
   printf("\n SupercellReinit  allocation in MB ssmap_memory=%g tl_memory=%g\n",ssmap_memory, tl_memory),fflush(stdout);
#endif


   // reallocate memory and reinitialize to -1
   this->ssmap = I3matrix(this->N_Cell, this->AtomsPerCellMax(), 
                          this->NeighborsMax());
   ...
   for (int l=0; l < this->N_Cell; l++) {
	 ...
	 for (int n = 0; n < this->Neighbors(m); n++) {
	    ...
	    // if virtual cell is in fact real, no need to look further
            if (!this->is_tilted) { /* Connect atoms as follows when lattice is NOT tilted */
       	     if ( cindx >= 0 ) continue;
	     ...
	     in = _isNotInX ? 
                i - (i_nbr - i)*
                (this->cell_xmax - this->cell_xmin)/this->unitcelldivide 
                : i_nbr;
	     jn = _isNotInY ? 
                j - (j_nbr - j)*
                (this->cell_ymax - this->cell_ymin)/this->unitcelldivide 
                : j_nbr;
	     kn = _isNotInZ ? 
                k - (k_nbr - k)*
                (this->cell_zmax - this->cell_zmin)/this->unitcelldivide 
                : k_nbr;
             } /*if !is_tilted */

            if (this->is_tilted) { /* Connect atoms as follows when lattice is tilted */
             ...
             double delta = 1e-6;
             int thickNoY = int((this->lattice_y*(this->cell_ymax-this->cell_ymin+1+this->dy)+ delta /*- this->adjThicknessY*/)/this->lattice_y);
             int thickNoZ = int((this->lattice_z*(this->cell_zmax-this->cell_zmin+1+this->dz)+ delta - this->adjThicknessZ)/this->lattice_z);
 
             int noUnitCellsX = int((this->cell_xmax - this->cell_xmin + 1)/this->dx);
 
             if (_isNotInXnegtv) {
                 in = i - (i_nbr - i)*(this->cell_xmax - this->cell_xmin)/this->unitcelldivide;
             ...
             if(_isNotInYnegtv) jn = jn + thickNoY/this->unitcelldivide;
             else if (_isNotInYpostv) jn = jn - thickNoY/this->unitcelldivide;
             else jn = jn;
 
             if(_isNotInZnegtv) kn = kn + thickNoZ/this->unitcelldivide;
             else if (_isNotInZpostv) kn = kn - thickNoZ/this->unitcelldivide;
             else kn = kn;
            } /*if is_tilted */

#ifdef ELIMINATE_SSMAP
            if(!this->is_tilted)
	       this->ijk__cell[i_nbr][j_nbr][k_nbr]= this->ijk__cell[in][jn][kn]- (this->N_Cell)-1 ;
#else /* ELIMINATE_SSMAP */
            this->ssmap[l][m][n] = this->ijk__cell[in][jn][kn];
//printf("ssmap[%d][%d][%d] = %d in=%d jn=%d kn=%d\n",l,m,n,this->ssmap[l][m][n],in,jn,kn);
#endif /* ELIMINATE_SSMAP */
	 }
      }
   }
  ...
  // Loop through all unit cells in the structure
   for (int l=0; l < this->N_Cell; l++) {
	 ...
	 if (!this->AtomType[l][m])  continue;
         ...
         if (i == this->cell_xmin)  this->surfaceInfo[l][m] += 4;
         if (i == this->cell_xmax)  this->surfaceInfo[l][m] += 8;
         if (j == this->cell_ymin)  this->surfaceInfo[l][m] += 16;
         if (j == this->cell_ymax)  this->surfaceInfo[l][m] += 32;
         if (k == this->cell_zmin)  this->surfaceInfo[l][m] += 64;
         if (k == this->cell_zmax)  this->surfaceInfo[l][m] += 128;

         /* Added for tilt */
//         if (j == this->cell_ymin+1)  this->surfaceInfo[l][m] += 16;
//         if (j == this->cell_ymax-1)  this->surfaceInfo[l][m] += 32;
         if (k == this->cell_zmin+1)  this->surfaceInfo[l][m] += 64;
         if (k == this->cell_zmax-1)  this->surfaceInfo[l][m] += 128;
         /* End Added for tilt */
...
int geo_struct::remove_few_bonded_atoms(int bond_to_remove)
{
   ...
   if ( this->hasPeriodicity() && bond_to_remove>0) {
      die("May not remove single-bonded atoms under periodic boundary conditions (remove_few_bonded_atoms)");
   }
   ...
   if (removed_atoms>0) 
      cout_master << "Removed " << removed_atoms 
                  << " atoms with " << bond_to_remove << " bond from list\n";
...
int geo_struct::remove_user_defined_atoms(void)
{
     ...
     for (int Za=0; Za < this->AtomsPerCellMax(); Za++ ) {
        if(Zc==0 && Za==1) continue;
        else if(Zc==2 && Za==0) continue;
        else if(Zc==2 && Za==1) continue;
        else if(Zc==2 && Za==2) continue;
        else if(Zc==6 && Za==1) continue;
        else { this->AtomType[Zc][Za] = 0;  removed_atoms++;}
     }}
     return removed_atoms;
}
...
int geo_struct::remove_other_surface_species()
{
   ...
   if (this->N_Cell>1 || this->AtomsPerCellMax()>1){
      for (int i=0; i < this->N_Cell; i++) {
      for (int j=0; j < this->AtomsPerCellMax(); j++) {
         if (!this->isOnSurface(i,j)) continue;

         if (this->AtomType[i][j]== EL_As || 
             this->AtomType[i][j]==EL_Sb ||
             this->AtomType[i][j]==EL_Se ||
             this->AtomType[i][j]==EL_Ge ) {
            // homo 2 
...
void geo_struct::changeLattConst(real ax_new, real ay_new, real az_new) 
{
   ...
   for (int Zatom=0; Zatom < this->AtomsPerCellMax(); Zatom++ ) {
      if ( !this->AtomType[Zcell][Zatom] ) continue;
...
#ifdef   MEMORY_SAVE_NEWSTRUCT  
   if(this->AtomType) rm_b2tensor(&this->AtomType,0);
   this->AtomType = B2tensor(0,0,0,this->AtomsPerCellMax()-1);
#else    /* MEMORY_SAVE_NEWSTRUCT  */
   if(this->AtomType) rm_i2tensor(&this->AtomType,0);
...
ostream& operator << (ostream& o, const geo_struct& g)
{
   ...
   o << "lattice constant (x,y,z) = (" 
     << g.lattice_x << "," << g.lattice_y << "," << g.lattice_z << ")\n";
   ...
   for (int l=0; l<g.N_Cell; l++) {
      for (int m=0; m<g.AtomsPerCellMax(); m++) {
        o << setw(4) << (g.atom_strain_only[l][m] ? 'X' : ' ') 
          << setw(4) << g.AtomType[l][m]
           << (g.isOnSurface(l,m) 
               ? 'S' 
   ...
   for (int l=0; l<g.N_Cell; l++) {
      ...
      for (int m=0; m<g.AtomsPerCellMax(); m++) {
         o << setw(6) << Zr[0] << "  "
           ...
           << setw(6) << Zr[2] << "     "
           << setw(6) << m << "  "
           << setw(6) << g.l_m_2_offset_ham_tot[l][m] << "  "
           << setw(10) << g.l_m_2_atomdsp[l][m][0] << "  "
           << setw(10) << g.l_m_2_atomdsp[l][m][1] << "  "
           << setw(10) << g.l_m_2_atomdsp[l][m][2] << "\n";
      }
      o << "\n";
   }
...
void geo_struct::getPosition_unstrained(real* pos, real unstrnd_latt, 
                                        int Zcell, int Zatom) {
      int* Rcell = cell__ijk[Zcell];
      real* Ratom = PositionInCell(Zatom);
      if(isCellWurtzite()) {
        pos[0] = unstrnd_latt * ( 0.5*(Rcell[0]+Rcell[1]) + Ratom[0] );
        pos[1] = unstrnd_latt * ( 0.5*sqrt(3.0)*(Rcell[0]-Rcell[1]) + Ratom[1] );
        pos[2] = unstrnd_latt * sqrt(8.0/3.0) * ( Rcell[2] + Ratom[2] );
      }

/* Added for ZB_110 */
      else if(isCellZB_110() || isCellZB_110_small() ||isCellZB_111() || isCellUserDef()) {
...
#define MEMCHECK2
      ...
      if (l_m_2_atomdsp){
          ...
          if(isCellWurtzite()) {
            ...
            pos[1] = lattice_y * ( 0.5*sqrt(3.0)*(Rcell[0]-Rcell[1]) + Ratom[1] ) + dR[1];
...
void geo_struct::getRelativePosVec(real* nnv0, real* nnv, real unstrnd_latt, int c1, int a1,
                                   int c2, int a2, int neighbor){
  if(cellgeom.IsCellWurtzite){
      real normalized[3];
      normalized[0] = 0.5*(NbrCell(a2,neighbor,0)+NbrCell(a2,neighbor,1))
                      + PositionInCell(a1,0) - PositionInCell(a2,0) ; 
      normalized[1] = 0.5*sqrt(3.0)*(NbrCell(a2,neighbor,0)-NbrCell(a2,neighbor,1)) 
                      + PositionInCell(a1,1) - PositionInCell(a2,1) ; 
      normalized[2] = NbrCell(a2,neighbor,2) 
                      + PositionInCell(a1,2) - PositionInCell(a2,2) ; 
      nnv0[0] = unstrnd_latt*normalized[0];
      nnv0[1] = unstrnd_latt*normalized[1];
      nnv0[2] = unstrnd_latt*sqrt(8.0/3.0)*normalized[2];
      
      nnv[0] = lattice_x*normalized[0] 
               + l_m_2_atomdsp[c1][a1][0] - l_m_2_atomdsp[c2][a2][0];
      nnv[1] = lattice_y*normalized[1] 
               + l_m_2_atomdsp[c1][a1][1] - l_m_2_atomdsp[c2][a2][1];
      nnv[2] = lattice_z*normalized[2] 
               + l_m_2_atomdsp[c1][a1][2] - l_m_2_atomdsp[c2][a2][2];
...
/* New ZB_110 unit cell */
  else if(cellgeom.IsCellZB_110 || cellgeom.IsCellZB_110_small || cellgeom.IsCellZB_111 ||cellgeom.IsCellUserDef) { /* Added for ZB_110 */
    for (int p=0; p < 3; p++) {
       real normalized;
       normalized =  NbrCell(a2,neighbor,p) + PositionInCell(a1,p) - PositionInCell(a2,p);
       if(p==0)
         { nnv0[p] = get_a_latt_rot_norm(0) * unstrnd_latt*normalized;
           nnv[p] = lattice_x*normalized + l_m_2_atomdsp[c1][a1][p] - l_m_2_atomdsp[c2][a2][p];
         }
       else if(p==1)
         { nnv0[p] = get_a_latt_rot_norm(1) * unstrnd_latt*normalized;
           nnv[p] = lattice_y*normalized + l_m_2_atomdsp[c1][a1][p] - l_m_2_atomdsp[c2][a2][p];
         }
       else if(p==2)
         { nnv0[p] = get_a_latt_rot_norm(2) * unstrnd_latt*normalized;
           nnv[p] = lattice_z*normalized + l_m_2_atomdsp[c1][a1][p] - l_m_2_atomdsp[c2][a2][p];
         }
    }
