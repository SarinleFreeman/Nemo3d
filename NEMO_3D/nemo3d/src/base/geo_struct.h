...
*****************************************************************************
$Header: /repo/nemo3d/src/base/geo_struct.h,v 1.36 2008/07/23 16:47:30 nkharche Exp $ 
*****************************************************************************/
...
class geo_struct 
{
    ...
    * that's needed for construction of a single unit cell. The value = 2 when the
    * structure is zincblend or diamond fcc and 1 otherwise.*/
...
/* End: Additional variables to tilt the lattice */
    ...
    */
   i3tensor ijk__cell; 
   /*!< Integer 3D array. ijk_cell[i][j][k] returns the index of cell whose relative location of unitcell is (i,j,k) */
   i2tensor cell__ijk; 
   /*!< Integer 2D array. If index of a cell is indx, cell_ijk[indx][0] returns i, cell[indx][1] returns j
    * and cell[indx][2] returns k. */
...
#else   /* MEMORY_SAVE_NEWSTRUCT  */
   i2tensor AtomType;
   /*!< 2D integer array. Atoptype[Cell index][Atom index] = 1 if atom is filled in.
    * See mat_def.h for more information.
    * If MEMORY_SAVE_NEWSTRUCT is defined, boolean type is used. */
   i2tensor ShapeType;
   /*!< No more use in Nemo3D. ShapeType[Cell index][Atom index] = 1 if atom is filled in
    * This was originally used to determine shape of structure. Shapetype start from
    * box shape. Then, according to the defined shape, NEMO3D fills 1 in part of this array 
    * where should be filled by atom. If MEMORY_SAVE_NEWSTRUCT is defined, boolean type is used. 
    */ 
   
   i2tensor isDiffNbr;
   /*!< 2D interger array. 
    * - isDiffNbr[cindx][aindx] = 1 
    *   - the atom is surrounded by atoms of different material. Parameters for Hamiltonian setup should be averaged.
    * - isDiffNbr[cindx][aindx] = 0 
    *   - the atom is surrounded by atoms of same material. Parameters for Hamiltonian doens't need to be averaged here. 
    * - If MEMORY_SAVE_NEWSTRUCT is defined, boolean type is used. */
   
   i2tensor atom_strain_only;
   /*!< 2D interger array.
    * - atom_strain_only[cindx][aindx] = 1 
    *   - the atom is in the region of so called 'Strain calculation only' region. 
    * - atom_strain_only[cindx][aindx] = 0
    *   - atom is not only in 'Strain calculation' region, but also in 'Electronic calculation' region.
    * - If MEMORY_SAVE_NEWSTRUCT is defined, boolean type is used. */
    ...
    *   (i.e : number of basis * number of atoms in unit cell * number of unitcell) 
    *   Therefore, when we want to access wave function array using a single index (l,m),
    *   it could be extremely difficult to calculate correspondent index.
    *   l_m_2_offset_ham_tot[cindx][aindx] has offset index of the atom with cell index of cindx and atomic index of aindex,
    *   therefore we can access wave function array by using simple index such as (l_m_2_offset_ham_tot[l][m] + i), where
    *   i is number of i-th basis. */
   ...
   /*!< Displacement info in one domain. Will not be used if  DISPL_SINGLE_PRESISTION isn't defined. */ 
#else /* DISPL_SINGLE_PRECISION */
   r3tensor l_m_2_atomdsp ;
   ...
   i3matrix ssmap;
   /*!< 3D integer array. 
    * ssmap[c][a][n] have index of a unit cell where n-th neighbor atom of a-th atom in this cell belongs. */
   
#endif /* ELIMINATE_SSMAP */

  
   geo_struct() : periodicity(0),  unitcelldivide(0), surfaceInfo(NULL), 
      N_Cell(0), cell_xmin(0), cell_ymin(0), cell_zmin(0), 
      cell_xmax(0), cell_ymax(0), cell_zmax(0), 
/* Additional variables to tilt the lattice */
/*tiltdir(NULL),*/ dx(0), dy(0), dz(0), is_tilted(0), /*NaddALY(0),*/ NaddALZ(0),/* adjThicknessY(0),*/ adjThicknessZ(0), delta_step(NULL),/*adjustedLenX(0),*/
/* End: Additional variables to tilt the lattice */
lattice_x(0), lattice_y(0), 
      lattice_z(0), ijk__cell(NULL), cell__ijk(NULL), 
      AtomType(NULL), 
      ShapeType(NULL), 
      isDiffNbr(NULL), 
      atom_strain_only(NULL), 
      l_m_2_offset_ham_tot(NULL), 
      l_m_2_atomdsp(NULL)
#ifndef ELIMINATE_SSMAP
...
/* Additional variables to tilt the lattice */
/*g.tiltdir,*/ g.dx, g.dy, g.dz, g.is_tilted,/* g.NaddALY,*/ g.NaddALZ,/* g.adjThicknessY,*/ g.adjThicknessZ, g.delta_step,/*g.adjustedLenX,*/
/* End: Additional variables to tilt the lattice */
                     g.lattice_x, g.lattice_y, g.lattice_z, g.unitcelldivide,
                     g.cellgeom, g.ijk__cell, g.cell__ijk, 
                     g.AtomType, 
                     g.ShapeType, 
                     g.isDiffNbr, 
                     g.surfaceInfo, 
                     g.atom_strain_only,  
                     g.l_m_2_offset_ham_tot, 
                     g.l_m_2_atomdsp
#ifndef ELIMINATE_SSMAP
   ...
   geo_struct& operator =(const geo_struct &g) { 
      set_geo_struct(g.periodicity, g.N_Cell, g.cell_xmin, g.cell_ymin, 
                     g.cell_zmin, g.cell_xmax, g.cell_ymax, g.cell_zmax, 
/* Additional variables to tilt the lattice */
/*g.tiltdir,*/ g.dx, g.dy, g.dz, g.is_tilted,/*g.NaddALY,*/ g.NaddALZ, /*adjThicknessY,*/ g.adjThicknessZ, g.delta_step,/*g.adjustedLenX,*/
/* End: Additional variables to tilt the lattice */
                     g.lattice_x, g.lattice_y, g.lattice_z, g.unitcelldivide,
                     g.cellgeom, g.ijk__cell, g.cell__ijk, 
                     g.AtomType, 
...
/* End: Additional functions to tilt the lattice */
   ...
   /*!< Allocate memories and Set initial values to the structure. But variables for strain calculation, 
    * (i.e: 3D array l_m_2_atomdsp) is not initialized here  */

   void InitializeFromInputFile(const top_struct& opt);
   /*!< Information such as lattice constant and periodicity should be directly read from input xml code.
    * This information comes to top_struct and is saved in dev_struct, which is a member class of top_struct.
    * This function allocate such information to geo_struct. 
   ...
   /*!< Allocate space for geo_struct and copies information from parameter. It copies
    * geo_struct::ijk__cell, geo_struct::Atoptype, geo_struct::atom_strain_only and 
    * geo_struct::cell_(x,y,z)(min,max). 
    * @param di object of class geo_struct which has geometric information to be used here. 
    * @see geo_struct::geo_struct() */

   void isNbrOut(int l, int m, int n, bool *_isNotInXnegtv,  bool *_isNotInXpostv,
                 bool *_isNotInYnegtv, bool *_isNotInYpostv,
                 bool *_isNotInZnegtv, bool *_isNotInZpostv); /* Additional function to tilt the lattice */
   
   void SupercellReInit();
   /*!< Initialize the super cell coupling map, which contains three indices ssmap[i][j][k].
    * i&j index an atome's unit cell and atom repectively. k is a nearest neighbor's index 
    * for the atom indexed by i&j. The mapping is to a unit cell. */
   
   int get_cindx_tilted(int l,int m,int n);
   /*!< Determine the unit cell number of neighbor n of atom (l,m) when lattice is tilted */

   void fill_is_DiffNeighbor();
   /*!< Fill the 2D array geo_struct::isDiffNbr which is used in the hamiltonian construction 
    * to check whether or not the parameter vector needs to be averaged over different neighbors. 
    * @see geo_struct::isDiffNbr */
   
   void FindSurfaceAtoms();
   /*!< scan all atoms in structure and chedck if an atom is on the surface of the structure. 
    * It fills geo_struct::surfaceInfo.
    * @see geo_struct::surfaceInfo */
   
   bool isInvalidPeriodicity();
   /*!< checks if periodicity is valid in the structure. If structure was formed such that 
    * periodicity is valid, will return true. Otherwise return false.
    * @return true if structure is periodic. */
   
   int   remove_few_bonded_atoms(int bond_to_remove);
   /*!< Function scans through the whole structure and removes atoms that have only a certain 
    * set of bonds.  if "bond_to_remove" is set to 0, we will remove all the atoms that have no 
    * neighbors. if "bond_to_remove" is set to 1 we will remove the atoms that have only a single neighbor.
   ...
   /*!< Function scans through the whole structure and removes a certain species of atoms from the surface. 
    * If the structure's periodic condition is open and the type of atom on the surface is one of EL_As, EL_Sb, 
    * EL_Se and EL_Ge, the atom will be removed from the surface.
   ...
   /*!< remove user-defined atoms. Criteria is defined with use of set of [cell index][atom index].
    * @return # of removed atoms */
   
   void adjust_for_periodic_bc();
   /*!< This function records the periodic boundary coupling information into the super cell map.
    * if neighbor cell does't exist in real, we create virtual cell and set the index such that 
    * periodic boundary condition is virtually satisfied. */
   ...
   /*!< this function constructs geometric information such as cell_ijk, cell_(x,y,z)(min,max) 
    * for this structure. This function is operated on each shape defined in the input xml file. 
    * After excution, it calls geo_struct::FillShape. This, should be seen as sorts of preparation
    * for 'real' filling of structure. 
    * @param shape This will give information of shape of each structure defined as "ShapeName" in input deck.
    * @param opt This has information of crystal structure type 
    * @param mat vector of objects of MaterialHandle class. It has various information of materials.
   ...
   /*!< Loops over unitcells & atoms. assigns atom types if atom is in the shape. To check this,
    * IsNotInShape function is used.
    * @param shape This will give information of shape of each structure defined as "ShapeName" in input deck.
    * @param mat vector of objects of MaterialHandle class. It has various information of materials.
    ...
    * @return returns position vector of cell where n-th neighbor of m-th atom in this cell is belonged.
    * @see Cell::nnmap
    */
   ...
   /*!< @param m atomic index
    * @param n neighbor index
    * @param d direction. 0 for x, 1 for y, 2 for z and 3 for the actual index of atom. 
    * @return position in the selected direction. Nbr(m,n,3) returns atomic index. */
    ...
    * @return component at (i,j) of a matrix which transforms initial basis (S,Px,Py,Px) to the set of hybridized orbital SP3. (i.e: weighted combination of initial basis.) */
   
   /* Rotation matrix from new unit cell to standard Zincblende basis. */
   double get_T_rot(int i, int j) const
      { return cellgeom.T_rot[i][j];}
...
/* End Normalized lattice constant for rotated unit cells. */
   ...
   bool isCellZB_110() const { return cellgeom.IsCellZB_110; } /* Added for ZB_110 */
   /*!< @return true if the crystal structure is ZB_110. */
   bool isCellZB_110_small() const { return cellgeom.IsCellZB_110_small; } /* Added for ZB_110_small */
   /*!< @return true if the crystal structure is ZB_110_small. */
   bool isCellZB_111() const { return cellgeom.IsCellZB_111; } /* Added for ZB_111 */
   /*!< @return true if the crystal structure is ZB_111. */
   bool isCellUserDef() const { return cellgeom.IsCellUserDef; } /* Added for CellUserDef */
   ...
   /*!< set the first 3 bits of periodicity to zero. This invalidate structural periodicity.
    * @see geo_struct::periodicity */
   void setPeriodicityToEl() 
      { zeroPeriodicity();  periodicity += (periodicity & 448)>>6; }
   /*!< Make periodicity of structure same as that of electric calculation. */
   void setPeriodicityToStrain() 
      { zeroPeriodicity();  periodicity += (periodicity & 56)>>3; }
   ...
   bool isCation(int l, int m) const {
      ...
      if (AtomType_lm==EL_As || AtomType_lm==EL_Sb 
          || AtomType_lm==EL_Se || AtomType_lm==EL_N 
          || AtomType_lm==EL_O || AtomType_lm==EL_P
          || AtomType_lm==EL_S )
         return false;
   ...
   int phaseInfoForNemo1D(int l, int m, int n) {
    ...
    /*!< Calculate and allocate value of PhaseInfo. It determines phaseInfo as follows.
    *  - Relative x location of neighbor atom is -1 and current atom is on xmin (cell) of structure
    *   - Electronic calculation is periodic in x direction : PhaseInfo = (PhaseInfo) | 01
    *   - Electronic calculation is not periodic in x direction : PhaseInfo = (PhaseInfo) | 0100
    *  - Relative x location of neighbor atom is 1 and current atom is on xmax (cell) of structure
    *   - Electronic calculation is periodic in x direction : PhaseInfo = (PhaseInfo) | 02
    *   - Electronic calculation is not periodic in x direction : PhaseInfo = (PhaseInfo) | 0200
    *  - Relative y location of neighbor atom is -1 and current atom is on ymin (cell) of structure
    *   - Electronic calculation is periodic in y direction : PhaseInfo = (PhaseInfo) | 04
    *   - Electronic calculation is not periodic in y direction : PhaseInfo = (PhaseInfo) | 04
    *  - Relative y location of neighbor atom is 1 and current atom is on ymax (cell) of structure
    *   - Electronic calculation is periodic in y direction : PhaseInfo = (PhaseInfo) | 010
    *   - Electronic calculation is not periodic in y direction : PhaseInfo = (PhaseInfo) | 01000
    *  - Relative z location of neighbor atom is -1 and current atom is on zmin (cell) of structure
    *   - Electronic calculation is periodic in z direction : PhaseInfo = (PhaseInfo) | 020
    *   - Electronic calculation is not periodic in z direction : PhaseInfo = (PhaseInfo) | 02000
    *  - Relative z location of neighbor atom is 1 and current atom is on zmax (cell) of structure
    *   - Electronic calculation is periodic in z direction : PhaseInfo = (PhaseInfo) | 040
    *   - Electronic calculation is not periodic in z direction : PhaseInfo = (PhaseInfo) | 04000
    *    
    * @param l cell index
    * @param m atomic index
    * @param n neighbor index
    * @return phase information.
    * @brief functional description only. Don't understand physical meaning quite well. */

   real phaseBloch(real kxL, real kyL, real kzL, int l, int m, int n) {
      real phase = 0.0;
   if (!is_tilted) { /* Calculate Bloch phase as follows when lattice is NOT tilted */
       const int* dCell = NbrCell(m,n);
       if ( isInX_min(l,m) && dCell[0]==-1 )  phase -= kxL;
       if ( isInX_max(l,m) && dCell[0]== 1 )  phase += kxL;
       if ( isInY_min(l,m) && dCell[1]==-1 )  phase -= kyL;
       if ( isInY_max(l,m) && dCell[1]== 1 )  phase += kyL;
       if ( isInZ_min(l,m) && dCell[2]==-1 )  phase -= kzL;
       if ( isInZ_max(l,m) && dCell[2]== 1 )  phase += kzL;
      } /* if !is_tilted */
...
//           if(isPeriodicY()) phase -= ((double)dy/(double)dx)*kxL;
           if(isPeriodicZ()) phase -= (((double)dz*lattice_z)/((double)dx*lattice_x))*kzL;   /* Modified for ZB_111: scaled by lattice constants */
          }
       if (_isNotInXpostv)  {
           phase += kxL;
//           if(isPeriodicY()) phase += ((double)dy/(double)dx)*kxL;
       ...
       if (_isNotInYnegtv)  { phase -= kyL; }
       if (_isNotInYpostv)  { phase += kyL; }
       if (_isNotInZnegtv)  { phase -= kzL; }
       if (_isNotInZpostv)  { phase += kzL; }
      } /* if is_tilted */

      return -Pi * (phase - 2*int(0.5*phase));
   }
   /*!< Seems that this function shifts phase by some constant value if neighbor cell is outside real structure.
    * According to the bloch theorm, kxL,kyL,kzL should be integer multiple of 2PI. But this function multiply PI before 
    * returning its final phase value. Therefore kxL,kyL,kzL here have a little different value from what we understand
    * from books. Need more study to understand what 'real value' of these are.
    *  
    * @param kxL not sure exactly. Seems that phase shift in x-direction between two adjacent unit-cell.
    * @param kyL not sure exactly. Seems that phase shift in x-direction between two adjacent unit-cell.
    * @param kzL not sure exactly. Seems that phase shift in x-direction between two adjacent unit-cell.    
    * 
    * @param l cell index
    * @param m atomic index
    * @param n neighbor index
    * @return amount of phase shift. can't understand equation for conversion exactly.  
    * @brief Don't understand physical meaning quite well. */
   
   real phasePeriodic(real kxL, real kyL, real kzL, 
                      int i_nbr, int j_nbr, int k_nbr) {
      real phase = 0.0;
      if (i_nbr < this->cell_xmin)  phase -= kxL;
      if (i_nbr > this->cell_xmax)  phase += kxL;
      if (j_nbr < this->cell_ymin)  phase -= kyL;
      if (j_nbr > this->cell_ymax)  phase += kyL;
      if (k_nbr < this->cell_zmin)  phase -= kzL;
      if (k_nbr > this->cell_zmax)  phase += kzL;
      return -Pi * (phase - 2*int(0.5*phase));
   }
   /*!< What this function does is exactly same (functionally) as that geo_struct::phaseBloch does.
    * But while phasBloch function shifts phase with use of (cindx, aindx, nindx),
    * this does so with use of (i,j,k) of neighbor cell.
    *
    * @param kxL not sure exactly. Seems that phase shift in x-direction between two adjacent unit-cell.
    * @param kyL not sure exactly. Seems that phase shift in x-direction between two adjacent unit-cell.
    * @param kzL not sure exactly. Seems that phase shift in x-direction between two adjacent unit-cell.    
   ...
   void getPosition_unstrained(real* pos, real unstrnd_latt, int Zcell, int Zatom);
    ...
    * @return position vector. pos[0],pos[1],pos[2] will have current atomic position in x,y,z respectively. */ 
    ...
    * @see geo_struct::getPosition_strained for description of parameters and returned value. */
   void getRelativePosVec(real* nnv0, real* nnv, real unstrnd_latt, 
                          int c1, int a1, int c2, int a2, int neighbor);
   /*!< get the relative position vector between two atoms. 
    * - Function first will calculate relative atomic position of a1,a2 in a unit cell. If neigbor atom of a2 
    *   and atom a2 are in the same unit cell, result is simply the relative posion of atom a1 to atom a2 in a 
    *   unit cell. Otherwise, function will add +1 or -1 to the result, which becomes then relative position of 
    *   atom a1 in a unitcell to atom a2 in the neighbor unitcell. Normalized result will be converted in a unit 
    *   of unstraind lattice constant and stored in nnv0.
    * - In unstrained environment, all the relative positions of atom X's are same in any unitcells. But once 
    *   strain starts to affect the structure, the displacements of atoms X due to strain become different with 
    *   different indice of unitcell. Therefore, we need to get displacement with consideration of cell index of 
    *   each unit cell. The result in a strained-structure will be saved in nnv.     
    *   
    *    
