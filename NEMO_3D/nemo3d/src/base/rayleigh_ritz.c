...
*****************************************************************************
$Header: /repo/nemo3d/src/base/rayleigh_ritz.c,v 1.22 2005/12/24 03:37:14 gekco
Exp $
*****************************************************************************/
...
void linSolv_CGNE(cvectr x, qd_struct d, real shift, cvectr r, real tol,
  ...
  // argList[0] = (void*) &d;
  // argList[1] = (void*)
  // &(d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.EigValGuess[indx]); double
  // shift1 = 0.0; argList[1] = shift1; argList[2] = (void*) &indx; argList[3] =
  // (void*) &eval_scoperti;
...
void rayleighQuotientIter(qd_struct d) {
  ...
  for (int indx = 0; indx < (signed int)d->WF.size(); indx++) {
    ...
    masterPrint("----------------------------------------------------------");
    masterPrint("Rayleigh Quotient Iteration:  eigenvalue guess[%d] = %e", indx,
                eval_guess);
    masterPrint("----------------------------------------------------------");
...
real calc_rayleigh_sq(rvectr p, void *data) {
  ...
  Hmult_spds_complete(
      y, d, d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.EigValGuess[0], x);
  Hmult_spds_complete(
      z, d, d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.EigValGuess[0], y);
...
#endif /* MPI3d */
{
  ...
  real shift =
      d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz
          .EigValGuess[d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.RunningIndex];
...
int min_rayleigh(qd_struct d) {
  const int &Nguess =
      d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.EigValGuess.size();
  ...
  cmacopt_defaults(&camac, d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.MaxIter,
  ...
  macopt_defaults(&amac, d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.MaxIter,
                  d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.verbose,
                  d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.tol,
                  d->opt.ExecParam.ElCalc.ElAlg.RayleighRitz.ConvCrit);
  ...
  for (int indx = 0; indx < Nguess; indx++) {
...

    {
      ...
      macoptII_par((rvectr)p, 2 * d->seg_ln[mpi_n3d_id], 2 * n_tot,
                   (gradRayleighQuotient), argList, (macopt_args *)&amac);
    ...
    // then use this info
    if (d->opt.ExecParam.ElCalc.ElAlg.ResFind ==
      ...
      masterPrint("### found a match:  indx=%d E=(%e,%e)\n", closest_indx,
                  amac.track_one_value, d->WF[closest_indx].E);
