...
int ctracemin_driver(qd_struct d){
  ...
  if(d->rc==NULL) d->rc = Cvectr(d->nvmap[mpi_n3d_id]*d->NBasisStates);  //kept this line for parpack_driver.c, not sure what it's for???
  int  neig = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.NumEigVal;        //number of eigenvalues wanted                                                          
  real refE = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.RefEigVal;        //shift the matrix A for it to be hermitian positive definite (A - refE*I) 
  real tol  = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.Tolerance;        //accuracy wanted 
  real seig = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.WantEigRange[0];  // min. of interval where eigenvalues are wanted
  real leig = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.WantEigRange[1];  //max. of interval where eigenvalues are wanted
  real saeig= d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.AllEigRange[0];   // min. all eigenvalues
  real laeig= d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.AllEigRange[1];   // max all eigenvalues
  int maxi  = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.maxi;
  int chebdegree = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.ChebyDeg;   // degree of chebyshev polynomial
  ...
  //real seig;                                                            //smallest eigenvalue you are looking for
  //real leig;                                                            //largest eigenvalue you are looking for
  //real saeig;                                                           //smallest of alleigenvalues
  ...
  ////else{
    refE = 0.0;
  ////}
  //seig = emin1;                ////emin1 - refE; ///-30-refE; //emin1 - refE;//???                   //define an interval where to look for the eigenvalues
  //leig = emax2;                ////emax2 - refE; ///30-refE;  //emax2 - refE;//???                   //THIS PARAMETER AND ABOVE ONE CAN BE AN OPTION IN THE INPUT FILE!!!
  //saeig= SMALLER_THEN_ALL_EIG; ////SMALLER_THEN_ALL_EIG - refE;                                      //define an interval in which all eigenvalues are contained  
  //laeig= LARGER_THEN_ALL_EIG;  ////LARGER_THEN_ALL_EIG  - refE;                                      //THIS PARAMETER AND ABOVE ONE CAN BE AN OPTION IN THE INPUT FILE!!!
  gamma= min(DTWO/((saeig-seig)*(saeig-leig)), DTWO/((laeig-seig)*(laeig-leig)));
  nloct=neig*nloc;                                                      //number of elements of all vectors combined on this processor 
  //maxi       = 40;  //50;   (comment by marta)                                           //THIS PARAMETER CAN BE AN OPTION IN THE INPUT FILE!!! 
  //chebdegree = 240; //6;                                                //THIS PARAMETER CAN BE AN OPTION IN THE INPUT FILE!!!
  ...
  for(j=1; j<=4*chebdegree; j++){
    ...
    #ifdef MATRIX_MULT
       map_chebyshev_shift_wrapper(&n,&nloc,&neig,&Vn[0][0],&Q[0][0],&R[0][0],&Vp[0][0],&S[0][0],&chebdegree,&gamma,&seig,&leig,&j,&mcsinfo);
	#else
       map_chebyshev_shift_wrapper(&n,&nloc,&neig,Vn,Q,R,Vp,S,&chebdegree,&gamma,&seig,&leig,&j,&mcsinfo);
	#endif
  ...
  for(i=1; i<=4*maxi; i++){ //every iteration requires two matrix-vector multiplies (the algorithm will stop when needed by itself)

    #ifdef MATRIX_MULT
       zgtracemin_wrapper(&n,&neig,&Y[0][0],E,&Q[0][0],&Qt[0][0],H,Hloc,G,ZWork,RWork,IWork,&R[0][0],&tol,&maxi,&i,&nloc,&rank,&np,&comm,&info);
	#else
       zgtracemin_wrapper(&n,&neig,Y,E,Q,Qt,H,Hloc,G,ZWork,RWork,IWork,R,&tol,&maxi,&i,&nloc,&rank,&np,&comm,&info);
	#endif
    ...
    if (info == 3){
      printf("--- Algorithm reached maximum # of iterations --- (iteration=%d)\n",i/2);
      break;
    }
    if (info < 0){
      printf("--- Error in the zgtracemin routine --- (iteration=%d; info=%d)\n",i/2,info);
      break; 
    }
  }
  maxi=i/2; 
  ...
  #ifdef MATRIX_MULT
     pcextract_eigenpairs_wrapper(&Y[0][0],E,&neig,&nloc,H,Hloc,ZWork,RWork,&T[0][0],&rank,&comm,&einfo);
     Hmatmult_spds_complete(T, d, refE, Y, neig);
     pcextract_eigenpairs_wrapper(&Y[0][0],E,&neig,&nloc,H,Hloc,ZWork,RWork,&T[0][0],&rank,&comm,&einfo);
  #else
     pcextract_eigenpairs_wrapper(Y,E,&neig,&nloc,H,Hloc,ZWork,RWork,T,&rank,&comm,&einfo);
  ...
  printf("print the eigenvalues and the norm of the estimate of the residual\n");
  for (i=0; i<neig; i++){
  ...
  */
  ...
  int chebdegree = d->opt.ExecParam.ElCalc.ElAlg.PCTracemin.ChebyDeg;   // degree of chebyshev polynomial
  int n      = d->n_ham_tot;                                            //dimension of eigenproblem                          
  int nloc   = 0;/*d->seg_ln[mpi_n3d_id];*/                                   //number of vector elements on current processor                                         
  int info;                                                             //flag for reverse communication; initialized to 0                                            
  //declare important MPI parameters
  //MPI_Fint comm = MPI_Comm_c2f(MPI_COMM_WORLD);                         //transform C into Fortran communicator (currently tracemin uses MPI_COMM_WORLD and comm doesn't matter)
  int comm = 0;
  ...
  #ifdef MATRIX_MULT
     pcextract_eigenpairs_wrapper(&Y[0][0],E,&neig,&n,H,Hloc,ZWork,RWork,&T[0][0],&rank,&comm,&einfo);
     Hmatmult_spds_complete(T, d, refE, Y, neig);
     pcextract_eigenpairs_wrapper(&Y[0][0],E,&neig,&n,H,Hloc,ZWork,RWork,&T[0][0],&rank,&comm,&einfo);
  #else
     pcextract_eigenpairs_wrapper(Y,E,&neig,&n,H,Hloc,ZWork,RWork,T,&rank,&comm,&einfo);
     //pcfmv(M,Y,T,neig,nloc,ne,rank,np)
     for (k=0; k<neig; k++){
