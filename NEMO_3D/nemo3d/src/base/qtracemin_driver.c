...
int qtracemin_driver(qd_struct d) {
  ...
  if (d->rc == NULL)
    d->rc = Cvectr(d->nvmap[mpi_n3d_id] *
                   d->NBasisStates); // kept this line from parpack_driver.c,
                                     // not sure what it's for???
  int neig = d->opt.ExecParam.ElCalc.ElAlg.PQTracemin
                 .NumEigVal; // number of eigenvalues wanted
  real refE = d->opt.ExecParam.ElCalc.ElAlg.PQTracemin
                  .RefEigVal; // shift the matrix A so that you are finding
                              // eigenvalues closest to the shift
  real tol =
      d->opt.ExecParam.ElCalc.ElAlg.PQTracemin.Tolerance; // accuracy wanted
  real cgtol =
      d->opt.ExecParam.ElCalc.ElAlg.PQTracemin.cgtol; // initial tol of GC
  int maxi = d->opt.ExecParam.ElCalc.ElAlg.PQTracemin
                 .maxi; // this was set to 40, in line 48
  int cgmaxi = d->opt.ExecParam.ElCalc.ElAlg.PQTracemin
                   .cgmaxim; // Number of iterations of CG
...
// allocate necessary temporary arrays
  ...
  // int maxi; //maximum number of iterations of tracemin (there are neig*2*6*2
  ...
  // maxi    =40;  (commented by marta) //maximum number of tracemin iterations
  // (SHOULD BE AN OPTION IN THE INPUT FILE) cgtol   = DONE;    (commented by
  // marta)                                                   //initial cg
  // tolerance is set to 1.0    (SHOULD BE AN OPTION IN THE INPUT FILE)
  ...
  for (i = 1; i <= 2 * maxi + 1;
       i++) { // every iteration requires one matrix vector multiply and one
              // system solve with matrix M=A^2
#ifdef MATRIX_MULT
    ztracemin_wrapper(&n, &neig, &Y[0][0], E, &Q[0][0], H, Hloc, ZWork, RWork,
                      IWork, &R[0][0], &tol, &maxi, &i, &nloc, &rank, &np,
                      &comm, &info);
#else
    ztracemin_wrapper(&n, &neig, Y, E, Q, H, Hloc, ZWork, RWork, IWork, R, &tol,
                      &maxi, &i, &nloc, &rank, &np, &comm, &info);
#endif
    if (info == 1) {
    ...
    if (info == 2) {
      ...
      // start CG
      for (j = 1; j <= cgmaxi + 2;
           j++) { // notice that one iteration is for setup
#ifdef MATRIX_MULT
        modified_conjugate_gradient_wrapper(
            &n, &neig, &X[0][0], &Y[0][0], &Qcg[0][0], G, Gloc, &P[0][0],
            &Rcg[0][0], gamma_old, rcgtol, &j, &cgmaxi, &cgtol, &tol, &nloc,
            &rank, &np, &comm, &cginfo);
#else
        modified_conjugate_gradient_wrapper(
            &n, &neig, X, Y, Qcg, G, Gloc, P, Rcg, gamma_old, rcgtol, &j,
            &cgmaxi, &cgtol, &tol, &nloc, &rank, &np, &comm, &cginfo);
#endif
        if (cginfo == 1) {
...
// call pcfmv(M,T,Qcg,neig,nloc,ne,rank,np)
        ...
        if (cginfo == 3) {
          printf("%d,modified CG has reached maximum number of iterations at "
                 "iteration %d\n",
                 rank, j);
          break;
        }
    ...
    if (info == 4) {
      printf("%d,--- Maximum number of iterations reached ---(iteration=%d; "
             "info=%d)\n",
             rank, i / 2, info);
      break;
    }
    if (info < 0) {
      printf(
          "%d,--- Error in the ztracemin routine --- (iteration=%d; info=%d)\n",
          rank, i / 2, info);
      break;
    }
...
// call pcfmv(M,Y,T,neig,nloc,ne,rank,np)
  ...
  printf(
      "%d,print the eigenvalues and the norm of the estimate of the residual\n",
      rank);
  for (i = 0; i < neig; i++) {
#ifdef MATRIX_MULT
  ...
  */
  ...
  //  //kept this line from parpack_driver.c, not sure what it's for???
  int neig = d->opt.ExecParam.ElCalc.ElAlg.PQTracemin
                 .NumEigVal; // number of eigenvalues wanted
  real refE = d->opt.ExecParam.ElCalc.ElAlg.PQTracemin
                  .RefEigVal; // shift the matrix A so that you are finding
...
// allocate necessary temporary arrays
  ...
  int i, j, k, cginfo; //,cgmaxi;
                       // real cgtol; //parameter used by tracemin
  int nloct;           // total number of elements on this processor: nloc*neig
  // ina maxi; //maximum number of iterations of tracemin (there are neig*2*6*2
  // matvecs per iteration)
  complex ytloc; // temporary variables used in computation of eigenvalues after
                 // tracemin (next 4 lines)
  complex yyloc;
  ...
  eigenvalues into a file
    ...
    //put back the eigenvectors
    if(d->opt.ExecParam.ElCalc.ElAlg.PQTracemin.CalEigVec) {
      d->wfc = Cmatrix(d->neigv, n);                                 //d->neigv
  was ncv, but I think it was a mistake and should've been nev in the original
  version??? for (i=0; i<d->neigv; i++){ for (j=0; j<n; j++){ #ifdef MATRIX_MULT
             d->wfc[i][j].r = Y[i][j].r;
             d->wfc[i][j].i = Y[i][j].i;
          #else
