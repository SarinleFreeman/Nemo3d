...
*****************************************************************************
$Header: /repo/nemo3d/src/base/phon_parpack.c,v 1.9 2007/01/26 05:28:35 lee509
Exp $
*****************************************************************************/
...
int phon_parpack(qd_struct d,
  ...
  // translate a C communicator handle into a Fortran handle to the same
  // communicator.
  ...
  while (TRUE) {
    ...
    MPI_TIC(Tstart);
    pznaupd(&comm, &ido, bmat, &nloc, which, &nev, &tol, resid, &ncv, vloc,
...
#ifdef MPI_TIMING
    MPI_TIC(Tstart);
    pzneupd(&comm, &rvec, &c, select, d1, vloc, &ldv, &sigma, workev, bmat,
            &nloc, which, &nev, &tol, resid, &ncv, vloc, &ldv, iparam, ipntr,
