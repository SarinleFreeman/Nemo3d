...
*****************************************************************************
$Header: /repo/nemo3d/src/base/phon_parpack.c,v 1.9 2007/01/26 05:28:35 lee509 Exp $ 
*****************************************************************************/
...
#define EnergyScale 0.0065821222 //=hbar[6.5821222*10^-16 eV*s^2]*sqrt{VFFC[1 N/m]/M_atom[10^-26 kg]}

//#define PR_EIGENVALUES_CONVERGED
//#define PR_PARPACK_PARAMETERS

int phon_parpack( qd_struct d,
                  int iq_ph//number of the point in the reciprocal space
                  )
{
   int num_states=3*d->n_atom_tot;//d->phon.N_cols[mpi_n3d_id];//total number of states
   int minn=find_min_ivec_mpi(d,d->phon.N_cols);
   int nev = (d->opt.ExecParam.PhonCalc.N_branches >= num_states) ?
       num_states-1 : d->opt.ExecParam.PhonCalc.N_branches;
   int ncv = ((nev*3>=num_states) ? num_states : 3*nev);
   if(ncv>minn){//Ensure the same size of the Krylov's subspace on all processors
       ncv=minn;
       if(nev>ncv-2) {
           nev=ncv-2;
       }
   ...
   // translate a C communicator handle into a Fortran handle to the same communicator.
   int ido=0; /* for reverse comm.; initialized to 0 */
   char bmat[]="I"; /* standard eigval problem */
   int n=num_states; /* dimension of eigenproblem */
   int nloc=d->phon.N_cols[mpi_n3d_id];//d->phon.seg_col_ln[mpi_n3d_id];//The number of elements of the vector V that result in nonzero product on the processor
   char * which;
   ...
   iparam[2] = d->opt.ExecParam.PhonCalc.PhonParpack.MaxIter; /* max # arnoldi update iter */
   ...
   d->phon.comm_vectors_allocate();//Allocates memory for parts of the eigenvector that participate in communication
   
   int dummy=0,NiterLooked=20;//NiterLooked determines the # of Arnoldy itteration print-outs
   double Tstart=0.0;

   div_t result;
   real precision=((d->opt.ExecParam.PhonCalc.PhonParpack.Tolerance>0.0 && d->opt.ExecParam.PhonCalc.PhonParpack.Tolerance<1e-10)?
                   1000*d->opt.ExecParam.PhonCalc.PhonParpack.Tolerance : 1e-10);//precision for check of
   ...
   printf("\nmpi_n3d_id=%d : pznaupd & pzneupd parameters:"
          ...
          mpi_n3d_id,
          comm,ido,bmat,which,tol,rvec,
          nev,ncv,nloc,ldv,lworkl);
   ...
   while (TRUE){
     ...
     MPI_TIC(Tstart);
     pznaupd(&comm,
             &ido, bmat, &nloc,which, &nev, &tol, resid, &ncv, vloc, 
     ...
     if (ido == -1 || ido == 1) {
       ...
       if(!result.rem)
         masterPrint("phon_parpack : %d-th time going to multiply...",dummy);
   ...
   if (info < 0) {
      printf("ERROR in pznaupd of PARPACK_SOLVER %d  (# eig=%d) (n=%d)\n", info, iparam[4], n);
   }
   else {
      if(mpi_n3d_id==0){
#ifdef PR_PARPACK_PARAMETERS
        cout << endl;
        cout << " ===== Results of PZNAUPD in phon_parpack===== " << endl << endl;
...
#endif// PR_PARPACK_PARAMETERS
#ifndef PR_PARPACK_PARAMETERS
        cout << "Converged " << iparam[4]
             << " from requested " << nev
             << " " << which
             <<" eigenvalues" << endl;
#endif// PR_PARPACK_PARAMETERS
      ...
      MPI_TIC(Tstart);
      pzneupd(&comm,
              &rvec, &c, select, d1, vloc, &ldv, &sigma, workev,
              bmat, &nloc, which, &nev, &tol, resid, &ncv, vloc, &ldv,
   ...
   if (info <0 ) {
      printf("ERROR in pzneupd of PARPACK SOLVER %d \n", info);
   }
     ...
     if (!mpi_n3d_id)  {
       ...
       for ( i=0; i<iparam[4]; i++){//loop over eigenvalues with
         ...
         if(d1[i].i>precision){
           masterPrint("!!!!!!!Large imaginary part of %d-th eigenvalue!!!!!!!",i);
           flag=true;
         }
         if(d1[i].r<0){
           if(d1[i].r<-precision){
             masterPrint("!!!!!!!Large negative real part of %d-th eigenvalue!!!!!!!",i);
             flag=true;
           }
           else
             d->phon.E_ph[iq_ph][i+3]=0.0;//Substitute small negative eigenvalue with zero
         }
         else
           d->phon.E_ph[iq_ph][i+3]=EnergyScale*sqrt(d1[i].r);//write the scaled to eV
       ...
       if(flag){//if something is wrong with eigenvalues
         masterPrint("Parpack thinks that there are %d eigenvalues converged:",iparam[4]);
   ...
   if(info>=0 && iparam[4]>0) {
     ...
     if(d->opt.ExecParam.Output.PsiSqrGnuPlot) writePsiSqr(d->wfc, d->neigv, d, 0);
     if(d->opt.ExecParam.Output.FullEigvec) 
       for(int ie=0; ie<d->neigv; ie++) writeEigenvector(d, d->wfc[ie], ie);        
   ...
   */
   ...
   rm_cvectr(&workl);
   rm_rvectr(&rwork);
   d->phon.comm_vectors_deallocate();//Allocates memory for parts of the eigenvector that
