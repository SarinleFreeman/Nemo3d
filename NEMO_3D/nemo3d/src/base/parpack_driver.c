...
*****************************************************************************
$Header: /repo/nemo3d/src/base/parpack_driver.c,v 1.24 2007/01/26 05:25:21 lee509 Exp $ 
*****************************************************************************/
...
//   if(mpi_n3d_numprocs==1) verify_ham(d);
   ...
   iparam[2] = d->opt.ExecParam.ElCalc.ElAlg.Parpack.MaxIter; /* max # arnoldi update iter */
   ...
   if (info < 0) {
      ...
      if(mpi_n3d_id==0){ 
        cout << endl;
        cout << " ====== Results of PZNAUPD ===== " << endl << endl;
   ...
   if (info <0 ) {
      printf("ERROR in pzneupd of PARPACK SOLVER %d \n", info);
       ...
       for (int i=0; i<iparam[4]; i++) cout<<i<<" "<<d1[i].r+refE<<" "<<d1[i].i<<endl;
       cout<<endl;
   }
   ...
   if(info>=0 && iparam[4]>0 && d->opt.ExecParam.ElCalc.ElAlg.Parpack.CalEigVec) {
