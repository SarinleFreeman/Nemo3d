...
*****************************************************************************
$Header: /repo/nemo3d/src/base/nemo3d_entry.c,v 1.25 2007/08/10 15:03:33 baeh
Exp $
*****************************************************************************/
    ...
    if (mpi_n3d_id == mpi_n3d_masterid) {
      ...
      for (cpu = 0; cpu < mpi_n3d_numprocs; cpu++) {
        if (cpu == mpi_n3d_masterid) {
          ...
          MPI_Recv(&rec_data[0], mpiTiming.out_data_length, MPI_DOUBLE, cpu,
                   cpu, MPI_COMM_WORLD, &mpi_n3d_status);
        }
      ...
      }
      {
        ...
        if (mpi_n3d_numprocs > 1) {
          ...
          fprintf(ftime,
                  "# CPU(min/max), total (min/max), setup (min/max), strain "
                  "(min/max), electr (min/max), fileio (min/max), sendrecev "
                  "(min/max), allreduce (min/max), barrier (min/max), bcast "
                  "(min/max), send(min/max), receive (min/max), matmul "
                  "(min/max), strain_sendrecv (min/max), strain_allreduce "
                  "(min/max), strain_barrier (min/max), strain_bcast "
                  "(min/max), strain_send (min/max), strain_recv (min/max), "
                  "electr_sendrecv (min/max), electr_allreduce (min/max), "
                  "electr_barrier (min/max), electr_bcast (min/max), "
                  "electr_send (min/max), electr_recv (min/max)\n");
        ...
        fprintf(
            ftime,
            "# cpu total, setup, strain, electr, fileio, sendrecev, allreduce, "
            "barrier, bcast, send, receive, matmul, strain_sendrecv, "
            "strain_allreduce, strain_barrier, strain_bcast, strain_send, "
            "strain_recv, electr_sendrecv, electr_allreduce, electr_barrier, "
            "electr_bcast, electr_send, electr_recv\n");
      ...
      MPI_Send(&out_data[0], mpiTiming.out_data_length, MPI_DOUBLE,
...
int nemo3d(const char *inputFile, const char *inFileName) {
  ...
  if (d->opt.ExecParam.Output.ShapeInfo)
    writeShapeInfo(d);
  if (d->opt.ExecParam.Output.AtomInfo)
    writeAtomInfo(d);
  ...
  if (d->opt.ExecParam.ElCalc.ElOut.OptMatrixEl) {
    calc_opt_mat_cmplx(d, d->opt.ExecParam.Abs_Ramp.Ephi,
