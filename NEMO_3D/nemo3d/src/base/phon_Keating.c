...
*****************************************************************************
$Header: /repo/nemo3d/src/base/phon_Keating.c,v 1.17 2007/08/10 15:03:33 baeh
Exp $
*****************************************************************************/
 ...
 * i=3*(l*d->N_MaxAtomCell+m)+xi, where xi counts the components [012] of the
 * displacement vector.
 */
  ...
  d->phon.DMmap[d->geo.N_Cell][d->geo.cellgeom.AtomsPerCellMax()]*/
{
  ...
  int size_of_the_matrix =
      (3 * (d->geo.N_Cell * d->geo.cellgeom.AtomsPerCellMax() - d->n_atom_tot) >
               0
  ...
  for (int cell = 0; cell < d->geo.N_Cell; cell++) {
    for (int atom = 0; atom < d->geo.cellgeom.AtomsPerCellMax(); atom++) {
      for (int proj = 0; proj < 3; proj++) {
        ...
        if (!d->geo.StrainHasPeriodicity()) { // Necessary for closed boundary
                                              // conditions only
          if (d->geo.AtomType[cell][atom]) {
            ...
            if (ind_i >= 3 * d->n_atom_tot) {
              ...
              if (n_free > 0) {
                ...
                if (WaitInd[n_wait][0]) {
                  ...
                  printf("n_wait=%d WaitInd[n_wait][0]=%d\n", n_wait,
                         WaitInd[n_wait][0]);
                  printf("WaitInd[%d][1](cell)=%d ", n_wait,
                         WaitInd[n_wait][1]);
                  printf("WaitInd[%d][2](atom)=%d ", n_wait,
                         WaitInd[n_wait][2]);
              ...
              if (n_free > 0) {
                for (int j = 0; j < n_rem; j++) {
                  if (FreeInd[j] > 0) {
                    if (FreeInd[j] <
                        ind_i) { // Free index in DM is smaller then ind_i
                      ind_f = FreeInd[j];
                      // Shift the stack of free indexies
                      for (int i = j; i < n_free; i++) {
                        FreeInd[i] = FreeInd[i + 1];
            ...
            if (ind_i < 3 * d->n_atom_tot) {
              ...
              if (n_wait > 0) {
                d->phon.DMmap[WaitInd[n_wait][1]][WaitInd[n_wait][2]]
...
// unit cell n_max=d->geo.Neighbors(m)
{
  ...
  for (int n = 0; n < d->geo.Neighbors(m); n++) {
    ...
    if (cindx < 0 && d->geo.StrainHasPeriodicity() &&
        (cindx + d->geo.N_Cell + 1) >= 0) {
    ...
    if (cindx >= 0) {
      ...
      if (AtomType_cindx_aindx) {
        ...
        sqrtMn[n] =
            sqrt(d->Amass[AtomType_cindx_aindx]); // atomic mass square root
        if (sqrtMn[n] == 0.0) {
        ...
        dssq[n] = nnv[n][0] * nnv[n][0] + nnv[n][1] * nnv[n][1] +
                  nnv[n][2] * nnv[n][2];
        if (dssq[n] == 0.0)
          printf("ERROR:  dssq[%d] vanishes!!!\n", n), fflush(stdout);
        d0sq[n] = nnv0[n][0] * nnv0[n][0] + nnv0[n][1] * nnv0[n][1] +
                  nnv0[n][2] * nnv0[n][2];
        if (d0sq[n] == 0.0)
          printf("ERROR:  d0sq[%d] vanishes!!!\n", n), fflush(stdout);
        ...
        if (d->opt.ExecParam.Strain.StrainModel ==
          ...
          if (((d->opt.ExecParam.PhonCalc.anh == PhonCalc_struct::Default) &&
              ...
              (d->opt.ExecParam.PhonCalc.anh ==
               PhonCalc_struct::AcousticalSet)) {
            anh = 1;
          } else if (((d->opt.ExecParam.PhonCalc.anh ==
                       PhonCalc_struct::Default) &&
                      (d->opt.ExecParam.PhonCalc.PS ==
                       PhonCalc_struct::Higher)) ||
                     (d->opt.ExecParam.PhonCalc.anh ==
                      PhonCalc_struct::OpticalSet)) {
            anh = 2;
          } else {
            if (!mpi_n3d_id) {
              printf("\nThe requested option for the optimization of the "
                     "anharmonicity corrections \n"
                     "has not been implemented yet!!!\n"
                     "Use anharmonicity corrections for better strain "
                     "distribution instead.\n");
            }
            anh = 1;
          }
          alpha[n] = d->strain.alpha_eff(AtomType_l_m, AtomType_cindx_aindx,
                                         dssq[n], d0sq[n], anh, true);
  ...
  Here 0<i<=d->phon.states_in_row */
{
  ...
  } else if (i > d->phon.Ind_DM[n_row][0]) { // New element inserted
  ...
  and call Add_element*/
{
  int YZblock = ((d->geo.cell_ymax - d->geo.cell_ymin + 1) *
                 (d->geo.cell_zmax - d->geo.cell_zmin + 1) * 3 *
                 d->geo.cellgeom.AtomsPerCellMax()); // # of DM rows/cols in 1
                                                     // u.c. slab in x-direction
  int DM_total_size = 3 * d->n_atom_tot; // Total # of DM rows/cols
  int n_col, n_row; // Indexies in the DM block stored on the processor

  // column
  if (col >= d->phon.seg_col_s[mpi_n3d_id] &&
      col <=
          d->phon.seg_col_s[mpi_n3d_id] + d->phon.seg_col_ln[mpi_n3d_id] - 1) {
      ...
      if (d->geo.StrainIsPeriodicX()) {
        if (mpi_n3d_id == mpi_n3d_numprocs - 1) {
          if (row >= YZblock) { // should not happen
            printf("row=%d >= %d=YZblock!!! => There must be a mistake on %d "
                   "processor!!!!",
                   row, YZblock, mpi_n3d_id);
            fflush(stdout);
          } else if (row < 0) { // should not happen
            printf(
                "row=%d < 0!!! => There must be a mistake on %d processor!!!!",
                row, mpi_n3d_id);
            fflush(stdout);
          } else { // add to the upper block on the last proc under periodic
                   // b.c.
            n_col = col - d->phon.seg_col_s[mpi_n3d_id];
            n_row = row + d->phon.max_row[mpi_n3d_id] -
            ...
            if (n_row > d->phon.N_rows[mpi_n3d_id]) { // should not happen
              printf("n_row=%d > %d=d->phon.N_rows[%d] => "
                     ...
                     n_row, d->phon.N_rows[mpi_n3d_id], mpi_n3d_id, mpi_n3d_id);
              fflush(stdout);
            }
            if (n_row <= d->phon.max_row[mpi_n3d_id] -
                             d->phon.min_row[mpi_n3d_id]) { // should not happen
              printf("n_row=%d <= "
                     "%d=(d->phon.max_row[%d]=%d)-(d->phon.min_row[%d]=%d) => "
                     "!!!!!!!!!There is OVERWRITE on %d processor!!!!!!!!",
                     n_row,
                     d->phon.max_row[mpi_n3d_id] - d->phon.min_row[mpi_n3d_id],
                     mpi_n3d_id, d->phon.max_row[mpi_n3d_id], mpi_n3d_id,
                     d->phon.min_row[mpi_n3d_id], mpi_n3d_id);
              fflush(stdout);
            }
...
real doublet(real beta_eff, // effective beta=sqrt(beta1*beta2), which may be
                            // scaled with strain real beta1,real beta2,//bond
                            // bending distorsion force constant
...
*/
{
  return ((r1[0] * r2[0] + r1[1] * r2[1] + r1[2] * r2[2] - d1[0] * d2[0] -
           d1[1] * d2[1] - d1[2] * d2[2]));
}
...
/*Add elements A1 to Dynamical Matrix*/
{
  ...
  Add_Herm(d, mDM, nDM, -A1 / (sqrtMn * sqrtMm) * phase_r,
...
/*Add elements A2 to Dynamical Matrix*/
{
  ...
  Add_Herm(d, mDM, nDM, -A2 / (sqrtMn * sqrtMm) * phase_r,
           -A2 / (sqrtMn * sqrtMm) * phase_i);
  Add_Herm(d, m1DM, n1DM, -A2 / (sqrtMn * sqrtMm) * phase_r,
           -A2 / (sqrtMn * sqrtMm) * phase_i);
}
  ...
  when projections of the displacements are the same*/
{
  ...
  Add(d, d->phon.DMmap[cell_n1][atom_n1][xi],
      d->phon.DMmap[cell_n1][atom_n1][xi], V * xn2 * xn2 / (sqrtMn1 * sqrtMn1),
      0.0);
  Add(d, d->phon.DMmap[cell_n2][atom_n2][xi],
      d->phon.DMmap[cell_n2][atom_n2][xi], V * xn1 * xn1 / (sqrtMn2 * sqrtMn2),
      0.0);
  Add_Herm(d, d->phon.DMmap[cell_n1][atom_n1][xi],
           d->phon.DMmap[cell_n2][atom_n2][xi],
           V * (D + xn1 * xn2) / (sqrtMn1 * sqrtMn2) *
               (phase_n1_r * phase_n2_r + phase_n1_i * phase_n2_i),
           V * (D + xn1 * xn2) / (sqrtMn1 * sqrtMn2) *
               (phase_n1_r * phase_n2_i - phase_n1_i * phase_n2_r));
  Add_Herm(d, d->phon.DMmap[cell_m][atom_m][xi],
           d->phon.DMmap[cell_n1][atom_n1][xi],
...
{
  Add_Herm(d, d->phon.DMmap[cell_m][atom_m][xi1],
           d->phon.DMmap[cell_m][atom_m][xi2],
           V * ((x1n1 + x1n2) * (x2n1 + x2n2)) / (sqrtMm * sqrtMm), 0.0);
  Add_Herm(d, d->phon.DMmap[cell_n1][atom_n1][xi1],
           d->phon.DMmap[cell_n1][atom_n1][xi2],
           V * x1n2 * x2n2 / (sqrtMn1 * sqrtMn1), 0.0);
  Add_Herm(d, d->phon.DMmap[cell_n2][atom_n2][xi1],
           d->phon.DMmap[cell_n2][atom_n2][xi2],
           V * x1n1 * x2n1 / (sqrtMn2 * sqrtMn2), 0.0);
  Add_Herm(d, d->phon.DMmap[cell_n1][atom_n1][xi1],
           d->phon.DMmap[cell_n2][atom_n2][xi2],
           V * x1n2 * x2n1 / (sqrtMn1 * sqrtMn2) *
               (phase_n1_r * phase_n2_r + phase_n1_i * phase_n2_i),
           V * x1n2 * x2n1 / (sqrtMn1 * sqrtMn2) *
               (phase_n1_r * phase_n2_i - phase_n1_i * phase_n2_r));
  Add_Herm(d, d->phon.DMmap[cell_n1][atom_n1][xi2],
           d->phon.DMmap[cell_n2][atom_n2][xi1],
           V * x1n1 * x2n2 / (sqrtMn1 * sqrtMn2) *
               (phase_n1_r * phase_n2_r + phase_n1_i * phase_n2_i),
           V * x1n1 * x2n2 / (sqrtMn1 * sqrtMn2) *
               (phase_n1_r * phase_n2_i - phase_n1_i * phase_n2_r));
  Add_Herm(d, d->phon.DMmap[cell_m][atom_m][xi1],
  ...
  Add_Herm(d, d->phon.DMmap[cell_m][atom_m][xi2],
           d->phon.DMmap[cell_n1][atom_n1][xi1],
...
#ifdef PR_ZEROS_SUBSTITUTION
          printf("\nDM[%d][%d].r=%g<%g => DM[%d][%d].r=0", row,
                 d->phon.Ind_DM[row][i], d->phon.DM[row][i - 1].r, zero, row,
                 d->phon.Ind_DM[row][i]);
...
#ifdef PR_ZEROS_SUBSTITUTION
          printf("\nDM[%d][%d].i=%g<%g => DM[%d][%d].r=0", row,
                 d->phon.Ind_DM[row][i], d->phon.DM[row][i - 1].i, zero, row,
                 d->phon.Ind_DM[row][i]);
#endif // PR_ZEROS_SUBSTITUTION
...
{
  cvectr
      phase; //=exp[i(q_ph.R)] -- phase factor for periodic boundary conditions
  ...
  real A1, A2, D;   /*dummies for terms to be added to different elements of
                       Dynamical Matrix*/
  ...
  rvectr dssq = Rvectr(
      maxNbr); // strained bonds squared between m-th atom and its neighbors
  rvectr d0sq = Rvectr(
      maxNbr); // relaxed bonds squared between m-th atom and its neighbors
  ...
  for (int l = cell0; l <= cellF; l++) {
    ...
    for (int m = 0; m < d->geo.AtomsPerCellMax(); m++) {
      ...
      initial_assignments(d, n_qph, l, m, phase, alpha, beta, sqrtMn, nnv, nnv0,
                          dssq, d0sq);
        ...
        for (int n = 0; n < d->geo.Neighbors(m); n++) {
          ...
          if (cindx >= 0) {
            if (d->geo.AtomType[cindx][aindx]) {
              ...
              /* Loop through  components of the displacement vector of n-th
               * neighbor of atom m*/
              for (int xin = xim; xin < 3; xin++) {
                if ((nDM = d->phon.DMmap[cindx][aindx][xin]) > mDM) {
                  ...
                  if (xim == xin) { // xx
                    ...
                    Add_A1(d, A1, mDM, nDM, sqrtMm, sqrtMn[n], phase[n].r,
                           phase[n].i);
#endif /*A1DM*/
                  } // xx
                  else { // xy
                    ...
                    Add_A2(d, A2, l, m, xim, cindx, aindx, xin, sqrtMm,
                           sqrtMn[n], phase[n].r, phase[n].i);
#endif /*A2DM*/
                  } // xy
                ...
                for (int nbr2 = n + 1; nbr2 < d->geo.Neighbors(m); nbr2++) {
                  ...
                  if (cnbr2 >= 0) { // nbr2 is within simulation domain
                    if (d->geo.AtomType[cnbr2][anbr2]) { // and nbr2 has not
                                                         // been removed
                      ////////////////////////////////
                      ...
                      if (d->opt.ExecParam.Strain.StrainModel ==
                        ...
                        /*  cout << "\nInput data for getting Beta:"
                             ...
                             << "\n Type of the "
                             << n <<"-th neighbor: "<< (int)
                           d->geo.AtomType[cindx][aindx]
                             << "\n Type of the "
                             << nbr2 << "-th neighbor: " << (int)
                           d->geo.AtomType[cnbr2][anbr2]
                             << "\n Realxed bond to "
                             << n <<"-th neighbor:  ("
                             << nnv0[n][0] << "," << nnv0[n][1] << ","
                           <<nnv0[n][2] <<")"
                           ...
                           <<")"
                             << "\n Strained bond to "
                             << nbr2 <<"-th neighbor:  ("
                             << nnv[nbr2][0] << "," << nnv[nbr2][1] << ","
                           <<nnv[nbr2][2] <<")"
                             << endl;*/
                        int anh;
                        if (((d->opt.ExecParam.PhonCalc.anh ==
                              PhonCalc_struct::Default) &&
                             (d->opt.ExecParam.PhonCalc.PS ==
                              PhonCalc_struct::Lower)) ||
                            (d->opt.ExecParam.PhonCalc.anh ==
                             PhonCalc_struct::AcousticalSet)) {
                          anh = 1;
                        } else if (((d->opt.ExecParam.PhonCalc.anh ==
                          ...
                          if (!mpi_n3d_id) {
                            printf(
                                "\nThe requested option for the optimization "
                                "of the anharmonicity corrections \n"
                                "has not been implemented yet!!!\n"
                                "Use anharmonicity corrections for better "
                                "strain distribution instead.\n");
                          }
                          anh = 1;
                        }
                        Beta = d->strain.beta_eff(
                            (int)d->geo.AtomType[l][m], // type of the m-th atom
                            (int)d->geo.AtomType[cindx][aindx], // type of the
                                                                // n1-th atom
                            (int)d->geo.AtomType[cnbr2][anbr2], // type of the
                                                                // n2-th atom
                            nnv0[n], nnv0[nbr2], // vectors of the relaxed bonds
                                                 // of the V doublet
                            nnv[n], nnv[nbr2], // vectors of the strained bonds
                                               // of the V doublet
                            anh, // variant of the anharmonicity corrections
                                 // optimization
                            true);
                      ...
                      if (xim == xin) { // x1=x2
                        ...
                        Add_BXX(d, Vcoeff, D, nnv[n][xim], nnv[nbr2][xim], l, m,
                                cindx, aindx, cnbr2, anbr2, xim, sqrtMm,
                                sqrtMn[n], sqrtMn[nbr2], phase[n].r, phase[n].i,
                                phase[nbr2].r, phase[nbr2].i);
#endif /*BXXDM*/
                      } // x1=x2
                      else { // x1<x2
#ifdef BX1X2DM
                        Add_BX1X2(d, Vcoeff, nnv[n][xim], nnv[nbr2][xim],
                                  nnv[n][xin], nnv[nbr2][xin], l, m, cindx,
                                  aindx, cnbr2, anbr2, xim, xin, sqrtMm,
                                  sqrtMn[n], sqrtMn[nbr2], phase[n].r,
                                  phase[n].i, phase[nbr2].r, phase[nbr2].i);
#endif /*BX1X2DM*/
                      } // x1<x2
              ...
              } // xin loop through components of the displacement vector of
                // n-th neighbor of m-th atom
  ...
  for (int row = 0; row < d->phon.N_rows[mpi_n3d_id]; row++) {
    comb_DM(d->phon.Ind_DM[row], d->phon.DM[row], 1,
            d->phon.Ind_DM[row][0] // lower and upper bounds of the arrays
    );
  } // sort elements of DM in ascending order of their indexes
...

{
  ...
  real q_ph_max,
      q_ph_min; // maximal and minimal values of the phonon wave vector

#ifdef TALK_A_LOT
  masterPrint("I am in phon_disp function now...\n");
#endif /* TALK_A_LOT*/

  d->phon.ph_s_alloc(d->geo.N_Cell, d->geo.cellgeom.AtomsPerCellMax(),
                     d->opt.ExecParam.PhonCalc.N_branches,
                     d->opt.ExecParam.PhonCalc.Nq_ph, d->nproc);
  /* If we are on edge, processor to past edge is NULL (-1) unless we
     have periodicity in x.*/
  d->phon.mpi_left[mpi_n3d_id] =
  ...
  int YZslab = ((d->geo.cell_ymax - d->geo.cell_ymin + 1) *
                (d->geo.cell_zmax - d->geo.cell_zmin +
                 1)); // # of unit cells in 1 u.c. slab in x-direction
  int rem_slabs = (d->geo.cell_xmax - d->geo.cell_xmin + 1) %
                  mpi_n3d_numprocs; /*# of processors that have
                                  1 extra XYslab.
                                  Those slabs are added to the
                                  last processors.*/
  int cell_ln =
      YZslab * ((d->geo.cell_xmax - d->geo.cell_xmin + 1) / mpi_n3d_numprocs +
                ((mpi_n3d_id < (mpi_n3d_numprocs - rem_slabs)) ? 0 : 1));
  int cell_s =
      (mpi_n3d_id * YZslab *
           ((d->geo.cell_xmax - d->geo.cell_xmin + 1) / mpi_n3d_numprocs) +
       ((mpi_n3d_numprocs - mpi_n3d_id < rem_slabs)
            ? YZslab * (rem_slabs - (mpi_n3d_numprocs - mpi_n3d_id))
            : 0));
  int cell0 = cell_s;
  int cellF = cell0 + cell_ln - 1;
  d->phon.seg_col_s[mpi_n3d_id] =
      3 * cell_s * d->geo.cellgeom.AtomsPerCellMax();
  d->phon.seg_col_ln[mpi_n3d_id] =
      3 * cell_ln * d->geo.cellgeom.AtomsPerCellMax();
  if (mpi_n3d_id)
    cell0 -= YZslab;
  if (mpi_n3d_id != mpi_n3d_numprocs - 1)
    cellF += YZslab;
  d->phon.szCommL[mpi_n3d_id] =
      ((d->phon.mpi_left[mpi_n3d_id] >= 0) &&
               (d->phon.mpi_left[mpi_n3d_id] != mpi_n3d_id)
           ? 3 * YZslab * d->geo.cellgeom.AtomsPerCellMax()
           : 0); // Number of rows
                 // in the buffer
                 // to be sent to the
                 // left processor
  d->phon.szCommR[mpi_n3d_id] =
      ((d->phon.mpi_right[mpi_n3d_id] >= 0) &&
               (d->phon.mpi_right[mpi_n3d_id] != mpi_n3d_id)
           ? 3 * YZslab * d->geo.cellgeom.AtomsPerCellMax()
           : 0); // Number of rows
                 // in the buffer
                 // to be sent to the
  ...
  d->phon.N_cols[mpi_n3d_id] =
      3 * cell_ln *
      d->geo.cellgeom.AtomsPerCellMax(); // Number of columns of DM
                                         // stored on the processor
  d->phon.N_rows[mpi_n3d_id] =
      3 * d->geo.cellgeom.AtomsPerCellMax() *
      (cellF - cell0 + 1 +
       ((d->geo.StrainIsPeriodicX() && (mpi_n3d_numprocs != 1) &&
         ((!mpi_n3d_id) || (mpi_n3d_id == mpi_n3d_numprocs - 1)))
            ? ((d->geo.cell_ymax - d->geo.cell_ymin + 1) *
               (d->geo.cell_zmax - d->geo.cell_zmin + 1))
            : 0)); // Number of rows of DM
                   // stored on the processor
  ...
  if (mpi_n3d_numprocs == 2 && (d->phon.N_cols[mpi_n3d_numprocs - 1] +
    ...
    if (d->phon.N_cols[0] + d->phon.szCommL[0] + d->phon.szCommR[0] <=
        d->phon.N_rows[0]) {
  ...
  if (d->phon.N_cols[mpi_n3d_id] + d->phon.szCommL[mpi_n3d_id] +
          d->phon.szCommR[mpi_n3d_id] >
      d->phon.N_rows[mpi_n3d_id]) {
    // Should not happen
    printf("\n!!!!![(d->phon.N_cols[%d]=%d)+(d->phon.szCommL[%d]=%d)+"
           "(d->phon.szCommR[%d]=%d)]=%d > %d=d->phon.N_rows[%d]!!!!!\n",
           mpi_n3d_id, d->phon.N_cols[mpi_n3d_id], mpi_n3d_id,
           d->phon.szCommL[mpi_n3d_id], mpi_n3d_id, d->phon.szCommR[mpi_n3d_id],
           d->phon.N_cols[mpi_n3d_id] + d->phon.szCommL[mpi_n3d_id] +
               d->phon.szCommR[mpi_n3d_id],
  ...
  d->phon.DM_allocate(d->phon.N_rows[mpi_n3d_id], // Number of rows
                      (d->phon.N_cols[mpi_n3d_id] < d->phon.states_in_row
                           ? d->phon.N_cols[mpi_n3d_id]
                           : d->phon.states_in_row) // Max number of elements
                                                    // per row on
                                                    // mpi_n3d_id proc
  );

#ifdef TALK_A_LOT
  masterPrint("%d points of %d branches of phonon dispersion to be computed "
              "within Keating model",
  ...
  d->phon.max_row[mpi_n3d_id] =
      d->phon.DMmap[cellF][d->geo.cellgeom.AtomsPerCellMax() - 1][2];

#ifdef PR_PHON_PARAMETERS
  d->phon.Print_parameters();
#endif // PR_PHON_PARAMETERS
  real alat =
      d->strain.amat[(int)d->geo.AtomType[0][0]][(int)d->geo.AtomType[0][4]];
  // Measure of hydrostatic strain
  real V_over_V0 =
      ((d->geo.lattice_x * ((d->geo.cell_xmax - d->geo.cell_xmin) + 1) *
        d->geo.lattice_y * ((d->geo.cell_ymax - d->geo.cell_ymin) + 1) *
        d->geo.lattice_z * ((d->geo.cell_zmax - d->geo.cell_zmin) + 1)) /
       (alat * ((d->geo.cell_xmax - d->geo.cell_xmin) + 1) * alat *
        ((d->geo.cell_ymax - d->geo.cell_ymin) + 1) * alat *
        ((d->geo.cell_zmax - d->geo.cell_zmin) + 1)));
...
#define BZ_BOUNDARY
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (d->opt.ExecParam.PhonCalc.Nq_ph != 1) {
      dq = (q_ph_max - q_ph_min) / (d->opt.ExecParam.PhonCalc.Nq_ph - 1);
    } else
      dq = 0.0;
    if (dq < 1e-10)
      dq = 0;
  ...
  for (int n_qph = 0; n_qph < d->opt.ExecParam.PhonCalc.Nq_ph; n_qph++) {
    masterPrint(">>>>>>>>>>> %d : q_ph=( %g, %g, %g )<<<<<<<<<<<<", n_qph,
                d->phon.E_ph[n_qph][0], d->phon.E_ph[n_qph][1],
                d->phon.E_ph[n_qph][2]);
    // Clean the Dynamical matrix before rebuilding
    d->phon.DM_reallocate(d->phon.N_rows[mpi_n3d_id],
                          (d->phon.N_cols[mpi_n3d_id] < d->phon.states_in_row
                               ? d->phon.N_cols[mpi_n3d_id]
                               : d->phon.states_in_row) // Max number of
                                                        // elements per row on
                                                        // mpi_n3d_id proc
    );
    ...
    if ((!mpi_n3d_id) && d->geo.StrainIsPeriodicX() &&
        (cellF < d->geo.N_Cell - 1)) {
      DM_Keating(d, n_qph, d->geo.N_Cell - YZslab, d->geo.N_Cell - 1);
    }
    // Upper block on the last processor at periodic X b.c.
    if ((mpi_n3d_id == mpi_n3d_numprocs - 1) && d->geo.StrainIsPeriodicX() &&
        (cell0 != 0)) {
  ...
  if (!mpi_n3d_id) {
    ...
    for (int n = 0; n < d->opt.ExecParam.PhonCalc.Nq_ph; n++) {
      ...
      printf("\n%d  %g  %g  %g  %g   %g    ", n, V_over_V0, d->phon.E_ph[n][0],
             d->phon.E_ph[n][1], d->phon.E_ph[n][2], q);
