     ...
     //Computes w<==DM*v
{
  ...
  for(n=row_init,n0=0; n<=row_final; n++,n0++){
    ...
    for(j = 1 ; j<=jmax; j++){
      w[n0].r +=
        (d->phon.DM[n][j-1].r)*(v[d->phon.Ind_DM[n][j]].r)-
        (d->phon.DM[n][j-1].i)*(v[d->phon.Ind_DM[n][j]].i);
      w[n0].i +=
        (d->phon.DM[n][j-1].r)*(v[d->phon.Ind_DM[n][j]].i)+
     ...
     //Computes the final value of w
{
  ...
  //Residential part 
  av(d,
     d->phon.seg_col_s[mpi_n3d_id]-d->phon.min_row[mpi_n3d_id],
  ...
  if(d->phon.szCommL[mpi_n3d_id]){//Block to be sent to the left processor
    if(mpi_n3d_numprocs!=1 && !mpi_n3d_id && d->geo.StrainIsPeriodicX()){
      av(d,
         d->phon.N_rows[mpi_n3d_id]-d->phon.szCommL[mpi_n3d_id],
         d->phon.N_rows[mpi_n3d_id]-1,
         v,d->phon.wL);
    }
    else{
      av(d,
  ...
  if(d->phon.szCommR[mpi_n3d_id]){//Block to be sent to the right processor
    if(mpi_n3d_numprocs!=1 && mpi_n3d_id==mpi_n3d_numprocs-1 && d->geo.StrainIsPeriodicX()){
      av(d,
         d->phon.N_rows[mpi_n3d_id]-d->phon.szCommR[mpi_n3d_id],
         d->phon.N_rows[mpi_n3d_id]-1,
         v,d->phon.wR);
    }
    else{
      av(d,
         d->phon.seg_col_s[mpi_n3d_id]+d->phon.seg_col_ln[mpi_n3d_id]
         -d->phon.min_row[mpi_n3d_id],
         d->phon.seg_col_s[mpi_n3d_id]+d->phon.seg_col_ln[mpi_n3d_id]+d->phon.szCommR[mpi_n3d_id]
         -d->phon.min_row[mpi_n3d_id]-1,
         v,d->phon.wR);
    }
    ...
    if (mpi_n3d_id % 2) {
      if (d->phon.mpi_left[mpi_n3d_id] >= 0) {
        MPI_Sendrecv(d->phon.wL, 2*d->phon.szCommL[mpi_n3d_id], MPI_DOUBLE,
                     d->phon.mpi_left[mpi_n3d_id], 1,
                     d->phon.vL, 2*d->phon.szCommR[d->phon.mpi_left[mpi_n3d_id]] , MPI_DOUBLE,
      ...
      if (d->phon.mpi_right[mpi_n3d_id] >= 0) {
        MPI_Sendrecv(d->phon.wR, 2*d->phon.szCommR[mpi_n3d_id], MPI_DOUBLE,
                     ...
                     d->phon.vR, 2*d->phon.szCommL[d->phon.mpi_right[mpi_n3d_id]], MPI_DOUBLE,
                     d->phon.mpi_right[mpi_n3d_id], 1, 
                     MPI_COMM_WORLD, &mpi_n3d_status);
      }
    }
    else {
  ...
  /////////////////////////////////////////////////////////
  if(d->phon.szCommR[d->phon.mpi_left[mpi_n3d_id]]){//Addition from the left processor vL
    for(i=0;i<d->phon.szCommR[d->phon.mpi_left[mpi_n3d_id]];i++){
      w[i].r+=d->phon.vL[i].r;
      w[i].i+=d->phon.vL[i].i;
    }
  }
  if(d->phon.szCommL[d->phon.mpi_right[mpi_n3d_id]]){//Addition from the right processor vR
    for(i=0;i<d->phon.szCommL[d->phon.mpi_right[mpi_n3d_id]];i++){
      w[d->phon.seg_col_ln[mpi_n3d_id]-d->phon.szCommL[d->phon.mpi_right[mpi_n3d_id]]+i].r+=d->phon.vR[i].r;
      w[d->phon.seg_col_ln[mpi_n3d_id]-d->phon.szCommL[d->phon.mpi_right[mpi_n3d_id]]+i].i+=d->phon.vR[i].i;
    }
  }
}//end of void av_mpi
...
//static void qsortEph(cvectr Eph,//[nconv==iparam[4]] vector of converged eigenvalues
