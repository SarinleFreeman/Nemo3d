...
int unit_cell(Dev_struct::CrystalStructType structure, int site, int anion,
              int cation, int cation1, real ratio1, int cation2, real ratio2) {
  int element;
...
void strnmat_init(qd_struct d, const vector<double> &mb_strain, int cation,
                  int anion) {
  ...
  if (!d->strnmat[anion][cation]) {
    cout_master << "   Copying strain parameters for (" << atomID_to_str(anion)
                << atomID_to_str(cation) << ") from (" << atomID_to_str(cation)
                << atomID_to_str(anion) << ")\n";
...
void param_init(qd_struct d) {
  ...
  for (vector<MaterialHandle>::iterator sm = d->sMatList.begin();
    ...
    if (d->BandModel == BM_20_sp3d5ss_spin ||
      ...
      param_sp3s_2_sp3d5s(d->parmat[cation][anion], d->parmat[cation][anion],
                          smh->unstrnd_cubic_cell_length, 1.0, 1.0, 1.0);
      ...
      */
      ...
      case Dev_struct::Cubic:
        ...
        d->parmat[anion][anion][0] = d->parmat[cation][cation][0] =
            d->parmat[cation][anion][0] = d->parmat[cation][anion][2] =
                3.0 * HBAR_MASSFACTOR / (smh->mb_pv[4] * smh->mb_pv[4]) * 2.0 /
                    smh->mb_pv[2] +
                smh->mb_pv[0] + smh->Ev_offset;
        d->parmat[cation][anion][10] = d->parmat[anion][anion][10] =
      ...
      case Dev_struct::Zincblende:
        ...
        d->parmat[anion][anion][0] = d->parmat[cation][cation][0] =
            d->parmat[cation][anion][0] = d->parmat[cation][anion][2] =
                2.0 * HBAR_MASSFACTOR / (smh->mb_pv[4] * smh->mb_pv[4]) * 2.0 /
                    (2.0 * smh->mb_pv[2]) +
                smh->mb_pv[0] + smh->Ev_offset;
        d->parmat[cation][anion][10] = d->parmat[anion][anion][10] =
            d->parmat[cation][cation][10] =
      ...
      default:
        d->parmat[anion][anion][0] = d->parmat[cation][cation][0] =
            d->parmat[cation][anion][0] = d->parmat[cation][anion][2] =
                HBAR_MASSFACTOR / (smh->mb_pv[4] * smh->mb_pv[4]) * 2.0 /
                    smh->mb_pv[2] +
                smh->mb_pv[0] + smh->Ev_offset;
        d->parmat[cation][anion][10] = d->parmat[anion][anion][10] =
            d->parmat[cation][cation][10] =
