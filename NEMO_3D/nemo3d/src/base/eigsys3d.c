...
int sym_lanc_it_c(qd_struct d, real shift, real *alpha, real *beta,
                  real beta_m1, cvectr r, cvectr q, cvectr q_m1, cvectr q_m2) {
  int i;
  complex alc;
  real bl, al;
...
int readstates_driver_c(qd_struct d) {
  ...
  for (k = 0; k < n_converged; k++) {
    sprintf(filename, "%s_%d",
            d->opt.ExecParam.ElCalc.ElAlg.ReadInEigen.StateFileRead.c_str(),
            k + 1);
    if (mpi_n3d_id == mpi_n3d_masterid)
      printf("Read file %s number %d/%d\n", filename, k + 1, n_converged);
    ...
    if (d->opt.ExecParam.ElCalc.ElOut.Eigval) {
      if (mpi_n3d_id == mpi_n3d_masterid)
        printf("Compute Rayleigh Quotient\n");
...

      {
        ...
        sym_lanc_it_c(d, 0.0, &alpha, &beta, beta_m1, d->wfc[k], q, q_m1, NULL);
...
energy range [emin:emax].  max_iter is the maximum number of iterations.
Algorithms stops if 100 iterations occur with no new converged eigenvalues */
int ns_lanc_driver(qd_struct d) { return (0); }

int sym_lanc_it(qd_struct d, real shift, real *alpha, real *beta, real beta_m1,
                rvectr r, rvectr q, rvectr q_m1, rvectr q_m2) {
  int i, n;
  real bl, al;
...
int nonsym_lanc_it(qd_struct d, real shift, complex *alpha, complex *beta,
                   complex *Gamma, cvectr r, cvectr s, cvectr q, cvectr q_m1,
                   cvectr p, cvectr p_m1) {
  int i;
  complex bl, al, gl;
...
int bulk_bandstructure(qd_struct d) {
  ...
  for (i = 0; i < d->geo.NeighborsMax(); i++)
    for (j = 0; j < 3; j++)
      nnv[i][j] = d->a * (d->geo.PositionInCell(d->geo.NbrCell(0, i, 3), j) -
