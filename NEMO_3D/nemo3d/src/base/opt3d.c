...
#define GAMMA_RANGE       5.0       /* 50 */
  ...
  printf("\n\n****** make_egrid_lineshape   optical absorptiongrid n_total=%d\n",n_total);
   */
...
*/
       ...
       for ( i=0; i < n-1; i++ ){
	 for ( j=i+1; j < n; j++ ){
             d->opmat_pxyz[i][j][3] = px*px*d->opmat_pxyz[i][j][0]*d->opmat_pxyz[i][j][0]
...
*/
    ...
    fprintf(f_tran,"# index energy initial final strength_x strength_y strength_z strength_angle\n");
    fprintf(f_tran_px,"# symolic plot of light and dark spectral lines - light polarixed in x\n");
    fprintf(f_tran_py,"# symolic plot of light and dark spectral lines - light polarixed in y\n");
    fprintf(f_tran_pz,"# symolic plot of light and dark spectral lines - light polarixed in z\n");
    fprintf(f_tran_pa,"# symolic plot of light and dark spectral lines - light polarixed at angle\n");
    /* Print out transition energies with corresponding states.  Also
       print file containing delta functions at each transition */
    ...
    #define LOWVAL 1e-30
        ...
        fprintf( f_tran_pa,"%e %e\n", d->te[i]-1e-12, LOWVAL);
            ...
            fprintf( f_tran_px,"%e %e\n", d->te[i], d->opmat_pxyz[d->tindx[i][0]][d->tindx[i][1]][0]); 
            fprintf( f_tran_py,"%e %e\n", d->te[i], d->opmat_pxyz[d->tindx[i][0]][d->tindx[i][1]][1]); 
            fprintf( f_tran_pz,"%e %e\n", d->te[i], d->opmat_pxyz[d->tindx[i][0]][d->tindx[i][1]][2]); 
            fprintf( f_tran_pa,"%e %e\n", d->te[i], d->opmat_pxyz[d->tindx[i][0]][d->tindx[i][1]][3]); 
	/*
...
int calc_rate_cmplx( rvectr optmatvec, qd_struct d, cvectr init, cvectr finl, real px, real py, real pz)
{
    int i, j, k, l, m, n;
    real Tf, mxm, mxp, sumx, sumy, sumz, sumi, sumf;
    ...
    optmatvec[3] = px*px*sumx*sumx + py*py*sumy*sumy + pz*pz*sumz*sumz;
