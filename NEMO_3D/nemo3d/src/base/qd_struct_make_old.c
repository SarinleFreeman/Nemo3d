...
*****************************************************************************
$Header: /repo/nemo3d/src/base/qd_struct_make_old.c,v 1.1 2006/11/23 19:48:55
hoonryu Exp $
*****************************************************************************/
...
void QD_struct::setTopStruct(const char *f_xml, const char *f_in) {
  ...
  if (this->opt.Version != VERSION_ID)
    die("!!! INVALID INPUT !!!\n"
        "The version of the input file you are using (%s) is invalid and "
        "probably outdated."
        "You should update your input to version %s.  One can update the "
        "grammar to the latest "
        "version by using the script "
        "$HOME_NEMO3D/nemo3d/misc/update-nemo3d-input.pl.  "
        "Please see the commented lines at the top for usage instructions.  "
        "After applying the "
        "script one should carefully examine the newly generated input to make "
        "sure there are "
