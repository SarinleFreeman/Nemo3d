...
*****************************************************************************
$Header: /repo/nemo3d/src/base/eigsys3d_par.c,v 1.78 2008/09/24 02:09:07 hoonryu Exp $ 
*****************************************************************************/
   ...
   case Lanczos_struct::CyclicRandom:
      {
         ...
         for ( j=0; j < d->seg_ln[mpi_n3d_id]; j++ ){
            ...
            r0[j].r = d->opt.ExecParam.ElCalc.ElAlg.Lanczos.CyclicString[(2*indx)%sz];
            r0[j].i = d->opt.ExecParam.ElCalc.ElAlg.Lanczos.CyclicString[(2*indx+1)%sz];
            sum_local += r0[j].r*r0[j].r + r0[j].i*r0[j].i;
         }
      }
      break;
   ...
   case Lanczos_struct::ReadFile:
      ...
      } 
      {
         ...
         if (mpi_n3d_id == mpi_n3d_masterid) {
            sprintf(filename,"%s",d->opt.ExecParam.ElCalc.ElAlg.Lanczos.StartFileRead.c_str());
            initStartVect(filename, d, r0_full);
            for  (proc=1; proc<mpi_n3d_numprocs; proc++) {
               /* might use non-blocking send? */
               MPI_Send(&r0_full[d->seg_s[proc]], 2*d->seg_ln[proc], 
...
int lanczos_driver(qd_struct d) 
{
    ...
    int ConvCheckStartIter = d->opt.ExecParam.ElCalc.ElAlg.Lanczos.ConvCheckStartIter;
    int ConvCheckSkipRate  = d->opt.ExecParam.ElCalc.ElAlg.Lanczos.ConvCheckSkipRate;
    ...
    double deflation_tolerance	=	d->opt.ExecParam.ElCalc.ElAlg.BLanczos.DefTolerance;//1e-7;
    ...
    int max_no_success		=	d->opt.ExecParam.ElCalc.ElAlg.BLanczos.Max_no_success;//2000;
    int save_BT_band		=	d->opt.ExecParam.ElCalc.ElAlg.BLanczos.Dump_blockTridiagonal_Sparse_Matrix_Format;//0;
    int save_BT_SMF		=	d->opt.ExecParam.ElCalc.ElAlg.BLanczos.Dump_blockTridiagonal_Band_Matrix_Format;//0;
    int read_BT_band		=	d->opt.ExecParam.ElCalc.ElAlg.BLanczos.Read_blockTridiagonal_Band_Matrix_Format;//0;
    ...
    int conv_vec_method		=	d->opt.ExecParam.ElCalc.ElAlg.BLanczos.conv_vec_method;//2;
    ...
    int do_lancz_eigvect = (
        ...
        ((d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::Lancz)||(d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::BlockLanczos));
	...
	else if(d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::BlockLanczos)
	{
	    do_lancz_eigvect = 1;
    ...
    nml_memory_report("beginning of lanczos_driver \n************\n**************** from here on all memory should be distributed\n");
    ...
    if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::Lancz){
        d->neigv = eigensolve_lanczos(&ev,
                                 ...
                                 &matvect_mult,
                                 d->opt.ExecParam.ElCalc.ElAlg.Lanczos.ReadInTridiag
                                 );
    }
    else if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::BlockLanczos){
        d->neigv = eigensolve_blk_lanczos(&ev,
                                  ConvCheckStartIter,
                                  ConvCheckSkipRate,
                                  max_iter,
                                  req_vb,
	...
	cout<<endl<<"[OPTICAL MATRIX] "<<"Computing Momentum Matrix Px"<<endl;
	...
	for(int fstate = 0; fstate < d->neigv; fstate++) {
		...
		for (int aIndx = 0; aIndx < aNumber; aIndx++) {
                        for(int oIndx = 0; oIndx < 10; oIndx++) {
                                psiDegenFine[aIndx*d->NBasisStates + oIndx+10].r = d->wfc[fstate][aIndx*d->NBasisStates + oIndx].r;
                                psiDegenFine[aIndx*d->NBasisStates + oIndx+10].i = (-1)*d->wfc[fstate][aIndx*d->NBasisStates + oIndx].i;
                                psiDegenFine[aIndx*d->NBasisStates + oIndx].r = (-1)*d->wfc[fstate][aIndx*d->NBasisStates + oIndx+10].r;
                                psiDegenFine[aIndx*d->NBasisStates + oIndx].i = d->wfc[fstate][aIndx*d->NBasisStates + oIndx+10].i;
                        }
                }                       
		...
		for(int cellIndx = isx; cellIndx < iex; cellIndx++) {
                        for(int atomIndx = 0; atomIndx < d->geo.AtomsPerCellMax(); atomIndx++) {
		...
		for (int istate = 0; istate < d->neigv; istate++) {
			...
			temp1.r = 0.0; temp1.i = 0.0;
			temp2.r = 0.0; temp2.i = 0.0;

			// writing Kramer's Degenerate State for Initial State.
			for (int aIndx = 0; aIndx < aNumber; aIndx++) {
                        	for(int oIndx = 0; oIndx < 10; oIndx++) {
                                	psiDegenInit[aIndx*d->NBasisStates + oIndx+10].r = d->wfc[istate][aIndx*d->NBasisStates + oIndx].r;
                                	psiDegenInit[aIndx*d->NBasisStates + oIndx+10].i = (-1)*d->wfc[istate][aIndx*d->NBasisStates + oIndx].i;
                                	psiDegenInit[aIndx*d->NBasisStates + oIndx].r = (-1)*d->wfc[istate][aIndx*d->NBasisStates + oIndx+10].r;
                               	 	psiDegenInit[aIndx*d->NBasisStates + oIndx].i = d->wfc[istate][aIndx*d->NBasisStates + oIndx+10].i;
                        	}
                	}
			...
			pXmatrix[istate][8*fstate].r = (double)(istate+1);
                        ...
                        pXmatrix[istate][8*fstate+1].i = Momentum_Constant*(temp2.r - temp1.r);
                        pXmatrix[istate][8*fstate+1].r = Momentum_Constant*(temp1.i - temp2.i);
			...
			pXmatrix[istate][8*fstate+2].r = (double)(istate+1);
                        ...
                        pXmatrix[istate][8*fstate+3].i = Momentum_Constant*(temp2.r - temp1.r);
                        pXmatrix[istate][8*fstate+3].r = Momentum_Constant*(temp1.i - temp2.i);
			...
			pXmatrix[istate][8*fstate+4].r = (-1.0)*(double)(istate+1);
                        ...
                        pXmatrix[istate][8*fstate+5].i = Momentum_Constant*(temp2.r - temp1.r);
                        pXmatrix[istate][8*fstate+5].r = Momentum_Constant*(temp1.i - temp2.i);
			...
			temp1.r = 0.0; temp1.i = 0.0;
                        ...
                        pXmatrix[istate][8*fstate+7].i = Momentum_Constant*(temp2.r - temp1.r);
                        pXmatrix[istate][8*fstate+7].r = Momentum_Constant*(temp1.i - temp2.i);
			...
			if(istate > fstate) {
			        ...
			        OVERLAP[OVLAPIndx].r = (double)istate + 1.0; 		OVERLAP[OVLAPIndx++].i = (double)fstate + 1.0;
			...
			//	OVERLAP[OVLAPIndx++] = vect_dot_vect(&d->wfc[istate][0], xpsi, d);
				...
				TransProbX[PointIndx] = pXmatrix[istate][8*fstate+1].r*pXmatrix[istate][8*fstate+1].r + pXmatrix[istate][8*fstate+1].i*pXmatrix[istate][8*fstate+1].i;
		        	TransProbX[PointIndx] += pXmatrix[istate][8*fstate+3].r*pXmatrix[istate][8*fstate+3].r + pXmatrix[istate][8*fstate+3].i*pXmatrix[istate][8*fstate+3].i;
                        	TransProbX[PointIndx] += pXmatrix[istate][8*fstate+5].r*pXmatrix[istate][8*fstate+5].r + pXmatrix[istate][8*fstate+5].i*pXmatrix[istate][8*fstate+5].i;
                        	TransProbX[PointIndx++] += pXmatrix[istate][8*fstate+7].r*pXmatrix[istate][8*fstate+7].r + pXmatrix[istate][8*fstate+7].i*pXmatrix[istate][8*fstate+7].i;    
        ...
        cout<<endl<<"[OPTICAL MATRIX] "<<"Finished : Computing Momentum Matrix Px"<<endl;
	...
	#ifdef DUMP_TRANSP
	cout<<endl<<"[OPTICAL MATRIX] "<<"Writing Transition Probability X"<<endl;
	writeTransitionRate(d, &TransProbX[0], 0, "TransX");
	
	#ifdef DUMP_OVLAP
	cout<<endl<<"[OPTICAL MATRIX] "<<"Writing Overlap Matrix for X"<<endl;
	...
	cout<<endl<<"[OPTICAL MATRIX] "<<"Computing Momentum Matrix Py"<<endl;
	...
	for(int fstate = 0; fstate < d->neigv; fstate++) {
        ...
        cout<<endl<<"[OPTICAL MATRIX] "<<"Finished : Computing Momentum Matrix Py"<<endl;
	...
	cout<<endl<<"[OPTICAL MATRIX] "<<"Writing Transition Probability Y"<<endl;
	#endif // DUMP_TRANSP
	...
	cout<<endl<<"[OPTICAL MATRIX] "<<"Computing Momentum Matrix Pz"<<endl;
	...
	for(int fstate = 0; fstate < d->neigv; fstate++) {
        ...
        cout<<endl<<"[OPTICAL MATRIX] "<<"Finished : Computing Momentum Matrix Pz"<<endl;
	...
	cout<<endl<<"[OPTICAL MATRIX] "<<"Writing Transition Probability Z"<<endl;
        #endif // DUMP_TRANSP
	...
	cout<<endl<<"[OPTICAL MATRIX] "<<"Dynamic Memory Freed"<<endl;
		...
		if(d->opt.ExecParam.ElCalc.ElOut.Visualization_3D) {
			...
			//}
            	...
            	if(d->opt.ExecParam.Output.AtomPosEquil)  writePos(d,"equil");
		...
		if(mask[i] == 0) {
			...
			int vector_err;
				...
				if(mask[j] == 0) {
					...
					// It should be orthonormal to each other, otherwise just discard it!
					// TBD TBD TBD TBD
			        	if(fabs(ref_ev - ev[j]) < conv_tol) {
                            		
						complex inner_prod_ij = vect_dot_vect(&d->wfc[i][0], &d->wfc[j][0], d);
...
//		double ev_tmp[num_eig];
		...
		printf("[BLKLANCZ] Valid number of Eigenvalues after residual filtering: %d\n", num_eig);
		...
		if(d->neigv > 0) {
			...
			if(d->neigv > 0) {
				...
				//write_cubic_struct_wavefunctions(  d );
                		...
                		for (int i=0; i<d->neigv; i++)  writeEigenvector(d, d->wfc[i], i);
...
void generate_wave_vectors( qd_struct d, rmatrix *kr, int* Nk ) 
{
   ...
   *kr = Rmatrix(*Nk, 3);
   ...
   for (int i=1; i < *Nk; i++ ) { 
      (*kr)[i][0] = kxL_min + 
         (real) i*(kxL_max-kxL_min)/(real)((*Nk)-1);
      (*kr)[i][1] = kyL_min + 
         (real) i*(kyL_max-kyL_min)/(real)((*Nk)-1);
      (*kr)[i][2] = kzL_min + 
         (real) i*(kzL_max-kzL_min)/(real)((*Nk)-1);
   }
...
void compute_electronic_structure( qd_struct d )
{
   ...
   } else if (d->opt.ExecParam.ElCalc.ElAlg.HamSize==ElAlg_struct::VCA_1_cell) {
      if (d->opt.ExecParam.ElCalc.ElOut.LocalBands){
         d->_primitiveCell_granularity = 1; 
         d->_bandstruct_on_full_domain = 1;
         d->_bandstruct_on_local_domain = 1; 
   ...
   d->_bandstruct_on_local_domain = 
      d->_bandstruct_on_local_domain &
      d->_primitiveCell_granularity;
   
   /* perform conistency checks */
   if (d->opt.ExecParam.ElCalc.ElAlg.ResFind==ElAlg_struct::Direct && mpi_n3d_numprocs!=1)
   ...
   if(n_nemo[0] == 0 || !do_lancz_eigvect) 
     {
      printf("Unfolding not needed because supercell has same size as the small cell. \n");
...
printf("\nNEERAVS TEST 2\n");
     ...
     if(d->geo.isCellZB_110_small()) primitive_p_cell = 2;
     else if(d->geo.isCellZB_110()) primitive_p_cell = 8;
     else if(d->geo.isCellZB_111()) primitive_p_cell = 6;
     else primitive_p_cell = 4;
     ...
     /* n_nanostruct[i]'s are the number of unit cells in direction i, these are defined 
        taking into account the periodicity of the lattice */
     define_small_cell_of_nanostructure(n_nemo,n_nanostruct,&NBASIS,&periodic_y,&periodic_z,primitive_p_cell);
     ...
     /* Test print */
     printf("natoms = %3d  nrec = %3d NBASIS = %3d Nc_nano = %3d\n",natoms,nrec,NBASIS,Nc_nano);
     printf("n_nanostruct = (%d %d %d)\n",n_nanostruct[0],n_nanostruct[1],n_nanostruct[2]);
     ...
     Ur = R2tensor(0,Nc_nano,0,Nc_nano);//allocate_real_matrix(0,Nc-1,0,Nc-1);
     Ui = R2tensor(0,Nc_nano,0,Nc_nano);//allocate_real_matrix(0,Nc-1,0,Nc-1);
...
#define TETRAHEDRAL_LOCAL  // do 5 atom calculation
   ...
   else if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::Readstates) {
      readstates_driver_c( d ); 
   }
   else if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::PCTraceminSolver) {
      ///////////////////////////////////////////////
   ...
   else if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::PQTraceminSolver) {
    // Tracemin Driver for Quadratic Mapping (using conjugate gradient)
      ...
      ///////////////////////////////////////////////
   else if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::ParpackSolver) {
#ifdef NO_PARPACK
   ...
   else if (d->opt.ExecParam.ElCalc.ElAlg.ResFind == ElAlg_struct::LanczAndRayleigh) {
      d->kxL = kr[0][0];
      d->kyL = kr[0][1];
      d->kzL = kr[0][2];
      ...
      if (N==0) {
         n_converged = ( d->opt.ExecParam.ElCalc.ElAlg.ResFind==ElAlg_struct::Direct
           ...
           for(int j=0;j<n_converged;j++,eix++)
              {
               ...
               MPI_Gatherv( sbuf, 2*d->seg_ln[mpi_n3d_id], MPI_DOUBLE, rbuf, rcounts, displs, MPI_DOUBLE,mpi_n3d_masterid, MPI_COMM_WORLD);
...
//printf("%3d  %12.10le  %12.10le\n",ii,vr[ii],vi[ii]);
                   ...
                   project_probs(Ekmat,Ur,Ui,vr,vi,wvr,wvi,NBASIS,Norb,Nc_nano,n_converged,eix,k,n_nanostruct);

                   /* Display progress on the command line */
                   printf("Supercell k %d of %d, Eigenvector %d of %d\n",i+1,max_ks,eix+1,n_converged);
                  } /* if master_id */
              } /* for j: n_converged */
               ...
               for(int ii=0;ii<Nc_nano;ii++)
                  {
                       ...
                       fprintf(fpk,"%6d   %12.5le   %12.5le   %12.5le\n",
                               ii,tmp_vec[0],tmp_vec[1],tmp_vec[2]);
                  }  /* for ii */
...
void gen_SmCell_kpGvecs(double **Gvecs,double *k,double **kpGvecs,double bz_max, int Nc)
{
...
//                                row_rv = Rv[2] + n_nanostruct[2]*Rv[1] + n_nanostruct[2]*n_nanostruct[1]*Rv[0];
                                        indx = ix*n_nanostruct[2]*n_nanostruct[1] + iy*n_nanostruct[2] + iz;
