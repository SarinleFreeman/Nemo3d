...
*****************************************************************************
$Header: /repo/nemo3d/src/base/h_cvectr_mult_old.c,v 1.2 2007/08/10 15:03:32
baeh Exp $
*****************************************************************************/
    ...
    for (atom = 0; atom < vdim1(d->Ham); atom++) {
      for (nbr = 0; nbr <= d->Ham_neighbor[atom]; nbr++) {
        if (d->Ham_yc[atom][nbr]) {
          ...
          cmatmul_spds(yc, d->Ham[atom][nbr], x, d->Ham_offset_y[atom][nbr],
                       d->Ham_offset_x[atom][nbr], d->NBasisStates);
        } else {
          if (d->Ham_transp[atom][nbr]) {
            /* HC	 = cadjoint(HC,d->Ham[atom][nbr]); */
            cmatmul_spds_hc(y, d->Ham[atom][nbr], x, d->Ham_offset_y[atom][nbr],
                            d->Ham_offset_x[atom][nbr], d->NBasisStates);
          } else {
            cmatmul_spds(y, d->Ham[atom][nbr], x, d->Ham_offset_y[atom][nbr],
                         d->Ham_offset_x[atom][nbr], d->NBasisStates);
          }
        }
      }
      ...
      for (n = 0; n < d->geo.Neighbors(m); n++) {
        ...
        if (d->opt.ExecParam.Phys.MagneticFieldOn &&
            (d->HzbComplex.isInitialized() ||
             d->opt.ExecParam.ElCalc.ElAlg.HamCompute ==
                 ElAlg_struct::FullStore))
