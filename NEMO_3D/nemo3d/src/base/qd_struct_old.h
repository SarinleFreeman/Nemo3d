...
*****************************************************************************
$Header: /repo/nemo3d/src/base/qd_struct_old.h,v 1.1 2006/11/27 16:33:29 hoonryu
Exp $
*****************************************************************************/
 ...
 *  This class encapsulates data relevant to the entire structure to be
 simulated by NEMO3D. It also has resultant output data that come from
 simulation. */
...
public:
  ...
  /*!< 1D array of data type <materialHandle>. This is the list of materials
   * avaiable in a specific simulation domain. */
  int Abort_result;
  /*!< Flag to chec whether simulation was forced to quit or not. Set to 1 if
   * process is forced to quit. */
  geo_struct geo;
  /*!< geometry infomation.
   * @see geo_struct::geo_struct() */

  imatrix Atom_to_lm;
  /*!< 2D integer matrix. Atom_to_lm[k][0] returns index of a cell this atom
   * belongs. Atom_to_lm[k][1] returns relative index of this atom in a cell of
   * index l. (Note : apart from its absolute atomic index, every atom has
   * another relative index in a unit cell that starts from 1 to
   * geo.AtomperCellMax())
   * @see geo_struct::AtomperCellMax()
   */

  ivectr surfaceAtoms;
  /*!< list of atoms on surface. surfaceAtoms[k] returns atomic index of k-th
   * surface atom. */
  vector<waveFunction> WF;
  /*!< This matrix has raw eigenvalues. This data will be additionally scaled by
   * HAMILTONIAN_SCALE_VALUE, which produces actual eigenvalues that we see from
   * the result. */
  int neigv;
   ...
   * (nth basis of the atom whose cell index = l and atomic index = m) for ev-th
   * eigen value
  ...
  int NBasisStates;
  /*!< number of basis per atom. It will be set according to the selected band
   * model. */
   ...
   * initial, final k values and number of steps between init/final values are
   * extraced from input deck.
   */
  ...
  imatrix l_m_2_atom;
  /*!< l_m_2_atom[l][m] returns absolute atomic index of an atom whose cell
   * index = l and relative atomic index in a unit cell is m.*/
  ...
  ivectr cell_s;
  /*!< starting index of a unitcell segment in each process. cell_s[n] has the
   * starting index of a unitcell segment allocated in n-th process. */

  ivectr cell_ln;
  /*!< size of a unitcell segment (# of unitcell, equivalently) in each process.
   * cell_ln[n] has # of unitcells allocated in n-th process. */
  ivectr seg_s;
  /*!< staring index of a vector segment in each process. seg_s[n] has the
   * starting index of a vector segment allocated in n-th process. */
  ivectr seg_ln;
  /*!< size (length) of a vector segment in each process. seg_ln[n] has the
   * length of a vector segment allocated in n-th process. */
  ivectr nvmap;
