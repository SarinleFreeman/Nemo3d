...
// Uncomment this part if you want to send the matrix as one chunk, not the
// vector by number of column times
...
void Hmatmult_spds_complete(cmatrix Y, qd_struct d, real shift, cmatrix X,
  ...
  if (0 == matrix_multiply % 100 && mpi_n3d_id == 0)
    fprintf(stdout,
            "# of matrix-matrix multiplies: %d num_col: %d total matrix-vector "
            "multiplies: %d\n",
...
#define NEED_TO_USE_STORED_MATRIX_ALWAYS
...
#ifdef TRANSPOSED
  if (d->mm == NULL)
    d->mm = Cmatrix(num_col, d->nvmap[mpi_n3d_id] * d->NBasisStates);
#else
  if (d->mm == NULL)
    d->mm = Cmatrix(d->nvmap[mpi_n3d_id] * d->NBasisStates, num_col);
#endif
#else
  if (d->mm == NULL)
    d->mm = Cmatrix(num_col, d->nvmap[mpi_n3d_id] * d->NBasisStates);
  ...
  if (d->opt.ExecParam.ElCalc.ElAlg.HamCompute == ElAlg_struct::SparseStore &&
    ...
    nyc = Hmatmult_spds_col_stored(Y, YC, d, shift, X, num_col, mpi_n3d_id);
                 ...
                 ElAlg_struct::FullStore &&
             d->Ham) {
    nyc = Hmatmult_spds_col_fullstored_par(Y, YC, d, shift, X, num_col,
                                           mpi_n3d_id);
  } else {
    ...
    if (d->opt.ExecParam.ElCalc.ElAlg.HamCompute != ElAlg_struct::Recompute) {
      if (d->opt.ExecParam.ElCalc.ElAlg.HamCompute ==
        ...
        Hmatmult_spds_col_stored(Y, YC, d, shift, X, num_col, mpi_n3d_id);
                   ...
                   ElAlg_struct::FullStore &&
               d->Ham)
        Hmatmult_spds_col_fullstored_par(Y, YC, d, shift, X, num_col,
                                         mpi_n3d_id);
      else
        die("ERROR in Hmatmult_spds_complete:\n"
            "Unidentified Hamiltonian storage system\n");
  ...
  for (commstep = 0; commstep < 2; commstep++) {
    ...
    if (tgt_i != -1) {
      ...
      // d->commsize[src_i][tgt_i], d->commsize[tgt_i][src_i]);
      if (d->commsize[src_i][tgt_i] >= d->commsize[tgt_i][src_i])
        comm_data_size = d->commsize[src_i][tgt_i];
      else
        comm_data_size = d->commsize[tgt_i][src_i];
#endif

      for (k = 0; k < num_col; k++)
        for (i = 0; i < d->commsize[src_i][tgt_i]; i++)
          for (j = 0; j < d->NBasisStates; j++) {
        // target[k][i*d->NBasisStates+j] =
        // YC[k][cmap_row[tgt_i][i]*d->NBasisStates+j];
#ifdef USE_BLAS
#ifdef TRANSPOSED
            send_data[k][i * d->NBasisStates + j] =
                YC[k][cmap_row[tgt_i][i] * d->NBasisStates + j];
#else
            send_data[k][i * d->NBasisStates + j] =
                YC[cmap_row[tgt_i][i] * d->NBasisStates + j][k];
#endif
#else

#ifdef SENDRECV_ONE_CHUNK
            // send_data[k*d->NBasisStates*d->commsize[src_i][tgt_i]+i*d->NBasisStates+j]
            // = YC[k][cmap_row[tgt_i][i]*d->NBasisStates+j];
            send_data[k * d->NBasisStates * comm_data_size +
                      i * d->NBasisStates + j] =
                YC[k][cmap_row[tgt_i][i] * d->NBasisStates + j];
// comm_data_size += d->NBasisStates*d->commsize[src_i][tgt_i];
      ...
      MPI_Sendrecv( // target[k],
          &send_data[0], d->NBasisStates * num_col * comm_data_size * 2,
          // 2*d->commsize[src_i][tgt_i]*d->NBasisStates*num_col,
          // 2*maxcom*num_col,
          MPI_DOUBLE, tgt_i, MPI_rc_distr,
          // source[k],
          &receive_data[0], d->NBasisStates * num_col * comm_data_size * 2,
          // 2*d->commsize[tgt_i][src_i]*d->NBasisStates*num_col,
          // 2*maxcom*num_col,
          MPI_DOUBLE, tgt_i, MPI_rc_distr, MPI_COMM_WORLD, &mpi_n3d_status);
#else
      for (k = 0; k < num_col; k++) {
        MPI_Sendrecv( // target[k],
            send_data[k], 2 * d->commsize[src_i][tgt_i] * d->NBasisStates,
            MPI_DOUBLE, tgt_i, MPI_rc_distr,
            // source[k],
            receive_data[k], 2 * d->commsize[tgt_i][src_i] * d->NBasisStates,
            MPI_DOUBLE, tgt_i, MPI_rc_distr, MPI_COMM_WORLD, &mpi_n3d_status);
      }
#endif // SENDRECV_ONE_CHUNK
      ...
      for (k = 0; k < num_col; k++) {
        for (i = 0; i < d->commsize[tgt_i][src_i]; i++) {
          for (j = 0; j < d->NBasisStates; j++) {
            // target[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx+j].r +=
            // source[k][i*d->NBasisStates + j].r;
            // target[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx+j].i +=
            // source[k][i*d->NBasisStates + j].i;
#ifdef USE_BLAS
#ifdef TRANSPOSED
            Y[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx + j].r +=
                receive_data[k][i * d->NBasisStates + j].r;
            Y[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx + j].i +=
                receive_data[k][i * d->NBasisStates + j].i;
#else
            Y[d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx + j][k].r +=
                receive_data[k][i * d->NBasisStates + j].r;
            Y[d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx + j][k].i +=
                receive_data[k][i * d->NBasisStates + j].i;
#endif
#else
#ifdef SENDRECV_ONE_CHUNK
            // Y[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx+j].r +=
            // receive_data[k*d->NBasisStates*d->commsize[tgt_i][src_i]+i*d->NBasisStates
            // + j].r; Y[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx+j].i
            // +=
            // receive_data[k*d->NBasisStates*d->commsize[tgt_i][src_i]+i*d->NBasisStates
            // + j].i;
            Y[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx + j].r +=
                receive_data[k * d->NBasisStates * comm_data_size +
                             i * d->NBasisStates + j]
                    .r;
            Y[k][d->vecmap[tgt_i][cmap_col[tgt_i][i]] - lo_indx + j].i +=
                receive_data[k * d->NBasisStates * comm_data_size +
...
*/
int Hmatmult_spds_col_fullstored_par(cmatrix Y, cmatrix YC, qd_struct d,
                                     real shift, cmatrix X, int num_col,
                                     int proc) {
 ...
 * have a Hermitian conjugate associated with them.... */
          ...
          zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA,
                (MKL_Complex16 *)(d->Ham[atom][nbr]), &K,
                (MKL_Complex16 *)(TMPx), &K, &BETA, (MKL_Complex16 *)(TMPy),
                &M);
#else
          zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (d->Ham[atom][nbr]), &K,
                (TMPx), &K, &BETA, (TMPy), &M);
#endif
          ...
          zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA,
                (MKL_Complex16 *)(d->Ham[atom][nbr]), &K,
                (MKL_Complex16 *)(X[offsetx]), &N, &BETA,
                /*&(YC[offsety][0])*/ (MKL_Complex16 *)(TMP), &M);
#else
          zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (d->Ham[atom][nbr]), &K,
                (X[offsetx]), &N, &BETA, (TMP), &M);
#endif // USE_MKL
          ...
          if (d->Ham_transp[atom][nbr]) {
            ...
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (MKL_Complex16 *)(HAMT),
                  &K, (MKL_Complex16 *)(TMPx), &K, &BETA,
                  (MKL_Complex16 *)(TMPy), &M);
#else
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (HAMT), &K, (TMPx), &K,
                  &BETA, (TMPy), &M);
#endif // USE_MKL
            ...
            // Transpose conjugate
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (MKL_Complex16 *)(HAMT),
                  &K, (MKL_Complex16 *)(X[offsetx]), &N, &BETA,
                  /*&(Y[offsety][0])*/ (MKL_Complex16 *)(TMP), &M);
#else
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, HAMT, &K, (X[offsetx]),
                  &N, &BETA, (TMP), &M);
#endif
...
/* Computes Y=(H-Is)X where H is the Hamiltonian, shift is input shift,
   ...
   in YC. It is arranged as follows:
   YC[i*d-nb] = Y[ycmap[i][0]*d->NBasisStates]. */
int Hmatmult_spds_col(cmatrix Y, cmatrix YC, qd_struct d, real shift, cmatrix X,
  ...
  for (int l = isx; l < iex; l++) {
    ...
    for (int m = 0; m < d->geo.AtomsPerCellMax(); m++) {
      ...
      for (n = 0; n < d->geo.Neighbors(m); n++) {
         ...
         * in calculation of Peierl's phase due to Magnetic field. nnv_B is
         * adjusted for strain later */
        ...
        if (/*cindx < 0 &&*/ has_periodicity && d->geo.ssmap[l][m][n] != -1) {
          /* When crystal is tilted ssmap should be used for connecting atoms in
           * cell with index = 0 as well */
        ...
        */
        ...
        if (out) {
          ...
          if (eik.i != 0.0 || eik.r != 1.0) {
            ...
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA,
                  (MKL_Complex16 *)(d->ho), &K, (MKL_Complex16 *)(TMPx), &K,
                  &BETA, (MKL_Complex16 *)(TMPy), &M);
#else
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &eik, (d->ho), &K, (TMPx), &K,
                  &BETA, (TMPy), &M);
#endif
            ...
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA,
                  (MKL_Complex16 *)(d->ho), &K, (MKL_Complex16 *)(X[is1]), &N,
                  &BETA, (MKL_Complex16 *)(TMP), &M);
#else
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &eik, (d->ho), &K, (X[is1]), &N,
                  &BETA, (TMP), &M);
#endif
...
#else
            for (int j = 0; j < num_col; j++)
              cmatmul_spds_sc(YC[j], d->ho, X[j], isyc, is1, d->NBasisStates,
                              eik);
#endif
          } else {
#ifdef USE_BLAS
            ...
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA,
                  ...
                  &BETA, (MKL_Complex16 *)(TMPy), &M);
#else
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (d->ho), &K, (TMPx), &K,
                  &BETA, (TMPy), &M);
#endif
            ...
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA,
                  ...
                  &BETA, (MKL_Complex16 *)(TMP), &M);
#else
            zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (d->ho), &K, (X[is1]),
                  &N, &BETA, (TMP), &M);
#endif
...
#else
            for (int j = 0; j < num_col; j++)
              cmatmul_spds_sc(Y[j], d->ho, X[j], isn - imin, is1,
                              d->NBasisStates, eik);
#endif
          } else {
#ifdef USE_BLAS
...
#else
            for (int j = 0; j < num_col; j++)
              cmatmul_spds_offdiag(Y[j], d->ho, X[j], isn - imin, is1,
                                   d->NBasisStates);
          ...
          for (int ii = 0; ii < 20; ii++)
            for (int jj = 0; jj < 20; jj++)
              fprintf(fp, "%4d %4d %e %e\n",
                      1 + 20 * d->geo.NbrCell(m, n, 3) + ii, 1 + 20 * m + jj,
...
#ifdef USE_MKL
      zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (MKL_Complex16 *)(d->hd), &K,
            (MKL_Complex16 *)(TMPx), &K, &BETA, (MKL_Complex16 *)(TMPy), &M);
#else
      zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (d->hd), &K, (TMPx), &K,
            &BETA, (TMPy), &M);
#endif
...
// Transpose first
#ifdef USE_MKL
      zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (MKL_Complex16 *)(d->hd), &K,
            (MKL_Complex16 *)(X[is1]), &N, &BETA, (MKL_Complex16 *)(TMP), &M);
#else
      zgemm(&TRANSA, &TRANSB, &M, &N, &K, &ALPHA, (d->hd), &K, (X[is1]), &N,
            &BETA, (TMP), &M);
#endif
  ...
  if (d->opt.ExecParam.Phys.Disorder1 != 0.0) {
    Randomize_H(d, proc);
    d->nvmap[proc] =
        Hmatmult_spds_col_fullstored_par(Y, YC, d, shift, X, num_col, proc);
  }
...
int Hmatmult_spds_col_prep_store(cmatrix Y, cmatrix YC, qd_struct d, real shift,
  ...
  nml_memory_report("before Allocation of Hamiltonian Storage - "
                    "Hmatmult_spds_col_prep_store ");
  ...
  */
  ...
  if (d->opt.ExecParam.ElCalc.ElAlg.HamCompute == ElAlg_struct::FullStore) {
    ...
    /*
    if(!d->opt.ExecParam.Phys.MagneticFieldOn)
       d->Hzb.Initialize(d->BandModel,d->NBasisStates, Zatom, Zatom_surf, Nout,
    Nin, false, HBxy); else
    ...
    if(d->opt.ExecParam.ElCalc.ElAlg.Efficiency==ElAlg_struct::Speed)
       d->HzbComplex.Initialize(d->BandModel,d->NBasisStates, Zatom, Zatom_surf,
    Nout, Nin, HBxy);
    */
