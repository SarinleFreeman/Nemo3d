...
void adjust_crystal_for_electronic_calculation(qd_struct d)
{
   ...
   for (; sh != d->opt.Dev.ShapeList.end(); sh++) {
      all_strain_and_electronic = all_strain_and_electronic && sh->use_for_strain_and_electronic;
   }
   ...
   if (all_strain_and_electronic){
     ...
     MPI_Bcast(&d->geo.AtomType[0][0], 
               d->geo.N_Cell*d->geo.cellgeom.AtomsPerCellMax(), 
               MPI_CHAR, mpi_n3d_masterid, MPI_COMM_WORLD);
#else   /* MEMORY_SAVE_NEWSTRUCT  */
     MPI_Bcast(&d->geo.AtomType[0][0], 
               d->geo.N_Cell*d->geo.cellgeom.AtomsPerCellMax(), 
               MPI_INT, mpi_n3d_masterid, MPI_COMM_WORLD);
...
//   cout<<"the number of atoms removed by user define option "<<removed_atoms<<endl;
	 ...
	 for ( n = 0; n < d->geo.Neighbors(m); n++ ) {
	    ...
	    /* Get location indices of the unit cell that we will couple to. 
	       ...
	       to our current location plus the neighbor displacement */
            int cindx, aindx; 
            if (!d->geo.is_tilted) { /* Connect atoms as follows when lattice is NOT tilted */ 
	      ...
	      aindx = d->geo.NbrCell(m,n,3);
              ...
              if (/*cindx < 0 &&*/ d->geo.ssmap[l][m][n]!=-1) { /* When crystal is tilted ssmap should be used for connecting atoms in cell with index = 0 as well */
...
int mpi_segment_init( qd_struct d, int nproc )
{
    ...
    * d->cell_ln[i] contains the length (number) of cells inside a single processor
    * number "i"
    * */
   /* set d->cell_ln */
   do {
   ...
   for ( i=0; i < d->nproc; i++ ) {
     ...
     for ( int c=d->cell_s[i]; c < d->cell_s[i]+d->cell_ln[i]; c++ ){
       /* loop through the atoms in each cell, break out if there is a initiated atom found */
      ...
      * begin from the back end of the included cells and cound back down to the beginning*/
	...
	* determine the length of the overall vector on CPU #i based the Hamiltonian resulution */ 
       for ( j=d->geo.AtomsPerCellMax()-1; j >= 0; j-- ){
         if ( d->geo.AtomType[c][j] ) {
            d->seg_ln[i] = d->geo.l_m_2_offset_ham_tot[c][j] + 
               d->NBasisStates - d->seg_s[i];
            break;
         }
       }
       if (d->seg_ln[i]!=-1) break;
     } 
    ...
    * 			total number of cells are evenly ditributed across all CPUs
    * d->cell_s[i]      starting cell index for CPU #i
    * 			CPU 0 starts with index 0
    * 			CPU 1 starts after cell_ln[0]
    * 			CPU 2 starts after cell_ln[0]+cell_ln[1]
    * d->seg_s[i]       starting atom index for CPU #i	
    *                   starting index of the first orbital element of the Hamltonian on CPU #i
    *                   starting index of the first vector element in the Hamiltonian
    * d->seg_ln[i]      length of the vector in the Hamiltonian basis
    *                   the length determination explicitly depende on the existence 
    *                   of uniform unit cells and a constant number of orbitals on each atom
    * d->maxseg         maximum segment length of all seg_ln[i]
    *
    * */
#ifdef NEW_VECMAP
   for ( i=0; i < d->nproc; i++ ){
      ...
      if (d->commsize[mpi_n3d_id][i]>0 || d->commsize[i][mpi_n3d_id]>0 || mpi_n3d_id==i){
...
#ifdef PRINT_COMMUNICATOR
   for ( source_i=0; source_i<d->nproc; source_i++ )
      printf("\nsource=%d targets=%2i,%2i", 
             source_i,d->commorder[source_i][0],d->commorder[source_i][1]); 
#endif /* PRINT_COMMUNICATOR */
   return(0);
}
...
void basis_init( qd_struct d )
{
   if      (d->opt.Dev.band_model==Dev_struct::Bands_1_s_nospin){
      ...
      die("Unsupported bandstructure model specified (Bands_1_s_nospin, Bands_10_sp3ss_spin, Bands_20_sp3d5ss_spin, Bands_10_sp3d5ss_nospin)\n");
   ...
   if (d->BandModel==BM_1_s_nospin) {
   ...
   else if (d->BandModel==BM_20_sp3d5ss_spin || d->BandModel==BM_10_sp3ss_spin) {
	 ...
	 d->BasisCation[i] = i+10;
      ...
      die("Unsupported bandstructure model specified (Bands_1,Bands_10_so_s,Bands_20_s,Bands_10_sp3d5s)\n");
...
/*   fp = fopen("GeoInfo.in","r");
   if(fp != NULL) {
      fclose(fp);
//      di.readTiltParams(Nxx,Nyy);  /* this function reads attitional /* tilt_fix */
//                           information needed for defining tilted geometry */ /* tilt_fix */
...
/* End Additions for tilt */
   ...
   {  
     d->geo.l_m_2_atomdsp = F3tensor(0,d->geo.N_Cell - 1,  
                                     0,d->geo.AtomsPerCellMax() - 1 ,  
                                     0, 2) ;  
     if ( 0 == mpi_n3d_id ) { 
       // nml_memory_report("RESTORED THE STRAIN ARRAY AFTER SETUP_OF_CRYSTAL_STRUCTURE") ;  
     } 
   } 
...
void setup_hamiltonian(qd_struct d) 
{
   ...
   nml_memory_report("setup_hamiltonian: After setup of setup_crystal_structure");
   // print atomic positions <--> d->opt.ExecParam.ElCalc.ElOut.PsiSqr is on

   // cout << "FAO:  uncommented writePos() in setup_hamiltonian()" << endl;
   // cout << "FAO:  the following file contains the PRE-STRAIN atomic positions" << endl;
if(d->opt.ExecParam.Output.AtomPosBeforeStrain) writePos(d, "atom_pos");
   ...
   if(d->opt.ExecParam.Strain.Keating.UpdateLatt){
     ...
     cout_master 
      << "Updating lattice constants before strain calculaton/reading displacements\n"; 
...
#endif
     cout_master << "Begin the First Strain Calculation or reading dispacements\n";
...
if(d->opt.ExecParam.Strain.StrainOut.StrainElOnly)
{
    ...
    masterPrint("Writing atomic displacements to '%s' for the electronic domain.", filename);
 #ifdef DISPL_SINGLE_PRECISION
...
if(d->opt.ExecParam.Strain.StrainOut.TensionOutput){
   ...
   nml_memory_report("setup_hamiltonian: adjusting memory for electronic structure calculation ");
   // print atomic positions <--> d->opt.ExecParam.ElCalc.ElOut.PsiSqr is on
   MPI_TOC(mpiTiming.setup, mpiTiming.setup_0);

   // cout << "FAO:  the following file contains the POST-STRAIN atomic positions" << endl;
  if(d->opt.ExecParam.Output.AtomPosAfterStrain) writePos(d, "atom_pos");
   ...
   if (d->opt.ExecParam.Phys.PotentialFile != "NULL") {
      ...
      if (fp) {
         ...
         cout_master << "NO SUCH FILE -- setting electrostatic potential to zero.\n";
      }
   }
   ...
   if(d->opt.ExecParam.Phys.EfieldON && d->opt.ExecParam.Phys.ImpNumber <= 0 )
     {
      ...
      for(int l=0; l < d->geo.N_Cell; l++ ){
          for(int m=0; m < d->geo.AtomsPerCellMax(); m++ ){
              ...
              real V=-(d->opt.ExecParam.Phys.Ex*(pos[0]-d->opt.Dev.ShapeList[0].xorigin))-
                      (d->opt.ExecParam.Phys.Ey*(pos[1]-d->opt.Dev.ShapeList[0].yorigin))-
                      (d->opt.ExecParam.Phys.Ez*(pos[2]-d->opt.Dev.ShapeList[0].zorigin));
              d->phi[l][m]+=V;
             }//m
         }//l
     } /* if */
    ...
    if (d->opt.ExecParam.Phys.ImpNumber > 0) {
        ...
        for(int i=0; i<numImpurities;i++){
            if((xyz_desired[3*i]<d->opt.Dev.ShapeList[0].xorigin) ||
               (xyz_desired[3*i]>(d->opt.Dev.ShapeList[0].xorigin+d->opt.Dev.ShapeList[0].x)) ||
               (xyz_desired[3*i+1]<d->opt.Dev.ShapeList[0].yorigin) ||
               (xyz_desired[3*i+1]>(d->opt.Dev.ShapeList[0].yorigin+d->opt.Dev.ShapeList[0].y)) ||
               (xyz_desired[3*i+2]<d->opt.Dev.ShapeList[0].zorigin) ||
               (xyz_desired[3*i+2]>(d->opt.Dev.ShapeList[0].zorigin+d->opt.Dev.ShapeList[0].z)) 
			)        
		    {
                flag=true;
                imp=i+1;
        ...
        if(flag)
            cout_master<<"Error in specifying coordinates of impurity "<<imp<<endl;
        ...
        for(int i=0;i<numImpurities;i++){
            ...
            for (int l=0; l < d->geo.N_Cell; l++ ){
                for (int m=0; m < d->geo.AtomsPerCellMax(); m++ ){ 
                    ...
                    d->geo.getPosition_strained(pos, l, m);
                    real R2 = (pos[0]-xyz_desired[3*i])*(pos[0]-xyz_desired[3*i]) +
                              (pos[1]-xyz_desired[3*i+1])*(pos[1]-xyz_desired[3*i+1]) +
                              (pos[2]-xyz_desired[3*i+2])*(pos[2]-xyz_desired[3*i+2]);
            ...
            cout_master << "Placing a donor at r = ("
                  << pos0[3*i] << ","
                  << pos0[3*i+1] << ","
                  << pos0[3*i+2] << ") with U0=" << U0 << " eV ... " << endl;
        ...
        for (int l=0; l < d->geo.N_Cell; l++ ){
            for (int m=0; m < d->geo.AtomsPerCellMax(); m++ ){
                ...
                for(int i=0;i<numImpurities;i++){
                    ...
                    //Rxy[i]=sqrt((pos[1]-pos0[3*i+1])*(pos[1]-pos0[3*i+1]) + (pos[2]-pos0[3*i+2])*(pos[2]-pos0[3*i+2])); // YuWang
					//Rz[i] = sqrt((pos[0]-pos0[3*i])*(pos[0]-pos0[3*i]));
		   ...
		   d->phi[l][m] += ( l==Zcell[i] && m==Zatom[i] ? U0 : inv_eps * esqr / R[i]);
		    //d->phi[l][m] = 0;
                    // sunnylee - for impurity donor potential used in Si:P 1D bandstructure
					// it will be commented out for this is only applied to specific application.
					// Again, This should NOT be uncommented...
					#if 0 
                ...
                if(d->opt.ExecParam.Phys.include_nuclear_term) d->phi[l][m] -= V_nuc;
...
/*
   ...
   for (int m=0; m < d->geo.AtomsPerCellMax(); m++ ) {
      ...
      if (compute_strain_tensor(&exx, &eyy, &ezz, &exy, &exz, &eyz, d, l, m) == -1) 
           continue;
   }}
...
//The following portion is added by Shaikh
  ...
  if(d->geo.isCellZB_110() || d->geo.isCellZB_110_small() || d->geo.isCellZB_111() || d->geo.isCellUserDef()) {
     fprintf(fp,"'compute_strain_tensor' not implemented for unit cells different than cubic Zincblende unit cell");
     fprintf(fpc,"'compute_strain_tensor' not implemented for unit cells different than cubic Zincblende unit cell");
    }
  else {
     //MPI_TIC(mpiTiming.strain_total_0);
     //for ( l=isx; l < iex; l++ ) {
     ...
     for (int m=0; m < d->geo.AtomsPerCellMax(); m++ ) {
           ...
           if (
               compute_strain_tensor(&exx, &eyy, &ezz, &exy, &exz, &eyz, d, l, m)
          == -1){fprintf(fp," 0 0 0 0 0 0 \n"); continue;}
...
void setup_hamiltonian_for_postprocessing(qd_struct d)
{
   ...
   if(d->opt.ExecParam.Strain.Keating.UpdateLatt){
     ...
     double tmp_z = d->opt.ExecParam.Strain.Keating.UpdateLattConst[2];
     d->geo.updateOnlyLattConst(tmp_x, tmp_y, tmp_z);
     cout_master << "Lattice constants are updated before reading displacements\n"; 
     cout_master <<" New lattice Constants: ["<<d->geo.lattice_x<<", "
                 << d->geo.lattice_y <<", "<< d->geo.lattice_z<<"]\n";
   }
