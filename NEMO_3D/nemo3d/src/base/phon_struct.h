...
class phon_struct {
 ...
 public:
  ...
  int states_in_row;//!< maximum number of nonzero elements in every row in Keating model

  //The storage of the Dynamical Matrix is almost similar to that by Harwell-Boeing 
  ...
  int nbr_uc;/*!< =1 for the standart Keating model with second-neares neighbors interaction limit.
               Number of neighbors unit cells in x direction, whose
               atoms interact with the boundary atoms of the given unit cell*/
  //Parallel stuff:
  ivectr seg_col_s, seg_col_ln; /*!< [d->nproc] Initial index and the number of the
                                  DM elements on every processor.
                                  The partition is by columns.*/
  ivectr min_row, max_row;/*!< [d->nproc] minimal and maximal indexies of the rows of DM
  ...
  ivectr N_cols,N_rows;/*!< Number of columns/rows of DM stored on every processor*/
  ivectr mpi_left,mpi_right;/*!< Left/right processors mpi_n3d_id  for communication*/
  ...
  void comm_vectors_allocate();//Allocates memory for parts of the eigenvector that
                               //participate in communication

  void comm_vectors_deallocate();//Deallocates memory for parts of the eigenvector that
                                 //participate in communication
  ...
  void Picture_DM(int DM_total_size);/*Picture the Dynamical matrix stored on the processor
