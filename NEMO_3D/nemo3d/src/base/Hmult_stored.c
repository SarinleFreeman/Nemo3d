    ...
    if (has_periodicity && kdotl != 0.0) {
      ...
      for (i = 0; i < Nb_2_20; i++) {
        ...
        for (j = 0; j < Nb_2_20; j++) {
          ...
          if (d->opt.ExecParam.Phys.MagneticFieldOn) {
            double phase_r = cs * d->HzbComplex.HphaseIn[k].r -
                             sn * d->HzbComplex.HphaseIn[k].i;
            double phase_i = cs * d->HzbComplex.HphaseIn[k].i +
                             sn * d->HzbComplex.HphaseIn[k].r;
    ...
    if (has_periodicity && kdotl != 0.0) {
      ...
      for (i = 0; i < Nb_2_20; i++) {
        for (j = 0; j < Nb_2_20; j++) {
          ...
          if (d->opt.ExecParam.Phys.MagneticFieldOn) {
            double phase_r = cs * d->HzbComplex.HphaseOut[k].r -
                             sn * d->HzbComplex.HphaseOut[k].i;
            double phase_i = cs * d->HzbComplex.HphaseOut[k].i +
                             sn * d->HzbComplex.HphaseOut[k].r;
...
#pragma message("INTEL_MODIFIED: Hmult memset opt`s is ON")
  ...
  case BM_10_sp3ss_spin:
    if (d->HzbComplex.isInitialized()) {
      for (int j = 0; j < num_col; j++)
        Hv_stored_10_HzbComplex(Y[j], YC[j], X[j], d);
      break;
    } else {
      for (int j = 0; j < num_col; j++)
        Hv_stored_10_Hzb(Y[j], YC[j], X[j], d);
      break;
    }
  ...
  case BM_10_sp3d5ss_nospin:
    if (d->HzbComplex.isInitialized()) {
      for (int j = 0; j < num_col; j++)
        Hv_stored_10_sp3d5s_HzbComplex(Y[j], YC[j], X[j], d);
      break;
    } else {
      for (int j = 0; j < num_col; j++)
        Hv_stored_10_sp3d5s_Hzb(Y[j], YC[j], X[j], d);
      break;
    }
