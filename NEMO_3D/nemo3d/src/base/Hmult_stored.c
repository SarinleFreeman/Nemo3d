...
// sunnylee comment: just for performance test purposes
//#undef INTEL_MODIFIED
#endif
...
#undef MPI_TIMEmeasure
   ...
   double T____diag=0.0, T____mag=0.0, T____Hdu=0.0, T____Hds=0.0, T____Hu=0.0, T____Ho=0.0;
   double T_V0_diag=0.0, T_V0_mag=0.0, T_V0_Hdu=0.0, T_V0_Hds=0.0, T_V0_Hu=0.0, T_V0_Ho=0.0;
   double T_V1_diag=0.0, T_V1_mag=0.0, T_V1_Hdu=0.0, T_V1_Hds=0.0, T_V1_Hu=0.0, T_V1_Ho=0.0;
...
#include "Hmult_stored_SSE_1.c"
   ...
   case BM_20_sp3d5ss_spin:
      if(d->HzbComplex.isInitialized()) {
         Hv_stored_20_HzbComplex(y, yc, x, d); break;
      }
      else {
         ...
         printf("CPU %3d Vector-time:  T____diag=%g T____mag=%g T____Hdu=%g T____Hds=%g T____Hu=%g T____Ho=%g\n",mpi_n3d_id,T____diag/counter, T____mag/counter, T____Hdu/counter, T____Hds/counter, T____Hu/counter, T____Ho/counter), fflush(stdout);
         printf("CPU %3d Vector-time:  T_V0_diag=%g T_V0_mag=%g T_V0_Hdu=%g T_V0_Hds=%g T_V0_Hu=%g T_V0_Ho=%g\n",mpi_n3d_id,T_V0_diag/counter, T_V0_mag/counter, T_V0_Hdu/counter, T_V0_Hds/counter, T_V0_Hu/counter, T_V0_Ho)/counter, fflush(stdout);
         printf("CPU %3d Vector-time:  T_V1_diag=%g T_V1_mag=%g T_V1_Hdu=%g T_V1_Hds=%g T_V1_Hu=%g T_V1_Ho=%g\n",mpi_n3d_id,T_V1_diag/counter, T_V1_mag/counter, T_V1_Hdu/counter, T_V1_Hds/counter, T_V1_Hu/counter, T_V1_Ho)/counter, fflush(stdout);
   ...
   case BM_10_sp3ss_spin:
      if(d->HzbComplex.isInitialized()) {
         Hv_stored_10_HzbComplex(y, yc, x, d); break;
      }
      else {
         Hv_stored_10_Hzb(y, yc, x, d); break;
      }
   case BM_10_sp3d5ss_nospin:
      if(d->HzbComplex.isInitialized()) {
         Hv_stored_10_sp3d5s_HzbComplex(y, yc, x, d); break;
      }
      else {
         Hv_stored_10_sp3d5s_Hzb(y, yc, x, d); break;
      }
   case BM_1_s_nospin:
      die("ERROR in Hmult_spds_col_stored: Bands_1_s_nospin is not yet implemented"); break;
   default:
      die("ERROR in Hmult_spds_col_stored: not yet implemented Hmult_spds_col_stored"); break;
   }
   ...
   if(d->opt.ExecParam.Phys.MagneticFieldOn) {
     ...
     for (atom=0; atom<d->Hzb.Natom; atom++) {
       ...
       for(int i=0; i<nb; i++) {
         ...
         y[offset+i+nb].r += d->Hzb.HBxy.r * x[offset+i].r
                           + d->Hzb.HBxy.i * x[offset+i].i;
         y[offset+i+nb].i += d->Hzb.HBxy.r * x[offset+i].i
                           - d->Hzb.HBxy.i * x[offset+i].r;
     }}
   }
...
#include "Hmult_stored_SSE_3a.c"
   ...
   for (k=0; k<thislength; k++) {
      ...
      if (has_periodicity) {
         ...
         kdotl = d->geo.phaseBloch(d->kxL, d->kyL, d->kzL, 
                                   d->Atom_to_lm[aindx][0], 
...
//==================================================================================================
  ...
  thislength = nml_iv_extent(d->Hzb.indxHO_to_row) ;
   for (k=0; k<thislength; k++) {
      ...
      if (has_periodicity && kdotl!=0.0) {
	    ...
	    if(d->opt.ExecParam.Phys.MagneticFieldOn) {
              Ho_indx_r = d->Hzb.Ho[indx]
                                      * (cs*d->Hzb.HphaseOut[k].r - sn*d->Hzb.HphaseOut[k].i);
              Ho_indx_i = d->Hzb.Ho[indx]
                                      * (cs*d->Hzb.HphaseOut[k].i + sn*d->Hzb.HphaseOut[k].r);
            }
            else {
         ...
         for (i=0; i<Nb_2; i++) {
          ...
          for (j=0; j<Nb_2; j++) {
            ...
            int J = d->Hzb.indxHO_to_col[k] + j;
	    if(d->opt.ExecParam.Phys.MagneticFieldOn) {
              yc[I].r += d->Hzb.Ho[indx] * (x[J].r*d->Hzb.HphaseOut[k].r - x[J].i*d->Hzb.HphaseOut[k].i);
              yc[I].i += d->Hzb.Ho[indx] * (x[J].i*d->Hzb.HphaseOut[k].r + x[J].r*d->Hzb.HphaseOut[k].i);
	 
              yc[I+Nb_2].r += d->Hzb.Ho[indx] 
                            * (x[J+Nb_2].r*d->Hzb.HphaseOut[k].r - x[J+Nb_2].i*d->Hzb.HphaseOut[k].i);
              yc[I+Nb_2].i += d->Hzb.Ho[indx]
                            * (x[J+Nb_2].r*d->Hzb.HphaseOut[k].i + x[J+Nb_2].i*d->Hzb.HphaseOut[k].r);
            }
            else {
  ...
  /* printf("Vector-time:  T____diag=%g T____mag=%g T____Hdu=%g T____Hds=%g T____Hu=%g T____Ho=%g\n",T____diag, T____mag, T____Hdu, T____Hds, T____Hu, T____Ho), fflush(stdout); */
#endif
         ...
         for (i=0; i<Nb; i++) {
            ...
            for (j=0; j<Nb; j++) {
               ...
               if(d->opt.ExecParam.Phys.MagneticFieldOn) {
			     ...
			     __m128d tmp1, y_i, x_j, y_j, x_i,  __mul;
...
#undef MPI_TIMEmeasure
#ifdef  MPI_TIMEmeasure
   double T_diag=0.0, T_mag=0.0, T_Hdu=0.0, T_Hds=0.0, T_Hu=0.0, T_Ho=0.0;
   MPI_TIME_INIT(Tstart);
   MPI_TIC(Tstart);
#endif
      ...
      if (has_periodicity && kdotl!=0.0) {
         ...
         for (i=0; i<Nb_2_20; i++) {
            ...
            for (j=0; j<Nb_2_20; j++) {
              ...
              /* Commented/Added for fixing bugs in Magnetic field */
			  #if 0
              register double Hu_indx_r = cs * d->HzbComplex.Hu[indx].r 
                                        - sn * d->HzbComplex.Hu[indx].i;
              register double Hu_indx_i = sn * d->HzbComplex.Hu[indx].r 
                                        + cs * d->HzbComplex.Hu[indx].i;
			  #endif

              register double Hu_indx_r, Hu_indx_i;

              if(d->opt.ExecParam.Phys.MagneticFieldOn) {
                  double phase_r = cs*d->HzbComplex.HphaseIn[k].r - sn*d->HzbComplex.HphaseIn[k].i;
                  double phase_i = cs*d->HzbComplex.HphaseIn[k].i + sn*d->HzbComplex.HphaseIn[k].r;
                  Hu_indx_r = phase_r * d->HzbComplex.Hu[indx].r
                            - phase_i * d->HzbComplex.Hu[indx].i;
                  Hu_indx_i = phase_i * d->HzbComplex.Hu[indx].r
                            + phase_r * d->HzbComplex.Hu[indx].i;
              }
              else {
                  Hu_indx_r = cs * d->HzbComplex.Hu[indx].r
                            - sn * d->HzbComplex.Hu[indx].i;
                  Hu_indx_i = sn * d->HzbComplex.Hu[indx].r
                            + cs * d->HzbComplex.Hu[indx].i;
              }
              /* End: Commented/Added for fixing bugs in Magnetic field */
         ...
         for (i=0; i<Nb_2_20; i++) {
            ...
            for (j=0; j<Nb_2_20; j++) {
               ...
               if(d->opt.ExecParam.Phys.MagneticFieldOn) {
                  double phase_r = d->HzbComplex.HphaseIn[k].r;
                  double phase_i = d->HzbComplex.HphaseIn[k].i;
                  Hu_indx_r = phase_r * d->HzbComplex.Hu[indx].r
                            - phase_i * d->HzbComplex.Hu[indx].i;
                  Hu_indx_i = phase_i * d->HzbComplex.Hu[indx].r
                            + phase_r * d->HzbComplex.Hu[indx].i;
                 ...
                 Hu_indx_r = d->HzbComplex.Hu[indx].r;
                 Hu_indx_i = d->HzbComplex.Hu[indx].i;
               }
               /* End: Commented/Added for fixing bugs in Magnetic field */
   ...
   for (k=0; k<thislength; k++) {
      ...
      if (has_periodicity && kdotl!=0.0) {
         ...
         for (j=0; j<Nb_2_20; j++) {
            ...
            /* Commented/Added for fixing bugs in Magnetic field */
			#if 0
            register double Ho_indx_r = cs *  d->HzbComplex.Ho[indx].r 
                                      - sn * d->HzbComplex.Ho[indx].i;
            register double Ho_indx_i = sn *  d->HzbComplex.Ho[indx].r 
                                      + cs * d->HzbComplex.Ho[indx].i;
			#endif

			register double Ho_indx_r, Ho_indx_i;

            if(d->opt.ExecParam.Phys.MagneticFieldOn) {
               double phase_r = cs*d->HzbComplex.HphaseOut[k].r - sn*d->HzbComplex.HphaseOut[k].i;
               double phase_i = cs*d->HzbComplex.HphaseOut[k].i + sn*d->HzbComplex.HphaseOut[k].r;
               Ho_indx_r = phase_r * d->HzbComplex.Ho[indx].r
                         - phase_i * d->HzbComplex.Ho[indx].i;
               Ho_indx_i = phase_i * d->HzbComplex.Ho[indx].r
                         + phase_r * d->HzbComplex.Ho[indx].i;
            }
            else {
               Ho_indx_r = cs * d->HzbComplex.Ho[indx].r
                         - sn * d->HzbComplex.Ho[indx].i;
               Ho_indx_i = sn * d->HzbComplex.Ho[indx].r
                         + cs * d->HzbComplex.Ho[indx].i;
            }
              /* End: Commented/Added for fixing bugs in Magnetic field */
         ...
         for (j=0; j<Nb_2_20; j++) {
            ...
            if(d->opt.ExecParam.Phys.MagneticFieldOn) {
               double phase_r = d->HzbComplex.HphaseOut[k].r;
               double phase_i = d->HzbComplex.HphaseOut[k].i;
               Ho_indx_r = phase_r * d->HzbComplex.Ho[indx].r
                         - phase_i * d->HzbComplex.Ho[indx].i;
               Ho_indx_i = phase_i * d->HzbComplex.Ho[indx].r
                         + phase_r * d->HzbComplex.Ho[indx].i;
               ...
               Ho_indx_r = d->HzbComplex.Ho[indx].r;
               Ho_indx_i = d->HzbComplex.Ho[indx].i;
            }
            ...
            yc[I+Nb_2_20].r += Ho_indx_r * x[J+Nb_2_20].r - Ho_indx_i * x[J+Nb_2_20].i;
            yc[I+Nb_2_20].i += Ho_indx_r * x[J+Nb_2_20].i + Ho_indx_i * x[J+Nb_2_20].r;
            /* End: Commented/Added for fixing bugs in Magnetic field */
         }}
      }
   }
#ifdef  MPI_TIMEmeasure
  MPI_TOC(T_Ho, Tstart);
  printf("Vector-time:  T_diag=%g T_mag=%g T_Hdu=%g T_Hds=%g T_Hu=%g T_Ho=%g\n",T_diag, T_mag, T_Hdu, T_Hds, T_Hu, T_Ho), fflush(stdout);
#endif
}

void Hv_stored_10_sp3d5s_HzbComplex( cvectr y, cvectr yc, cvectr x, qd_struct d)
{
   const int Nb_2_20= 10;
   int atom, indx, i, j, k, thislength;
   bool has_periodicity = d->geo.hasPeriodicity();
...
/* Computes Y=(H-Is)X where H is the Hamiltonian, s is input shift, and X is 
   input matrix.  In this function X is a submatrix starting at isx and ending
   at iex.  isx and iex are atomic (not actual) indices.  On output Y contains
   the [isx:isy] subvector.  The rest of Y is contained in YC. 
   It is arranged as follows YC[i*d-nb] = Y[ycmap[i][0]*d->NBasisStates]. */
int Hmatmult_spds_col_stored ( cmatrix Y, cmatrix YC, qd_struct d, real shift, 
				               cmatrix X, int num_col, int proc )
{
   ...
   #ifdef INTEL_MODIFIED
      #pragma message ("INTEL_MODIFIED: Hmult memset opt`s is ON")
	  for (j=0; j < num_col; j++)
         memset(Y[j], 0,d->seg_ln[proc]*sizeof(nml_dcscalar));
   #else 
      for ( i=0; i < d->seg_ln[proc]; i++ ) {
	     for ( j=0; j < num_col; j++)
            Y[j][i].r = Y[j][i].i = 0.0;
   ...
   switch (d->BandModel){
      case BM_20_sp3d5ss_spin:
         if(d->HzbComplex.isInitialized()) {
		    for(int j=0; j < num_col; j++)
               Hv_stored_20_HzbComplex(Y[j], YC[j], X[j], d); 
			break;
         }
         else {
		    for(int j=0; j < num_col; j++)
               Hv_stored_20_Hzb(Y[j], YC[j], X[j], d); 
      ...
      case BM_10_sp3ss_spin:
         if(d->HzbComplex.isInitialized()) {
		    for(int j=0; j < num_col; j++)
               Hv_stored_10_HzbComplex(Y[j], YC[j], X[j], d); break;
         }
         else {
		    for(int j=0; j < num_col; j++)
               Hv_stored_10_Hzb(Y[j], YC[j], X[j], d); break;
         }

      case BM_10_sp3d5ss_nospin:
         if(d->HzbComplex.isInitialized()) {
		    for(int j=0; j < num_col; j++)
               Hv_stored_10_sp3d5s_HzbComplex(Y[j], YC[j], X[j], d); break;
         }
         else {
		    for(int j=0; j < num_col; j++)
               Hv_stored_10_sp3d5s_Hzb(Y[j], YC[j], X[j], d); break;
         }
