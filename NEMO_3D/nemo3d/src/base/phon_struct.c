...
void phon_struct::ph_s_alloc(
    int N_uc,       // number of unit cells
...
// Allocates memory for phonon-related matrixes
{
  ...
  this->nbr_uc =
      nbr_uc;             /*Number of neighbor unit cells in x direction, whose
                            atoms interact with the boundary atoms of the given unit cell*/
  int states_in_row = 51; //=51 Redefine for Coulomb interaction!!!
  this->states_in_row =
      states_in_row; /*Maximum value on nonzero elements per row in Keating's
                       model. Change later to defined in the code*/
  this->DMmap =
      I3matrix(N_uc, N_a, 3); // Map of indexies in the dynamical matrix

  this->seg_col_s =
      Ivectr(nproc); // Array of the initial indexies of the DM on the processor
  this->seg_col_ln = Ivectr(
      nproc); // Array of the number of elements of the DM on the processor
  ...
  this->N_cols =
      Ivectr(nproc); /*Number of columns of DM stored on every processor*/
  this->N_rows =
      Ivectr(nproc); /*Number of rows of DM stored on every processor*/
  this->mpi_left =
      Ivectr(nproc); /*Left processors mpi_n3d_id  for communication*/
  this->mpi_right =
      Ivectr(nproc); /*Right processors mpi_n3d_id  for communication*/

  this->E_ph = Rmatrix(
      Nq_ph, N_branches + 3); /*[Nq_ph][N_branches+3] matrix of phonon energy
                               dispersion branches.
...
// Allocates memory for parts of the eigenvector that participate in
// communication
{
  this->wL = Cvectr(szCommL[mpi_n3d_id]); // part of the vector to be transfered
                                          // to Left processor
  this->wR = Cvectr(szCommR[mpi_n3d_id]); // part of the vector to be transfered
                                          // to Right processor
  this->vL =
      Cvectr(szCommR[mpi_left[mpi_n3d_id]]); // part of the vector to be
                                             // received from Left processor
  this->vR =
      Cvectr(szCommL[mpi_right[mpi_n3d_id]]); // part of the vector to be
                                              // received from Right processor
}

void phon_struct::comm_vectors_deallocate()
// Deallocates memory for parts of the eigenvector that participate in
// communication
{
  if (this->wL) {
    rm_cvectr(&this->wL);
...
// Allocate the Dynamical matrix and buffers for exchange with neighbor
// processors
{
  this->DM = Cmatrix(N_rows, states_in_row); // dynamical matrix
  this->Ind_DM =
      Imatrix(N_rows, states_in_row + 1); /*indexies of nonzero elements
                                                    in m-th row of DM.
...
// Refresh the Dynamical matrix and buffers for exchange with neighbor
// processors
{
  if (this->DM) {
    rm_cmatrix(&this->DM);
...
// Prints out the Dynamical matrix stored on the processor
{
  ...
  for (int i = 0; i < N_rows[mpi_n3d_id]; i++) {
    for (int j = 0; j < Ind_DM[i][0]; j++) {
      printf("mpi_n3d_id=%d  i=%d <=> row=i+min_row=%d; j=%d <=> Ind_DM=%d <=> "
             "col=Ind_DM+seg_col_s=%d : %g+i%g\n",
             mpi_n3d_id, i, (i + min_row[mpi_n3d_id]), j, Ind_DM[i][j + 1],
             (Ind_DM[i][j + 1] + seg_col_s[mpi_n3d_id]), DM[i][j].r,
             DM[i][j].i);
      fflush(stdout);
    }
  }
...
// Prints all the parameters on all processors
{
  ...
  fflush(stdout);
  printf("mpi_n3d_id=%d : N_cols=%d\n     Index of the initial column stored "
         "on the proc seg_col_s=%d\n"
         "                                            # of nonzero elements "
         "per row seg_col_ln=%d\n",
         mpi_n3d_id, N_cols[mpi_n3d_id], seg_col_s[mpi_n3d_id],
         seg_col_ln[mpi_n3d_id]);
  fflush(stdout);
  printf("mpi_n3d_id=%d : N_rows=%d\n     Index of the initial row stored on "
         "the proc min_row=%d\n"
         "                               Index of the last row stored on the "
         "proc max_row=%d\n",
         mpi_n3d_id, N_rows[mpi_n3d_id], min_row[mpi_n3d_id],
         max_row[mpi_n3d_id]);
  fflush(stdout);
  printf("mpi_n3d_id=%d : Communicates with mpi_left=%d and mpi_right=%d "
         "processors\n"
         "            It sends them szCommL=%d and szCommR=%d elements of the "
         "eigenvector\n",
         mpi_n3d_id, mpi_left[mpi_n3d_id], mpi_right[mpi_n3d_id],
         szCommL[mpi_n3d_id], szCommR[mpi_n3d_id]);
  fflush(stdout);
}
  ...
  pre... by <row,col> in the whole DM*/
{
  ...
  for (int col = 0; col < seg_col_ln[mpi_n3d_id]; col++) {
    if ((col + seg_col_s[mpi_n3d_id]) % 10) {
      if (((col + seg_col_s[mpi_n3d_id]) % 10) != 5 &&
          ((col + seg_col_s[mpi_n3d_id]) % 10) != 3)
        DM_ostr << " ";
      else if (((col + seg_col_s[mpi_n3d_id]) % 10) == 3 &&
               (col + seg_col_s[mpi_n3d_id]) <= 100)
        DM_ostr << " ";
  ...
  for (int row = 0; row < N_rows[mpi_n3d_id]; row++) {
    if (row < (max_row[mpi_n3d_id] - min_row[mpi_n3d_id] + 1)) {
      ...
      DM_ostr << "\n"
              << setw(3) << DM_total_size - (N_rows[mpi_n3d_id] - row) << " ";
