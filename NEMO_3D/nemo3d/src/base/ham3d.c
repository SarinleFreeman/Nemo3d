...
static bool isNotInBox(real* posAtom, real* dimensions) 
{
   ...
   if ( *posAtom < *rMin || *posAtom > *rMax )  return true;
   posAtom++;  rMin++;  rMax++;
   if ( *posAtom < *rMin || *posAtom > *rMax )  return true;
   posAtom++;  rMin++;  rMax++;
   if ( *posAtom < *rMin || *posAtom > *rMax )  return true;
...
static bool isNotInTiltedBox(real* posAtom, real* dimensions, int **delta_step, int shape_rank)
{
   real *rMin=dimensions;
   real *rMax=dimensions+3;
   double delta=1e-6;
   double ymin, ymax;
   ymin = (((posAtom[0]-dimensions[0])/dimensions[12])*dimensions[13])*(dimensions[7]/dimensions[6]) + dimensions[1] + delta;
          /* exclude all atoms on lower boundary */
   ymax = ymin + dimensions[9];
          /* include all atoms on upper boundary */

   double zmin,zmax;
   zmin = (((posAtom[0]-dimensions[0])/dimensions[12])*dimensions[14])*(dimensions[8]/dimensions[6]) + dimensions[2] + delta;
          /* exclude all atoms on lower boundary */
   ...
   if(shape_rank==2) { /* If this line is uncommented roughness is introduced in Si QW only */
      ...
      int delta_indx=int(posAtom[1]/(dimensions[13]/4)); /* row index in delta_step */
      double Delta;
      ...
      if(posAtom[0]>(double(stepNo2))*2*stepLen && posAtom[0]<(double(stepNo2)+1)*2*stepLen) {
         if(Delta>0 && stepNo==(2*stepNo2+1)) {
            if(posAtom[0]>(double(stepNo))*stepLen && posAtom[0]<(double(stepNo))*stepLen+Delta) {
               zmin = (stepNo)*dimensions[14]/4 + dimensions[2] - delta;
              }
           }
         if(Delta<0 && stepNo==2*stepNo2) {
            if(posAtom[0]>(stepNo+1)*stepLen+Delta && posAtom[0]<(stepNo+1)*stepLen) {
   ...
   if(posAtom[0]<dimensions[0] || posAtom[0]>dimensions[3]) { return true; }

   if(posAtom[1]<ymin) { return true; }
   if(posAtom[1]>ymax) { return true; }

   if(posAtom[2]<zmin) { return true; }
   if(posAtom[2]>zmax) { return true; }
...
*/  
   if ( transform_pos[0] < rMin[0] || transform_pos[0] > rMax[0] )  return true;
   if ( transform_pos[1] < rMin[1] || transform_pos[1] > rMax[1] )  return true;
   if ( transform_pos[2] < rMin[2] || transform_pos[2] > rMax[2] )  return true;
   return false;
}
...
static bool isNotInCylinder_x(real* posAtom, real* dimensions)
{
   ...
   if ( posAtom[0] < rMin[0] || posAtom[0] > rMax[0] )  return true;

   real sum = 0.0;
   sum += pow( (posAtom[2]- rCenter[0]), 2.0 );
   sum += pow( (posAtom[1]- rCenter[1]), 2.0 );

   if(double(sum-Radius2)>-1e-6) return true;
   return false;
}
...
static bool isNotInCylinder(real* posAtom, real* dimensions) 
{
   ...
   if ( posAtom[2] < rMin[2] || posAtom[2] > rMax[2] )  return true;

   real sum = 0.0;
   sum += pow( (posAtom[0]- rCenter[0]), 2.0 );
   sum += pow( (posAtom[1]- rCenter[1]), 2.0 );
   if (sum > Radius2) return true;
...
static bool isNotInPyramid(real* posAtom, real* dimensions) 
{
   ...
   if ( posAtom[2] < rMin[2] ) return true;
   if ( posAtom[2] > rMin[2] + dzdx*(posAtom[0] - rMin[0]) ) return true;
   if ( posAtom[2] > rMin[2] - dzdx*(posAtom[0] - rMax[0]) ) return true;
   if ( posAtom[2] > rMin[2] + dzdy*(posAtom[1] - rMin[1]) ) return true;
   if ( posAtom[2] > rMin[2] - dzdy*(posAtom[1] - rMax[1]) ) return true;
...
static bool isNotInDome(real* posAtom, real* dimensions) 
{
   ...
   if( posAtom[0] < rMin[0] || posAtom[0] > rMax[0] ) return true;
   if( posAtom[1] < rMin[1] || posAtom[1] > rMax[1] ) return true;
   if( posAtom[2] < rMin[2] || posAtom[2] > rMax[2] ) return true;

   real sum = 0.0;
   sum += pow( (*posAtom) - rCenter[0], 2.0) / Radius_Square[0];  posAtom++; 
   sum += pow( (*posAtom) - rCenter[1], 2.0) / Radius_Square[1];  posAtom++; 
...
static void PrintShapeInfo(const Shape_struct& shape,
                           const MaterialHandle& mh,
                           int* species_count,
                           real* rMax, real* rMin, int* cMin, int* cMax) 
{
   ...
   case Shape_struct::TiltedBox:
      shapeHdr="[TILTEDBOX] -------------------------";  break;
   case Shape_struct::Cylinder:  
      shapeHdr="[Cylinder] -------------------------";  break;
   case Shape_struct::Cylinder_x:  
      shapeHdr="[Cylinder_x] -------------------------";  break;
   case Shape_struct::Pyramid:
      shapeHdr="[PYRAMID] -------------------------";  break;
   case Shape_struct::Dome:
      shapeHdr="[DOME] -------------------------";  break;
   case Shape_struct::Ellipsoid:
      shapeHdr="[Ellipsoid] -------------------------";  break;
   case Shape_struct::Rhombohedron:
      shapeHdr="[RHOMBOHEDRON] -------------------------";  break;
   default:
      shapeHdr="   UNKNOWN SHAPE  "; break;
   }

   int NatomsInShape=0;
   for (int i=0;i<EL_TOTAL;i++){
      if (species_count[i]==0) continue;
      NatomsInShape += species_count[i];
   }

   cout << "\n   " << shapeHdr << endl;
   cout << "      material:  " << mh.cation << mh.anion << endl;
   if(shape.has_cell_granularity) cout << "      will have cell granularity "<<endl;
   else  cout << "      will not have cell granularity "<<endl;
   cout << "      will use this shape for strain "
        << (shape.use_for_strain_and_electronic 
            ? "and electronic calculation"
            : "calculation only") << endl;
   cout << "      requested dimensions:  " 
        << shape.x << "   "
        << shape.y << "   "
        << shape.z << "   " << endl;

   if (NatomsInShape==0) {
      cout << "      !!! THIS SHAPE AND FIRST SHAPE HAVE NO ATOMS IN COMMON !!!" << endl;
   } else {
      cout << "      adjusted dimensions:  " 
           << rMax[0] - rMin[0] << "   " 
           << rMax[1] - rMin[1] << "   " 
           << rMax[2] - rMin[2] << "   " << endl;
   ...
   if(shape.Shape==Shape_struct::Rhombohedron){
     cout << "      important note: the dimensions of rhombohedron are defined as "<<endl;  
     cout << "                      the lengths along the wurtzite primitive vectors "<<endl;  
...
{ 
   ...
   for (int i=0;i<EL_TOTAL;i++)  species_count[i]=0;
...
//      isNotInShape = isNotInTiltedBox;  
      ...
      /* Adjust origin and dimensions of the shape and make them integer multiples of ax, ay, az */
 ...
 //     double tiltedUnitCellX  = this->dx*this->lattice_x;
      shDim[11] = ((int)((shDim[3]-shDim[0])/(this->dx*this->lattice_x)))*(this->dx*this->lattice_x); /* Adjusted x-dimension. It is made commensurate with tilt direction */
      ...
      for (int Za=0; Za < NatomsPerCell; Za++) {
         ...
         if (this->is_tilted && shape.Shape==Shape_struct::TiltedBox) {
           if ( isNotInTiltedBox(pos, shDim, this->delta_step, shape_rank) )  {
             if (has_cell_granularity) invalidate_this_cell=true;
             continue;
            }
          } /* if is_tilted */
         else { // !is_tilted
         ...
         if(shape.Shape == Shape_struct::Rhombohedron) {
           ...
           if (rMin[0] > transform_pos[0])  rMin[0]=transform_pos[0];
           if (rMax[0] < transform_pos[0])  rMax[0]=transform_pos[0];
           if (rMin[1] > transform_pos[1])  rMin[1]=transform_pos[1];
           if (rMax[1] < transform_pos[1])  rMax[1]=transform_pos[1];
           if (rMin[2] > transform_pos[2])  rMin[2]=transform_pos[2];
           if (rMax[2] < transform_pos[2])  rMax[2]=transform_pos[2];
         }
         else {
           if (rMin[0] > pos[0])  rMin[0]=pos[0];
           if (rMax[0] < pos[0])  rMax[0]=pos[0];
           if (rMin[1] > pos[1])  rMin[1]=pos[1];
           if (rMax[1] < pos[1])  rMax[1]=pos[1];
           if (rMin[2] > pos[2])  rMin[2]=pos[2];
           if (rMax[2] < pos[2])  rMax[2]=pos[2];
         }
         if (cMin[0] > this->cell__ijk[Zc][0])  cMin[0]=this->cell__ijk[Zc][0];
         if (cMax[0] < this->cell__ijk[Zc][0])  cMax[0]=this->cell__ijk[Zc][0];
         if (cMin[1] > this->cell__ijk[Zc][1])  cMin[1]=this->cell__ijk[Zc][1];
         if (cMax[1] < this->cell__ijk[Zc][1])  cMax[1]=this->cell__ijk[Zc][1];
         if (cMin[2] > this->cell__ijk[Zc][2])  cMin[2]=this->cell__ijk[Zc][2];
         if (cMax[2] < this->cell__ijk[Zc][2])  cMax[2]=this->cell__ijk[Zc][2];
         
         inSet[Zc][Za] = 1;
         NatomsInCell++;
      }
      if (invalidate_this_cell && !Shape_struct::Cylinder_x )
         for (int Za=0; Za < NatomsPerCell; Za++) inSet[Zc][Za]=0;
      else
   ...
   if (shape.AlloyFixConcentration) {
      if (shape.AlloyTreatment!=Shape_struct::RandomInUnitCell ||
          isNotRandomAlloy)
         die("Cannot only use fixed alloy concentration with a random alloy and atomic granularity");
...
//      reset_random_seed_with_time();
      for (int Zc=0; Zc < this->N_Cell; Zc++) {
         ...
         for (Za=0; Za < NatomsPerCell/2; Za++) {
            if ( !inSet[Zc][Za] ) continue;

            if (shape.AlloyTreatment==Shape_struct::RandomInUnitCell)
               rnd = real(rand()) / real(RAND_MAX);

#ifdef   MEMORY_SAVE_NEWSTRUCT  
            this->AtomType[Zc][Za] = (unsigned char) cation[rnd <= prob_cation ? 0 : 1];
...
#ifdef   MEMORY_SAVE_NEWSTRUCT 
            this->AtomType[Zc][Za] = (unsigned char) anion[rnd <= prob_anion ? 0 : 1];
            this->atom_strain_only[Zc][Za] = (unsigned char) useOnlyForStrain;
            species_count[ (int) this->AtomType[Zc][Za] ]++;
#else   /* MEMORY_SAVE_NEWSTRUCT  */
...
#ifdef MEMCHECK
{
   ...
   int3_memory = this->N_Cell*NatomsPerCell *4  / 1.0e6;
   tl_memory -= int3_memory;
   printf("\n FilleShape memory deallocationallocation in MB int3_memory=%g tl_memory=%g\n",int3_memory, tl_memory),fflush(stdout);
}
#endif
...
void geo_struct::FillFirstShape(const Shape_struct& shape, const top_struct& opt,
                                const vector<MaterialHandle>& sMatList)
{
   int Zcell=0;
   ...
   if ( opt.Dev.CrystalStruct == Dev_struct::Diamond_FCC ||
      ...
      for (int i=1; i < Nx-1; i++ ){
         ...
         for (int j=1; j < Ny-1; j++ ) {
            ...
            for (int k=1; k < Nz-1; k++ ) {
               ...
               if ( i >= Zx0 && i < Zxf &&
                    j >= Zy0 && j < Zyf &&
                    k >= Zz0 && k < Zzf && k%2 == evenodd ) 
                  {
                     this->cell__ijk[Zcell][0] = i;
                     this->cell__ijk[Zcell][1] = j;
                     this->cell__ijk[Zcell][2] = k;
                     if ( i < this->cell_xmin ) this->cell_xmin = i;
                     if ( i > this->cell_xmax ) this->cell_xmax = i;
                     if ( j < this->cell_ymin ) this->cell_ymin = j;
                     if ( j > this->cell_ymax ) this->cell_ymax = j;
                     if ( k < this->cell_zmin ) this->cell_zmin = k;
                     if ( k > this->cell_zmax ) this->cell_zmax = k;
      ...
      if (mpi_n3d_id==mpi_n3d_masterid) {
         cout << "   [BOX (domain)] --------------------" << endl
              << bullet << "requested dimensions:  " 
              << shape.x << "   "
              << shape.y << "   "
              << shape.z << "   " << endl
              << bullet << "adjusted dimensions:  " 
              << (Zxf-Zx0) * this->lattice_x << "   " 
