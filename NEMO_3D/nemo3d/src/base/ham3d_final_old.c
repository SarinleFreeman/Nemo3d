...
if(d->opt.ExecParam.Strain.StrainOut.TensionOutput){
 ...
 if (d->opt.ExecParam.Phys.ImpNumber > 0) {
      ...
      int Zcell[numImpurities];
      int Zatom[numImpurities];
      real dist_min=999999;
      float xyz_desired[numImpurities][3];
      bool flag=false;
      int imp=0;
      int k=0;
      int i,j;
      ...
      for(i=0;i<numImpurities;i++){
        for (j=0;j<3;j++){
            Zcell[i]=-1;
            Zatom[i]=-1;
            xyz_desired[i][j] = d->opt.ExecParam.Phys.ImpsXYZ[k];
            k++;
        }
      }
      for(int i=0; i<numImpurities;i++){
        if( (xyz_desired[i][0]<d->opt.Dev.ShapeList[0].xorigin) ||
            (xyz_desired[i][0]>(d->opt.Dev.ShapeList[0].xorigin+d->opt.Dev.ShapeList[0].x)) ||
            (xyz_desired[i][1]<d->opt.Dev.ShapeList[0].yorigin) ||
            (xyz_desired[i][1]>(d->opt.Dev.ShapeList[0].yorigin+d->opt.Dev.ShapeList[0].y)) ||
            (xyz_desired[i][2]<d->opt.Dev.ShapeList[0].zorigin) ||
            (xyz_desired[i][2]>(d->opt.Dev.ShapeList[0].zorigin+d->opt.Dev.ShapeList[0].z)) )        {
                flag=true;
                imp=i+1;
                break;
      ...
      real pos[3], pos0[numImpurities][3];
      ...
      for(int i=0;i<numImpurities;i++){
        ...
        for (int l=0; l < d->geo.N_Cell; l++ ) {
          for (int m=0; m < d->geo.AtomsPerCellMax(); m++ ) {
            ...
            real R2 = (pos[0]-xyz_desired[i][0])*(pos[0]-xyz_desired[i][0]) +
            (pos[1]-xyz_desired[i][1])*(pos[1]-xyz_desired[i][1]) +
            (pos[2]-xyz_desired[i][2])*(pos[2]-xyz_desired[i][2]);
      ...
      d->geo.getPosition_strained(pos0[i], Zcell[i], Zatom[i]);
  ...
  if (d->opt.ExecParam.Phys.imp_model==2)
     d->geo.AtomType[Zcell[i]][Zatom[i]] = EL_P;
      ...
      cout_master << "Placing a donor at r = ("
                  << pos0[i][0] << ","
                  << pos0[i][1] << ","
                  << pos0[i][2] << ") with U0=" << U0 << " eV ... " << endl;

      } //end of impurity loop
       real R[numImpurities];
       real V_nuc=0.0;
       if(d->opt.ExecParam.Phys.include_nuclear_term && (numImpurities>1) ){
                real dd=(pos0[0][0]-pos0[1][0])*(pos0[0][0]-pos0[1][0]) +
                        (pos0[0][1]-pos0[1][1])*(pos0[0][1]-pos0[1][1]) +
                        (pos0[0][2]-pos0[1][2])*(pos0[0][2]-pos0[1][2]);
      ...
      for (int m=0; m < d->geo.AtomsPerCellMax(); m++ ) {
        ...
        for(int i=0;i<numImpurities;i++){
         ...
         d->geo.getPosition_strained(pos, l, m);
         R[i]=sqrt((pos[0]-pos0[i][0])*(pos[0]-pos0[i][0]) +
                     (pos[1]-pos0[i][1])*(pos[1]-pos0[i][1]) +
                     (pos[2]-pos0[i][2])*(pos[2]-pos0[i][2]));
