...
*****************************************************************************
$Header: /repo/nemo3d/src/base/HamZincBlendeComplex.c,v 1.13 2006/11/30 22:08:30 nkharche Exp $ 
*****************************************************************************/
...
void HamZincBlendeComplex::Initialize(BM_Type BandModel, int N_Basis, int Natom, int Natom_surf,
                               int Nout, int Nin, 
bool MagneticFieldOn, /* Magnetic field added to HamZincBlendeComplex class */
complex HBxy) 
{
   nml_memory_report("Before Allocation of Hamiltonian in Hzb.Initialize - HamZincBlendeComplex::Initialize");
   this->N_Basis = N_Basis;
   this->BandModel = BandModel;
   this->Natom = Natom;
   this->Natom_surf = Natom_surf;
   this->MagneticFieldOn = MagneticFieldOn; /* Magnetic field added to HamZincBlendeComplex class */
   this->HBxy = HBxy;
   double memory_estimate=0;

   switch (BandModel){
   case BM_20_sp3d5ss_spin:
      memory_estimate = (Natom * N_Basis + Natom_surf*5) * sizeof(ham_mem_real) +  
   ...
   case BM_10_sp3d5ss_nospin:
      memory_estimate = (Natom * N_Basis + Natom_surf*6) * sizeof(ham_mem_real) +  
      ...
      break;
   case BM_1_s_nospin:
      die("BM_1_s_nospin is not yet implemented HamZincBlendeComplex::Initialize"); break;
   default:
      die("Unsupported bandstructure model specified.  Choose one from Bands_1_s_nospin, Bands_10_sp3ss_spin, Bands_20_sp3d5ss_spin, Bands_10_sp3d5ss_nospin\n");
      break;
   }
   ...
   nml_memory_report("After Allocation of Hamiltonian in Hzb.Initialize - HamZincBlendeComplex::Initialize");
                            ...
                            int Zsurf) 
{
   ...
   case BM_1_s_nospin:
      die("BM_1_s_nospin is not yet implemented HamZincBlendeComplex::setDiag"); break;
   default:
      die("Unsupported bandstructure model specified.  Choose one from Bands_1_s_nospin, Bands_10_sp3ss_spin, Bands_20_sp3d5ss_spin, Bands_10_sp3d5ss_nospin\n");
      break;
   }
...
void HamZincBlendeComplex::printStorageInfo() 
{
   ...
   case BM_10_sp3d5ss_nospin:
      ...
      break;
   case BM_1_s_nospin:
      die("Band_1_s_nospin is not yet implemented HamZincBlendeComplex::printStorageInfo"); break;
   default:
      die("Unsupported bandstructure model specified.  Choose one from Bands_1_s_nospin, Bands_10_sp3ss_spin, Bands_20_sp3d5ss_spin, Bands_10_sp3d5ss_nospin\n");
      break;
   }
...
void HamZincBlendeComplex::print_20_sp3d5ss_spin()
{
   ...
   for (k=0; k<nml_iv_extent(this->indxHU_to_row); k++) {
	 ...
	 fprintf(fp, "%d %d %20.12e %20.12e\n", I, J, this->Hu[indx].r, this->Hu[indx].i);
	 fprintf(fp, "%d %d %20.12e %20.12e\n", J, I, this->Hu[indx].r, this->Hu[indx].i);
	 fprintf(fp, "%d %d %20.12e %20.12e\n", I+Nb_2, J+Nb_2, this->Hu[indx].r, this->Hu[indx].i);
	 fprintf(fp, "%d %d %20.12e %20.12e\n", J+Nb_2, I+Nb_2, this->Hu[indx].r, this->Hu[indx].i);
      }}
   }
   fclose(fp);
}
