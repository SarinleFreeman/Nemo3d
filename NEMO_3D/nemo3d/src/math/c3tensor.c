...
c3tensor C3tensor(int dimRowLow, int dimRowHigh, int dimColLow, int dimColHigh, int dimDepthLow, int dimDepthHigh) {
    ...
    c3t = (complex ***) nml_malloc((size_t)((dimRow+NR_END+6)*sizeof(complex**)));
    if (!c3t) fprintf(LOGFILE,"In C3tensor():: Unable to allocate memory for dimRow+NR_END+6.\n");
    ...
    c3t[dimRowLow] = (complex **) nml_malloc((size_t)((dimRow*dimCol+NR_END)*sizeof(complex*)));
    if (!c3t[dimRowLow]) fprintf(LOGFILE,"In C3tensor():: Unable to allocate memory for dimRow*dimCol+NR_END.\n");
    c3t[dimRowLow] += NR_END;
    c3t[dimRowLow] -= dimColLow;


    c3t[dimRowLow][dimColLow] = (complex *) nml_malloc((size_t)((dimRow*dimCol*dimDepth+NR_END) * sizeof(complex)));
    if (!c3t[dimRowLow][dimColLow]) fprintf(LOGFILE,"In C3tensor():: Unable to allocate memory for dimRow*dimCol*dimDepth+NR_END.\n");
 ...
 * Note that the dimension size of the row's lower bound is needed as an argument to the destructor. 
 */
void rm_c3tensor(c3tensor *ptrC3t, int dimRowLow) {
    ...
    if (c3t != NULL) {
        ...
        if (d_1l(c3t) != dimRowLow) {
            n3d_warning("In rm_c3tensor():: Incorrect match of 1st dimension's lower bound %d, with the passed in value of %d.\n", dimRowLow, d_1l(c3t));
        }
 ...
 * If c3t does not exists, it creates a new one. But if c3t does exist, a check on the
 * dimension is performed. If the dimensions are valid, the c3tensor is returned. Otherwise,
 * it removes the invalid c3tensor and creates a new one with the correct dimensions.
 */
c3tensor check_c3tensor(c3tensor c3t, int d1l, int d1h, int d2l, int d2h, int d3l, int d3h) {
    ...
    if (!c3t) {
        ...
        if (d_1l(c3t) != d1l) {
            n3d_warning("In check_c3tensor():: Incorrect match of 1st dimension's lower bound %d, with the passed in value of %d.\n", d1l, d_1l(c3t));
        }
        ...
        /* Else, incorrect dimensions. So remove the old c3tensor and create a new one. */
        else {
            c3t -= d1l;
            rm_c3tensor(&c3t,d1l);
