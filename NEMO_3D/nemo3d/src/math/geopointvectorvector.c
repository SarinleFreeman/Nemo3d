...
*****************************************************************************
$Header: /repo/nemo3d/src/math/geopointvectorvector.c,v 1.3 2004/10/27 19:43:10
gekco Exp $
*****************************************************************************/
...
geopointvectrvectr Geopointvectrvectr(int n) {
  ...
  geopointvectrvectr I =
      (geopointvectr *)nml_calloc((n + 1), sizeof(geopointvectr));
  if (I == NULL) {
...
int add_elem_geopointvectrvectr(geopointvectr data_vectr, int pos,
...
geopoint min_gp_geopointvectrvectr(geopointvectrvectr gpvv) {
  ...
  for (i = 0; i < vdim(gpvv); i++) {
    for (j = 0; j < vdim(gpvv[i]); j++) {
      if (gpvv[i][j].x < gp.x && gpvv[i][j].x < gp.x && gpvv[i][j].x < gp.x) {
        gp = gpvv[i][j];
      }
    }
  }
...
geopoint max_gp_geopointvectrvectr(geopointvectrvectr gpvv) {
  ...
  for (i = 0; i < vdim(gpvv); i++) {
    for (j = 0; j < vdim(gpvv[i]); j++) {
      if (gpvv[i][j].x > gp.x && gpvv[i][j].y > gp.y && gpvv[i][j].z > gp.z) {
        gp = gpvv[i][j];
      }
    }
  }
