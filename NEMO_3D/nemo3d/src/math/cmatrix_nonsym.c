...
*****************************************************************************
$Header: /repo/nemo3d/src/math/cmatrix_nonsym.c,v 1.9 2005/06/02 18:46:26 marek
Exp $
*****************************************************************************/
...
int complexCompare(const void *complexA, const void *complexB) {
  if (((complex *)complexA)->r < ((complex *)complexB)->r)
    return -1;
  else if (((complex *)complexA)->r > ((complex *)complexB)->r)
    return 1;
  else
    return 0;
}
...
void ceigval_ns_full(cvectr cvEigenValue, cmatrix h) {
  ...
  if (info == 0) {
    /*
    printf("In ceigval_ns_full():: Hessenberg Elimination -> Successful exit
    from LAPACK's zgehrd(). Optimal LWORK is %f.\n", work[0].r );
    */
  } else if (info < 0) {
    n3d_warning("In ceigval_ns_full():: Hessenberg Elimination -> From call to "
                "LAPACK's zgehrd(), the %d-th argument had an illegal value.\n",
                abs(info));
  }
   ...
   */
  zhseqr(&job, &compz, &n, &ilo, &ihi, &h[0][0], &ldh, cvEigenValue, &z[0][0],
         &ldz, work, &lwork, &info);

  if (info == 0) {
    /*
    printf("In ceigval_ns_full():: QR Algorithm -> Successful exit from LAPACK's
    zhseqr(). Optimal LWORK is %f.\n", work[0].r );
    */
  } else if (info < 0) {
    n3d_warning("In ceigval_ns_full():: QR Algorithm -> From call to LAPACK's "
                "zhseqr(), the %d-th argument had an illegal value.\n",
                abs(info));
  } else if (info > 0) {
    n3d_warning("In ceigval_ns_full():: QR Algorithm -> From call to LAPACK's "
                "zhseqr(), ZHSEQR failed to compute all the eigenvalues in a "
                "total of 30*(IHI-ILO+1) iterations.\n");
  }
...
void ceigvec_ns_full(cvectr cvEigenValue, cmatrix cmEigenVector, cmatrix h) {
  ...
  work = Cvectr(lwork);
  zheev(&jobz, &uplo, &n, &cmEigenVector[0][0], &lda, w, work, &lwork, rwork,
        &info);
  if (info == 0) {
    /*
    printf("In ceigvec_ns_full():: Successful exit from LAPACK's zheev().
    Optimal LWORK is %f.\n", work[0].r );
    */
  } else if (info < 0) {
    n3d_warning("In ceigvec_ns_full():: From call to LAPACK's zheev(), the "
                "%d-th argument had an illegal value.\n",
                abs(info));
  } else if (info > 0) {
    n3d_warning("In ceigvec_ns_full():: From call to LAPACK's zheev(), the "
                "algorithm failed to converge, %d off-diagonal elements of an "
                "intermediate tridiagonal form did not converge to zero.\n",
                info);
  }
  rm_cvectr(&work);

  /* Copy the real eigenvalues set in w[] to the real part of the cvector
   * cvEigenValue. */
  for (i = 0; i < n; i++) {
