...
/*
 * Solves the system "y = Ax" for cvector y. It multiplies cmatrix A with cvector y.
 * Returns the resultant cvector y.
 */
...
cmatrix cp_cmatrix(cmatrix cmTarget, cmatrix cmSource) {
    ...
    /* If no cmatrix cmTarget exists, create one with the same dimension as cmSource. */
    if (!cmTarget) {
