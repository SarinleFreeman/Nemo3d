 ...
 * If cvTarget is NULL, a new Cvector is allocated with the same dimension as
 * cvSource. If cvTarget exists and is different dimension from cvSource, memory
 * for cvTarget is deallocated, and a new Cvector is allocated with the same
 * dimensions as cvSource.
 */
cvectr cp_cvectr_dim(cvectr cvTarget, cvectr cvSource, int arraySize) {
  ...
  if (!cvSource) {
    n3d_warning(
        "In cp_cvectr_dim():: Error - Source cvector 'cvSource' is NULL.\n");
    return cvSource;
  }
  ...
  if (nml_dcv_extent(cvSource) < arraySize) {
    n3d_warning("In cp_cvectr_dim():: specified arraySize to copy is larger "
                "than dimension of source cvector 'cvSource'.\n");
...
void cp_carray2carray(complex *caSource, complex *caTarget, int arraySize) {
  ...
  if (!caSource) {
    n3d_warning("In cp_carray2carray():: Error - Source complex array "
                "'caSource' is NULL.\n");
    return;
  }

  if (!caTarget) {
    n3d_warning("In cp_carray2carray():: Error - Target complex array "
                "'caTarget' is NULL.\n");
    return;
  }
