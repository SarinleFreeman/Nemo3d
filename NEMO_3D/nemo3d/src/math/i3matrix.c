...
/*T_HEADER_FILE{i3matrix.c}
  ...
  in the FORTRAN or F90 code by the usual matrix addressing scheme.  However the ordering of 
  the indices is inverted from the usual C notation. 
  Within the C code the starting and ending point of each dimension
  is stored and retrieved with macros like \verb|vdim1| REFERENCE{vdim1,i3matrix.h}. T*/
...
i3matrix I3matrix(int nrow, int ncol, int ndep)
{
  ...
  if (!t) fprintf(LOGFILE,"I3matrix unable to allocate memory on stage 1.\n");
  t += 3;
  vdim1(t) = nrow;
  vdim2(t) = ncol;
  vdim3(t) = ndep;
  ...
  if (!t[0]) fprintf(LOGFILE,"I3matrix unable to allocate memory on stage 2.\n");
  

  t[0][0] = (int *) nml_malloc((size_t)((nrow*ncol*ndep) * sizeof(int)));
  if (!t[0][0]) fprintf(LOGFILE,"I3matrix unable to allocate memory on stage 3.\n");
...
/*T_HEADER_SHORT{rm_i3matrix}
  ...
  The lower bound of the first dimension must be given as an input for the proper destruction 
  of the object. T*/
...
/*T_HEADER_SHORT{check_i3matrix}
  ...
  Note that there is a restriction to the reshaping of the i3matrix:  The first index cannot be reshaped!
  T*/ 
