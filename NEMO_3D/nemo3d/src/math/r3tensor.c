...
r3tensor R3tensor(int dimRowLow, int dimRowHigh, int dimColLow, int dimColHigh, int dimDepthLow, int dimDepthHigh) {
    ...
    if (!r3t) fprintf(LOGFILE,"In R3tensor():: Unable to allocate memory for dimRow+NR_END+6.\n");
    ...
    r3t[dimRowLow] = (real **) nml_malloc((size_t)((dimRow*dimCol+NR_END)*sizeof(real*)));
    if (!r3t[dimRowLow]) fprintf(LOGFILE,"In R3tensor():: Unable to allocate memory for dimRow*dimCol+NR_END.\n");
    r3t[dimRowLow] += NR_END;
    r3t[dimRowLow] -= dimColLow;


    r3t[dimRowLow][dimColLow] = (real *) nml_malloc((size_t)((dimRow*dimCol*dimDepth+NR_END) * sizeof(real)));
    if (!r3t[dimRowLow][dimColLow]) fprintf(LOGFILE,"In R3tensor():: Unable to allocate memory for dimRow*dimCol*dimDepth+NR_END.\n");
...
void rm_r3tensor(r3tensor *ptrR3t, int dimRowLow) {
    ...
    if (r3t != NULL) {
        ...
        if (d_1l(r3t) != dimRowLow) {
            n3d_warning("In rm_r3tensor():: Incorrect match of 1st dimension's lower bound %d, with the passed in value of %d.\n", dimRowLow, d_1l(r3t));
        }
 ...
 * If r3t does not exists, it creates a new one. But if r3t does exist, a check on the
 * dimension is performed. If the dimensions are valid, the r3tensor is returned. Otherwise,
 * it removes the invalid r3tensor and creates a new one with the correct dimensions.
 */
r3tensor check_r3tensor(r3tensor r3t, int d1l, int d1h, int d2l, int d2h, int d3l, int d3h) {
    ...
    if (!r3t) {
        ...
        if (d_1l(r3t) != d1l) {
            n3d_warning("In check_r3tensor():: Incorrect match of 1st dimension's lower bound %d, with the passed in value of %d.\n", d1l, d_1l(r3t));
        }
        ...
        /* Else, incorrect dimensions. So remove the old r3tensor and create a new one. */
        else {
            r3t -= d1l;
            rm_r3tensor(&r3t,d1l);
