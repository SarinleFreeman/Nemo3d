...
*****************************************************************************
$Header: /repo/nemo3d/src/db_old/nemo_mb_models.c,v 1.1 2004/02/16 03:36:56
gekco Exp $
*****************************************************************************/
...
Boolean isIndependentSingleBand(char *bandModel) {
  ...
  if (!strcmp(bandModel, Coken_BM_Bands_1) ||
...
/*
 * Adjusts the pointer to the multiband parameter lists based on the mutliband
 * model.
 */
semiMaterial adjustMBParamPointer(semiMaterial sm) {

  if (!strcmp(sm->band_model, Coken_BM_Bands_1) ||
...
semiMaterial initMBSemiMaterial(semiMaterial sm) {
  ...
  *sm->Abort_result = setMBModelParam(
      sm->band_model, &sm->mb_param_n, &sm->mb_model, &sm->mb_bdim,
      &sm->mb_band_inc, &sm->mb_nbr, &sm->mb_abdim, &sm->mb_spin_orbit,
      &sm->mb_spin_degeneracy, &sm->mb_sigma_singular);
