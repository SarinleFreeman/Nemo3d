...
#define NEW_param_distr
    ...
    /* Chris decided to copy the parameters into the d matrices to conform to the 
       old nemo standard of not carrying the paramters around after the construction of the
       d's and s's.  
       So now we have to be careful in reusing this for bandstructure models
       whose basis is smaller than the parameter length.
       assume we have at least the size of
...
void fold_t_k_spd_s ( real kxa, real kya, cmatrix d, cmatrix s, cmatrix d_fold, cmatrix s_fold )
{
    ...
    tbh_const_1d_nemo( d_fold, s_fold, nnv, nnv0, basis, spin, param, mb_strain, unstrnd_cubic_cell_length, k ); 
...
void fold_t_k_sp3s_s_new ( real kxa, real kya, cmatrix d, cmatrix s, cmatrix d_fold, cmatrix s_fold )
{
    ...
    /* This really loops over all the orbitals that are included in the model here.	
       in mb_ham_spds_micro we define the list of available orbitals
       STa Sa Pxa Pya etc.... 
       Here we will take out only the orbitals 
       STa=0 Sa=1 Pxa=2 Pya=3 Pza=4  and
       STc=10 Sc=11 Pxc=12 Pyc=13 Pzc=14  and
       The order is as follows in the blocks:
       anion up-spin     0- 4
       anion down-spin   5- 9
       cation up-spin   10-14
       cation down spin 15-19
     */
    for ( i=0; i < 5; i++ )
    {
        spin[i] = 1;
	...
	while (i<sp3d5s_nparam){
	    ...
	    r++;
	    len = min(sp3d5s_nparam - r*sp3d5s_nparam_copy_size, sp3d5s_nparam_copy_size);
	}
	if (d[0][2].i!=0.0 || d[0][5].i!=0.0){
		...
		r++;
		len = min(nstrain - r*sp3d5s_nparam_copy_size, sp3d5s_nparam_copy_size);
	    }
	}
    }
...
void fold_t_k_spds_s ( real kxa, real kya, cmatrix d, cmatrix s, cmatrix d_fold, cmatrix s_fold )
{
    ...
    /* This really loops over all the orbitals that are included in the model here.	
       in mb_ham_spds_micro we define the list of available orbitals
       STa Sa Pxa Pya etc.... 
    Here we take all the 20 orbitals over s, p, d, and s* 
    The order is as follows in the blocks:
    anion up-spin     0- 9
    anion down-spin  10-19
    cation up-spin   20-29
    cation down spin 30-39
    */
    for ( i=0; i < 10; i++ )
    {
        spin[i] = 1;
...
real mb_newton_spds( rvectr p, rvectr param, rvectr mb_strain, real unstrnd_cubic_cell_length)
{
    ...
    {
        it++;
        E = Newton_micro( h, nnv, nnv0, basis, spin, param, mb_strain, unstrnd_cubic_cell_length, k, band, f, jac); 
	...
	printf("\niter=%i dkmax = %e  k = %e %e %e  E = %e f= %e", it, dkmax, k[0],k[1],k[2],E,fmax);
    }
    while ( dkmax > 1e-8 );
...
real mb_mstar_spds( rvectr p, rvectr param, rvectr mb_strain, real unstrnd_cubic_cell_length, int xdir, int ydir, int zdir, rmatrix mtens)
{
    ...
    mtensor( h, nnv, nnv0, basis, spin, param, mb_strain, unstrnd_cubic_cell_length, k, band, mtens);
...
real mb_mstar_spds_111( rvectr p, rvectr param, rvectr mb_strain, real unstrnd_cubic_cell_length, rmatrix mtens)
{
    ...
    eigval_h_full( eigv, h, &idum ); 
    Ep1 = eigv[band];    
    mtens[1][1] = mtens[2][2] = 2*HBAR_MASSFACTOR/((Em1 - 2*E0 + Ep1)/kdivt);
