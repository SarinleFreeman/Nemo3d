...
*****************************************************************************
$Header: /repo/nemo3d/src/io/i_3dout_files.c,v 1.63 2008/09/24 02:09:19 hoonryu
Exp $
*****************************************************************************/
...
void writeHeader_siteInfo(qd_struct d, const char *infoType,
  ...
  if (!strcmp(infoType, "basis")) {
    ...
    int ev = 0;
    strcpy(temp, "");
    if (strstr(filename, "nd_dx_shapeL") != NULL)
      sprintf(temp, "shapeL");
    else if (strstr(filename, "nd_dx_shapeS") != NULL)
      sprintf(temp, "shapeS");
    sprintf(usr_comment[2], "dim%11.3e%11.3e%11.3e", (real)d->geo.cell_xmax,
            (real)d->geo.cell_ymax, (real)d->geo.cell_zmax);
    sprintf(usr_comment[3], "%s", temp);
    sprintf(usr_comment[4], "latticeConsT%11.3e%11.3e%11.3e",
            (real)d->geo.lattice_x, (real)d->geo.lattice_y,
            (real)d->geo.lattice_z);
    ...
    int ev = 0;
    strcpy(temp, "");
    if (strstr(filename, "nd_dx_exxS") != NULL)
      sprintf(temp, "exxS");
    else if (strstr(filename, "nd_dx_exxL") != NULL)
      sprintf(temp, "exxL");
    else if (strstr(filename, "nd_dx_eyyS") != NULL)
      sprintf(temp, "eyyS");
    else if (strstr(filename, "nd_dx_eyyL") != NULL)
      sprintf(temp, "eyyL");
    else if (strstr(filename, "nd_dx_ezzS") != NULL)
      sprintf(temp, "ezzS");
    else if (strstr(filename, "nd_dx_ezzL") != NULL)
      sprintf(temp, "ezzL");
    else if (strstr(filename, "nd_dx_exyS") != NULL)
      sprintf(temp, "exyS");
    else if (strstr(filename, "nd_dx_exyL") != NULL)
      sprintf(temp, "exyL");
    else if (strstr(filename, "nd_dx_exzS") != NULL)
      sprintf(temp, "exzS");
    else if (strstr(filename, "nd_dx_exzL") != NULL)
      sprintf(temp, "exzL");
    else if (strstr(filename, "nd_dx_eyzS") != NULL)
      sprintf(temp, "eyzS");
    else if (strstr(filename, "nd_dx_eyzL") != NULL)
      sprintf(temp, "eyzL");
    sprintf(usr_comment[2], "dim%11.3e%11.3e%11.3e", (real)d->geo.cell_xmax,
            (real)d->geo.cell_ymax, (real)d->geo.cell_zmax);
    sprintf(usr_comment[3], "%s", temp);
    ...
    sprintf(usr_comment[2], "dim%11.3e%11.3e%11.3e", (real)d->geo.cell_xmax,
            (real)d->geo.cell_ymax, (real)d->geo.cell_zmax);
    sprintf(usr_comment[3], "ev%11.3e%21.12e", (real)ev,
            (real)d->WF[ev - 1].E * HAMILTONIAN_SCALE_VALUE);
    sprintf(usr_comment[4], "latticeConsT%11.3e%11.3e%11.3e",
            (real)d->geo.lattice_x, (real)d->geo.lattice_y,
            (real)d->geo.lattice_z);
    ...
    sprintf(usr_comment[2], "delta %11.3e%11.3e%11.3e", (real)d->geo.lattice_x,
            (real)d->geo.lattice_y, (real)d->geo.lattice_z);
    sprintf(usr_comment[3], "count %11d%11d%11d", (int)d->geo.cell_xmax,
            (int)d->geo.cell_ymax, (int)d->geo.cell_zmax);
...
void writeSiteInfo(qd_struct d, const char *infoType, const char *filename,
  ...
  parseRecordFormat(label, &Nint, &Nreal, &Nfloat, &Ncol_var[0], &sz_var[0],
                    &Nvar, fieldType);
...
/* format of label:  {var11,...,var1n} .. {varm1,...,varmn} */
void readSiteInfo(qd_struct d, const char *infoType, const char *filename,
                  void *dat0, ...) {
  FILE *fp;
  int i, j, proc, Ncol_var[MAX_VARS], sz_var[MAX_VARS], Ncol_tot, Nvar;
  int Nreal, Nint, Nfloat;
  ...
  /* determine Ncol[], Ncol_tot, Nvar and number of reals and ints and floats*/
  parseRecordFormat(recFmt, &Nint, &Nreal, &Nfloat, &Ncol_var[0], &sz_var[0],
                    &Nvar, fieldType);
  Ncol_tot = Nint + Nreal + Nfloat;
...
void writePos(qd_struct d, const char *type) {
  ...
  if ((!strcmp(type, "atom_strain_dx_large") ||
       !strcmp(type, "atom_strain_dx_small")) &&
    ...
    if (d->opt.Dev.CrystalStruct == Dev_struct::Zincblende) {
      ...
      real *pos_xx =
          (real *)nml_malloc(4 * sizeof(real) * d->cell_ln[mpi_n3d_id]);
      real *pos_yy =
          (real *)nml_malloc(4 * sizeof(real) * d->cell_ln[mpi_n3d_id]);
      real *pos_zz =
          (real *)nml_malloc(4 * sizeof(real) * d->cell_ln[mpi_n3d_id]);
      ...
      for (i = isx; i < iex; i++) {
        ...
        for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
          if (d->geo.AtomType[i][j]) {
            ...
            if (compute_strain_tensor(&exx, &eyy, &ezz, &exy, &exz, &eyz, d, i,
                                      j) == -1) {
              exx = 0;
              eyy = 0;
              ezz = 0;
      ...
      writeSiteInfo(d, "dxS", filename, "{real x, real y, real z, real exx }",
                    pos_xx);
      str_free(filename);
      ...
      n3d_FileTypeSet(&filename, label, TRUE);
      masterPrint("writing strain profile exx to '%s' for dx", filename);
      writeSiteInfo(d, "dxS", filename, "{real x, real y, real z, real eyy }",
                    pos_yy);
      str_free(filename);
      ...
      writeSiteInfo(d, "dxS", filename, "{real x, real y, real z, real ezz }",
                    pos_zz);
      str_free(filename);
      ...
      writeSiteInfo(d, "dxS", filename, "{real x, real y, real z, real exy }",
                    pos_xy);
      str_free(filename);
      ...
      writeSiteInfo(d, "dxS", filename, "{real x, real y, real z, real exz }",
                    pos_xz);
      str_free(filename);
  ...
  if (!strcmp(type, "shape_dx_large")) {
    ...
    writeSiteInfo(d, "dxP", filename, "{real x, real y, real z, real shape }",
                  pos_shape);
    str_free(filename);
  ...
  if (!strcmp(type, "atom_pos_strain") &&
    ...
    real *pos_xx = (real *)nml_malloc(4 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                      d->NBasisStates);
    real *pos_yy = (real *)nml_malloc(4 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                      d->NBasisStates);
    real *pos_zz = (real *)nml_malloc(4 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                      d->NBasisStates);
    real *pos_xy = (real *)nml_malloc(4 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                      d->NBasisStates);
    real *pos_yz = (real *)nml_malloc(4 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                      d->NBasisStates);
    real *pos_xz = (real *)nml_malloc(4 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                      d->NBasisStates);
    ...
    for (i = isx; i < iex; i++) {
      for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
        if (d->geo.AtomType[i][j]) {
          ...
          if (compute_strain_tensor(&pos_xx[indx], &pos_yy[indx], &pos_zz[indx],
                                    &pos_xy[indx], &pos_xz[indx], &pos_yz[indx],
                                    d, i, j) == -1) {
            pos_xx[indx] = 0;
            pos_yy[indx] = 0;
            pos_zz[indx] = 0;
    ...
    writeSiteInfo(d, "atom", filename, "{real x, real y, real z, real exx }",
                  pos_xx);
    str_free(filename);
    ...
    writeSiteInfo(d, "atom", filename, "{real x, real y, real z, real eyy }",
                  pos_yy);
    str_free(filename);
    ...
    writeSiteInfo(d, "atom", filename, "{real x, real y, real z, real ezz }",
                  pos_zz);
    str_free(filename);
    ...
    writeSiteInfo(d, "atom", filename, "{real x, real y, real z, real exy }",
                  pos_xy);
    str_free(filename);
    ...
    writeSiteInfo(d, "atom", filename, "{real x, real y, real z, real exz }",
                  pos_xz);
    str_free(filename);
    ...
    writeSiteInfo(d, "atom", filename, "{real x, real y, real z, real eyz }",
                  pos_yz);
    str_free(filename);
  ...
  if (!strcmp(type, "atom_pos") &&
    ...
    int indx = 0;
    real *pos = (real *)nml_malloc(3 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                   d->NBasisStates);
  ...
  if (d->opt.ExecParam.Strain.StrainOut.DisplacementData &&
    ...
    masterPrint("Writing atomic displacements to '%s' after the first strain "
                "calculation.",
                filename);
#ifdef DISPL_SINGLE_PRECISION
  ...
  if (d->opt.ExecParam.Strain.StrainOut.DisplacementData &&
    ...
    masterPrint("Writing atomic displacements to '%s' after the second strain "
                "calculation.",
                filename);
#ifdef DISPL_SINGLE_PRECISION
    writeSiteInfo(d, "atom", filename, "{float x, float y, float z}", disp);
  ...
  if (d->opt.ExecParam.Strain.StrainOut.DisplacementData3D &&
      !strcmp(type, "strain_all")) {
    int indx = 0;
    char *filename = n3d_strdup_n(d->inputfile);
    real *pos = (real *)nml_malloc(7 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                   d->NBasisStates);
    ...
    writeSiteInfo(
        d, "atom", filename,
        "{real x, real y, real z, real dx, real dy, real dz, real dr2}", pos);
    str_free(filename);
    nml_free(pos);
  }

  if (d->opt.ExecParam.Strain.StrainOut.DisplacementData3D &&
      !strcmp(type, "strain_all_second")) {
    int indx = 0;
    char *filename = n3d_strdup_n(d->inputfile);
    real *pos = (real *)nml_malloc(7 * sizeof(real) * d->seg_ln[mpi_n3d_id] /
                                   d->NBasisStates);
...
void writeAtomInfo(qd_struct d) {
  ...
  pos =
      (int *)nml_malloc(sizeof(int) * d->seg_ln[mpi_n3d_id] / d->NBasisStates);
  for (i = isx; i < iex; i++) {
    for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
      if (d->geo.AtomType[i][j]) {
...
void writeNbrIndx(qd_struct d) {
  ...
  nbrInfo = (int *)nml_malloc(neighbors * sizeof(int) * d->seg_ln[mpi_n3d_id] /
                              d->NBasisStates);
  ...
  writeSiteInfo(d, "atom", filename, "{int n1, int n2, int n3, int n4}",
                nbrInfo);
  str_free(filename);
  nml_free(nbrInfo);
  MPI_TOC(mpiTiming.fileio, mpiTiming.fileio_0);
...
void writePhaseInfoForNEMO1D(qd_struct d) {
  ...
  int indx = 0;
  int *phaseInfo = (int *)nml_malloc(NeighborsMax * sizeof(int) *
                                     d->seg_ln[mpi_n3d_id] / d->NBasisStates);
  ...
  for (int Zc = isx; Zc < iex; Zc++) {
    ...
    for (int Za = 0; Za < d->geo.AtomsPerCellMax(); Za++) {
      ...
      for (int n = 0; n < d->geo.Neighbors(Za); n++) {
        ...
        phaseInfo[indx++] =
            (Zc_nbr >= 0 ? d->geo.phaseInfoForNemo1D(Zc, Za, n) : 0);
      }
    }
  }
  ...
  masterPrint("Writing neighbor info to '%s'", filename);
  writeSiteInfo(d, "atom", filename, "{int ph1, int ph2, int ph3, int ph4}",
                phaseInfo);
  str_free(filename);
  nml_free(phaseInfo);
}
...
void writeEigenvector(qd_struct d, complex *wf, int indx, const char *suffix) {
  if (!d->opt.ExecParam.ElCalc.ElOut.Eigvect)
    return;
...
void writeOverLap(qd_struct d, complex *wf, int indx, const char *suffix) {
  ...
  writeSiteInfo(d, "overlap", filename,
                "{real istate, real fstate, real overlap.r, real overlap.i}",
                wf);
  str_free(filename);
  MPI_TOC(mpiTiming.fileio, mpiTiming.fileio_0);
}
...
void writeTransitionRate(qd_struct d, real *wf, int indx, const char *suffix) {
  ...
  writeSiteInfo(d, "transitionRate", filename,
                "{real istate, real fstate, real E, real transitionrate}", wf);
  str_free(filename);
  MPI_TOC(mpiTiming.fileio, mpiTiming.fileio_0);
}

void writeMomentumMatrix(qd_struct d, complex *wf, int indx,
                         const char *suffix) {
  char label[40];
  ...
  n3d_FileTypeSet(&filename, label, TRUE);
  masterPrint("writing momentum matrix to '%s'", filename);
  writeSiteInfo(d, "singleValue", filename,
                "{real initState, real finState, real wf.r, real wf.i}", wf);
  str_free(filename);
  MPI_TOC(mpiTiming.fileio, mpiTiming.fileio_0);
}
...
void writePsiSqr(cmatrix wfc, int n, qd_struct d, int wf_indx) {
  ...
  for (k = 0; k < n; k++) {
    ...
    if (d->opt.ExecParam.ElCalc.ElOut.PsiSqr) {
      real *psi2 = (real *)malloc_strippedVector(d, sizeof(real), &psisq[0][0]);
    ...
    if (d->opt.ExecParam.ElCalc.ElOut.Visualization_3D) {
      ...
      writeSiteInfo(d, "dxW", filename,
                    "{real x, real y, real z, real psi2_unit}", psi2);
      str_free(filename);
      nml_free(psi2);
    }

    // Newly added routines to generate atomic scale data on which the new
    // visualizer is based.
...
// #define ATOM_DATA_DUMP
    ...
    real *psi2 =
        (real *)nml_malloc(8 * 4 * sizeof(real) * d->cell_ln[mpi_n3d_id]);
    int index = 0;
    ...
    writeSiteInfo(d, "zcatom", filename, "{real x, real y, real z, real psi2}",
                  psi2);
    str_free(filename);
    nml_free(psi2);
...
// #define PSI2_INDIVIDUAL_COMPONENT
    ...
    for (l = 0; l < d->NBasisStates; l++) {
      ...
      filename = n3d_strdup_n(d->inputfile);
      n3d_FileTypeSet(&filename, label, TRUE);
      masterPrint("writing zc data to '%s' for atom-based visualizer",
                  filename);
      writeSiteInfo(d, "atom", filename,
                    "{real x, real y, real z, real psi2_unit}", psi2_1);
      str_free(filename);
      nml_free(psi2_1);
    }
...
// #define PSI2_SPDSS_SEP
    ...
    for (int seq = 0; seq < 8; seq++) {
      ...
      masterPrint("writing zc data to '%s' for atom-based visualizer",
                  filename);
      writeSiteInfo(d, "atom", filename,
                    "{real x, real y, real z, real psi2_unit}", psi2_2);
      str_free(filename);
      nml_free(psi2_2);
    }
...
// #define PSI2_SPDSS_ALL
    ...
    for (int seq = 0; seq < 4; seq++) {
      ...
      masterPrint("writing zc data to '%s' for atom-based visualizer",
                  filename);
      writeSiteInfo(d, "atom", filename,
                    "{real x, real y, real z, real psi2_unit}", psi2_3);
      str_free(filename);
      nml_free(psi2_3);
    }
...
int print_bond_lengths(qd_struct d) {
  ...
  if (!d->opt.ExecParam.Strain.StrainOut.Bondlength ||
  ...
  for (l = 0; l < d->geo.N_Cell; l++) {
    ...
    for (m = 0; m < d->geo.AtomsPerCellMax(); m++) {
      ...
      if (d->geo.AtomType[l][m]) {
          ...
          if (cindx >= 0) {
            ...
            if (d->geo.AtomType[cindx][aindx]) {
              ...
              for (p = 0; p < 3; p++) {
                ...
                disp = d->geo.l_m_2_atomdsp[cindx][aindx][p] -
                       d->geo.l_m_2_atomdsp[l][m][p];
                dum = a_lattice[p] *
                          (d->geo.NbrCell(m, n, p) +
                           (d->geo.PositionInCell(d->geo.NbrCell(m, n, 3), p) -
                            d->geo.PositionInCell(m, p))) +
                      disp;
                bl += dum * dum;
                if (d->strain.debugp)
                  printf("\n%1.10f", fabs(dum) * 4);
        ...
        blave /= nb;
        x = d->geo.cell__ijk[l][0] * d->geo.lattice_x +
            d->geo.lattice_x * d->geo.PositionInCell(m, 0);
        y = d->geo.cell__ijk[l][1] * d->geo.lattice_y +
            d->geo.lattice_y * d->geo.PositionInCell(m, 1);
        z = d->geo.cell__ijk[l][2] * d->geo.lattice_z +
            d->geo.lattice_z * d->geo.PositionInCell(m, 2);
  ...
  case 'a':
    ...
    if ((fpr = fopen(filename, "r"))) {
      ...
      if (!(fread(dum, sizeof(char), 1, fpr))) {
        ...
        int Ndim, Nuser;
        char recFmt[100];
        fp = readHeader(filename, recFmt, &Ndim, dim, dim_lbl, &Nuser,
                        usr_comment);
        int sz_dat = 4 * dim[0];
...
void multicastDisplacement(qd_struct d) {
  ...
  if (mpi_n3d_numprocs > 1) {
    ...
    if (!mpi_n3d_id)
      printf("  multicasting displacement data\n"), fflush(stdout);
    ...
    for (j = 0; j < mpi_n3d_numprocs; j++) {
      ...
      MPI_Bcast(&d->geo.l_m_2_atomdsp[d->cell_s[j]][0][0],
                d->cell_ln[j] * d->geo.AtomsPerCellMax() * 3, MPI_FLOAT, j,
                MPI_COMM_WORLD);
#else  /* DISPL_SINGLE_PRECISION */
      MPI_Bcast(&d->geo.l_m_2_atomdsp[d->cell_s[j]][0][0],
                d->cell_ln[j] * d->geo.AtomsPerCellMax() * 3, MPI_DOUBLE, j,
                MPI_COMM_WORLD);
#endif /* DISPL_SINGLE_PRECISION */
...
void readDisplacement(qd_struct d, rvectr p) {
  ...
  if (d->opt.ExecParam.Strain.StrainModel == Strain_struct::Read) {
    if (d->opt.ExecParam.Strain.Keating.StartFileRead == "NULL") {
      if (mpi_n3d_id == mpi_n3d_masterid)
        printf(
            "ERROR:  There is no strain file specified in the input deck!!\n");
      fflush(stdout);
      die("");
    }
  ...
  /* otherwise, be fault tolerant if we can't find the file */
    ...
    if (d->opt.ExecParam.Strain.Keating.StartFileRead == "NULL") {
      ...
      return;
    } else if (!fopen(d->opt.ExecParam.Strain.Keating.StartFileRead.c_str(),
                      "r")) {
      masterPrint(
          "ACHTUNG:  Unable to read displacement file: '%s'\nContinuing anyway",
          d->opt.ExecParam.Strain.Keating.StartFileRead.c_str());
      return;
    } else
      masterPrint("Read displacement data from file: '%s'",
  ...
  if (!strcmp(fmt, "hamB")) {
    ...
    for (atom = 0; atom < vdim1(d->Ham); atom++) {
      for (nbr = 0; nbr <= d->Ham_neighbor[atom]; nbr++) {
        fprintf(fp, "%d %d %d \n", d->Ham_offset_y[atom][nbr] / d->NBasisStates,
                atom, d->Ham_yc[atom][nbr]);
      }
    }
    ...
    for (Zc = 0; Zc < d->geo.N_Cell; Zc++) {
      for (Za = 0; Za < d->geo.AtomsPerCellMax(); Za++) {
        ...
        X = d->geo.lattice_x *
                (d->geo.cell__ijk[Zc][0] + d->geo.PositionInCell(Za, 0)) +
            d->geo.l_m_2_atomdsp[Zc][Za][0];
        Y = d->geo.lattice_y *
                (d->geo.cell__ijk[Zc][1] + d->geo.PositionInCell(Za, 1)) +
            d->geo.l_m_2_atomdsp[Zc][Za][1];
        Z = d->geo.lattice_z *
                (d->geo.cell__ijk[Zc][2] + d->geo.PositionInCell(Za, 2)) +
          ...
          for (j = 0; j < d->NBasisStates; j++) {
            if (d->Ham[atom][nbr][i][j].r != 0.0 ||
              ...
              if (d->Ham_transp[atom][nbr]) {
                fprintf(
                    fp, "%d %d %g %g %g\n", d->Ham_offset_y[atom][nbr] + i + 1,
                    d->NBasisStates * atom + j + 1, d->Ham[atom][nbr][j][i].r,
                    -d->Ham[atom][nbr][j][i].i,
                    sqrt(d->Ham[atom][nbr][j][i].r * d->Ham[atom][nbr][j][i].r +
                         d->Ham[atom][nbr][j][i].i *
                             d->Ham[atom][nbr][j][i].i));
              } else {
                fprintf(
                    fp, "%d %d %g %g %g\n", d->Ham_offset_y[atom][nbr] + i + 1,
                    d->NBasisStates * atom + j + 1, d->Ham[atom][nbr][i][j].r,
                    d->Ham[atom][nbr][i][j].i,
                    sqrt(d->Ham[atom][nbr][i][j].r * d->Ham[atom][nbr][i][j].r +
                         d->Ham[atom][nbr][i][j].i *
                             d->Ham[atom][nbr][i][j].i));
              }
            }
            ...
            if (d->Ham[atom][nbr][i][j].r != 0.0 ||
              ...
              if (d->Ham_transp[atom][nbr]) {
                fprintf(fp, "%d %d %20.12e %20.12e %d\n",
                        d->Ham_offset_y[atom][nbr] + i + 1,
                        d->NBasisStates * atom + j + 1,
                        d->Ham[atom][nbr][j][i].r, -d->Ham[atom][nbr][j][i].i,
                        d->Ham_neighbor[atom]);
              } else {
                fprintf(fp, "%d %d %20.12e %20.12e %d\n",
                        d->Ham_offset_y[atom][nbr] + i + 1,
                        d->NBasisStates * atom + j + 1,
                        d->Ham[atom][nbr][i][j].r, d->Ham[atom][nbr][i][j].i,
                        d->Ham_neighbor[atom]);
              }
            }
          }
...
void readWaveFunction(char *filename, qd_struct d, cvectr p) {
  ...
  FILE *fvec =
      readHeader(filename, recFmt, &Ndim, dim, dim_lbl, &Nuser, usr_comment);
  ...
  if (d->opt.Dev.band_model !=
    ...
    die("Cannot continue -- inconsistency between input file and current "
        "parameters.");
  }
...
int initStartVect(char *filename, qd_struct d, cvectr p) {
  ...
  fvec = readHeader(filename, recFmt, &Ndim, dim, dim_lbl, &Nuser, usr_comment);
  ...
  if (d->opt.Dev.band_model ==
    ...
    /* File has been written using the Bands_1_s_nospin, zincblende models and
       we are currently using the Bands_10_sp3ss_spin model, so that the file
       countains ten times less orbital information than currently required.
       Various mapping algorithms can be tried here to achieve better
       convergence.
    */
    complex dummy;
    real norm;
...
#undef UseForHoles2 /* UseForHoles2 seemed to have worked best in the few      \
                       cases gekco looked at. */
#ifdef UseForHoles1
      p[ii + 1].r = p[ii + 6].r =
          dummy.r / 2000000.0; /* s orbitals have small contribution */
      p[ii + 1].i = p[ii + 6].i = dummy.i / 2000000.0;

      p[ii + 0].r = p[ii + 5].r =
          dummy.r / 1000000000.0; /* s* orbitals have almost no contribution */
      ...
      p[ii + 3].r = p[ii + 4].r = p[ii + 7].r = p[ii + 8].r = p[ii + 9].r =
          dummy.r / 100000.0;
      p[ii + 3].i = p[ii + 4].i = p[ii + 7].i = p[ii + 8].i = p[ii + 9].i =
          dummy.i / 100000.0;
#endif
#ifdef UseForHoles2
      p[ii + 1].r = p[ii + 6].r =
      ...
      p[ii + 2].r = p[ii + 4].r = p[ii + 7].r = p[ii + 8].r = p[ii + 9].r =
          dummy.r / 100000.0;
      p[ii + 2].i = p[ii + 4].i = p[ii + 7].i = p[ii + 8].i = p[ii + 9].i =
          dummy.i / 100000.0;
#endif
#ifdef UseForHoles3
      p[ii + 1].r = p[ii + 6].r =
      ...
      p[ii + 2].r = p[ii + 3].r = p[ii + 7].r = p[ii + 8].r = p[ii + 9].r =
          dummy.r / 100000.0;
      p[ii + 2].i = p[ii + 3].i = p[ii + 7].i = p[ii + 8].i = p[ii + 9].i =
          dummy.i / 100000.0;
#endif
#ifdef UseForHoles4
      p[ii + 1].r = p[ii + 6].r =
      ...
      /* px, py, pz orbitals have small contributions */
      p[ii + 2].r = p[ii + 3].r = p[ii + 4].r = p[ii + 7].r = p[ii + 8].r =
          p[ii + 9].r = dummy.r / 2.0;
      p[ii + 2].i = p[ii + 3].i = p[ii + 4].i = p[ii + 7].i = p[ii + 8].i =
          p[ii + 9].i = dummy.i / 100000.0;
#endif
#ifdef UseForHoles5
      p[ii + 1].r = p[ii + 6].r =
          dummy.r / 2000000.0; /* s orbitals have small contribution */
...
#define UseForElectrons
      ...
      p[ii + 1].r =
          -dummy.r / 2000.0; /* s orbitals have the major contribution */
      p[ii + 1].i = -dummy.i / 2000000.0;

      p[ii + 0].r = p[ii + 5].r =
          -dummy.r / 1000000.0; /* s* orbitals have almost no contribution */
      p[ii + 0].i = p[ii + 5].i = -dummy.i / 1000000000.0;

      /* px, py, pz orbitals have small contributions */
      p[ii + 2].r = p[ii + 3].r = p[ii + 4].r = p[ii + 7].r = p[ii + 8].r =
          p[ii + 9].r = -dummy.r / 100000.0;
      p[ii + 2].i = p[ii + 3].i = p[ii + 4].i = p[ii + 7].i = p[ii + 8].i =
          p[ii + 9].i = -dummy.i / 100000000.0;
#endif
#if 1
      p[ii + 1].r = p[ii + 6].r =
          dummy.r / 2.0; /* s orbitals have the major contribution */
      p[ii + 1].i = p[ii + 6].i = dummy.i / 2.0;

      p[ii + 0].r = p[ii + 5].r =
          dummy.r / 1000.0; /* s* orbitals have almost no contribution */
      p[ii + 0].i = p[ii + 5].i = dummy.i / 1000.0;

      /* px, py, pz orbitals have small contributions */
      p[ii + 2].r = p[ii + 3].r = p[ii + 4].r = p[ii + 7].r = p[ii + 8].r =
          p[ii + 9].r = dummy.r / 100.0;
      p[ii + 2].i = p[ii + 3].i = p[ii + 4].i = p[ii + 7].i = p[ii + 8].i =
          p[ii + 9].i = dummy.i / 100.0;
#endif
    ...
    /* File has been written using the Bands_1_s_nospin, zincblende models and
       we are currently using the Bands_20_sp3d5ss_spin model, so that the file
       countains ten times less orbital information than currently required.
       Various mapping algorithms can be tried here to achieve better
       convergence.
    */
...
#undef sp3d5_UseForHoles2 /* UseForHoles2 seemed to have worked best in the    \
                             few cases gekco looked at. */
      ...
      p[ii + 3].r = dummy.r / 2.0;
      p[ii + 3].i = dummy.i / 2000.0;
      p[ii + 2].r = p[ii + 4].r = p[ii + 12].r = p[ii + 13].r = p[ii + 14].r =
          dummy.r / 100000.0;
      p[ii + 2].i = p[ii + 4].i = p[ii + 12].i = p[ii + 13].i = p[ii + 14].i =
          dummy.i / 100000.0;

      /* 	d orbitals assume small contribution as well */
      p[ii + 5].r = p[ii + 6].r = p[ii + 7].r = p[ii + 8].r = p[ii + 9].r =
          -dummy.r / 100000.0;
      p[ii + 5].i = p[ii + 6].i = p[ii + 7].i = p[ii + 8].i = p[ii + 9].i =
          -dummy.i / 100000000.0;
      p[ii + 15].r = p[ii + 16].r = p[ii + 17].r = p[ii + 18].r = p[ii + 19].r =
          -dummy.r / 100000.0;
      p[ii + 15].i = p[ii + 16].i = p[ii + 17].i = p[ii + 18].i = p[ii + 19].i =
          -dummy.i / 100000000.0;
#endif
...
int print_struct_gmv(qd_struct d) {
  ...
  if (!d->opt.ExecParam.ElCalc.ElOut.StructureGmv ||
      mpi_n3d_id != mpi_n3d_masterid)
    return 0;
      ...
      if (d->geo.AtomType[i][j] > 0) {
        ...
        for (l = 0; l < d->geo.Neighbors(j); l++) {
          ...
          if (d->geo.ijk__cell[id][jd][kd] >= 0) {
            ...
            if (d->geo.AtomType[d->geo.ijk__cell[id][jd][kd]]
                               [d->geo.NbrCell(j, l, 3)] > 0)
              nnbr++;
          }
        }
        if (nnbr >= 2 && nnbr <= 4)
...
int print_struct_pdb(qd_struct d) {
  ...
  if (!d->opt.ExecParam.ElCalc.ElOut.StructurePdb ||
      mpi_n3d_id != mpi_n3d_masterid)
    return 0;
    ...
    for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
      ...
      if (d->geo.AtomType[i][j] > 0) {
        ...
        z = 10 * ((real)d->geo.cell__ijk[i][2] * d->geo.lattice_z +
                  d->geo.lattice_z * d->geo.PositionInCell(j, 2)) +
            d->geo.l_m_2_atomdsp[i][j][2];
        /* the output of multimillion atom simulations requires the ability to
         * look at more than 5 digits of integer numbers. The original
         * formatting required these limitations to 5 digits.
         *
        ...
        case (EL_None):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  "              1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_B):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " B            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_C):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " C            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_N):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " N            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_O):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " O            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_F):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " F            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Al):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Al           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Si):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Si           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_P):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " P            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Zn):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Zn           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Ga):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Ga           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Ge):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Ge           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_As):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " As           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Cd):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Cd           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_In):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " In           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Sn):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Sn           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_Sb):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Sb           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_H):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " H            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
          break;
        case (EL_S):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " S            1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
        case (EL_Se):
          fprintf(f_pdb,
                  "ATOM  " PDBFORMATATOM
                  " Se           1    %8.3f%8.3f%8.3f" COORDINATEWRITE "\n",
                  na, x, y, z);
        case (EL_EXPLICIT):
        case (EL_EXPLICIT + 1):
        case (EL_EXPLICIT + 2):
        ...
        /* if ( d->geo.AtomType[i][j] == 12 )
           fprintf(f_pdb, "ATOM  "FORMATOUT" As           1    %8.3f%8.3f%8.3f
           "COORDINATEWRITE"\n",na,x,y,z); else if ( d->geo.AtomType[i][j] == 10
           ) fprintf(f_pdb, "ATOM  "FORMATOUT" Ga           1    %8.3f%8.3f%8.3f
           "COORDINATEWRITE"\n",na,x,y,z); else if ( d->geo.AtomType[i][j] == 14
           ) fprintf(f_pdb, "ATOM  "FORMATOUT" In           1    %8.3f%8.3f%8.3f
           "COORDINATEWRITE"\n",na,x,y,z); */
      }
    }
  }
  /*   fprintf(f_pdb, "TER   "FORMATOUT"\n",na+1); */ /* Commented to make
                                                         outputfile readable in
                                                         rasmol */
...
int print_isosurf_pdb(qd_struct d, rmatrix data, real isoval, int eigvallabel) {
  ...
  if (!d->opt.ExecParam.ElCalc.ElOut.PsiSqrPdb ||
      mpi_n3d_id != mpi_n3d_masterid)
    return 0;
      ...
      if (d->geo.AtomType[i][j] > 0) {
        ...
        if (data[i][j] >= isoval) {
          ...
          case (EL_None):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    "              1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_B):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " B            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_C):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " C            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_N):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " N            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_O):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " O            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_F):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " F            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Al):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Al           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Si):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Si           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_P):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " P            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Zn):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Zn           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Ga):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Ga           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Ge):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Ge           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_As):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " As           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Cd):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Cd           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_In):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " In           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Sn):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Sn           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_Sb):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Sb           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_H):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " H            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
            break;
          case (EL_S):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " S            1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
          case (EL_Se):
            fprintf(f_pdb,
                    "ATOM  " FORMATOUT
                    " Se           1    %8.3f%8.3f%8.3f  1.00  0.00\n",
                    na, x, y, z);
          }
        }
      }
        ...
        for (l = 0; l < d->geo.Neighbors(j); l++) {
          ...
          if (d->geo.ijk__cell[id][jd][kd] >= 0) {
            ...
            if (d->geo.AtomType[d->geo.ijk__cell[id][jd][kd]]
              ...
              /* if ( data[d->geo.ijk__cell[id][jd][kd]][d->geo.NbrCell(j,l,3)]
                 >= isoval )
                 { */
              nb = d->geo.ijk__cell[id][jd][kd] * d->geo.AtomsPerCellMax() +
                   d->geo.NbrCell(j, l, 3) + 1;
...
int scatter_print_all(qd_struct d, rmatrix psisq, FILE *f_out, int isx,
                      int iex) {
  int i, j;
...
int scatter_print_unitcell(qd_struct d, rmatrix psisq, FILE *f_out, int isx,
                           int iex) {
  int i, j;
  real xa = 0.0, ya = 0.0, za = 0.0;
  real dummy;
  ...
  if (mpi_n3d_id == mpi_n3d_masterid) {
    ...
    for (proc = 0; proc < mpi_n3d_numprocs; proc++) {
      if (proc != mpi_n3d_masterid) {
        ...
        psisqr_others =
            check_rmatrix(psisqr_others, iex - isx, d->geo.AtomsPerCellMax());

        MPI_Recv(&psisqr_others[0][0], (iex - isx) * d->geo.AtomsPerCellMax(),
                 MPI_DOUBLE, proc, MPI_ANY_TAG, MPI_COMM_WORLD,
                 &mpi_n3d_status);

        if (d->opt.ExecParam.ElCalc.ElOut.PsiSqr)
          scatter_print_all(d, psisqr_others, f_all, isx, iex);
        if (d->opt.ExecParam.ElCalc.ElOut.PsiSqr_cell)
          scatter_print_unitcell(d, psisqr_others, f_unit, isx, iex);
      }
    }
    if (d->opt.ExecParam.ElCalc.ElOut.PsiSqr)
      fclose(f_all);
...
void print_psisq_gnu_pdb_k(cmatrix wfc, int k, qd_struct d, int n_offset) {
  ...
  /* print_isosurf_pdb( d, psisq, d->opt.ExecParam.ElCalc.ElOut.PsiSqrCutoffPdb
   * , k+n_offset+1);  */
...
void print_psisq_gnu_pdb(cmatrix wfc, int n, qd_struct d, int n_offset) {
  ...
  for (k = 0; k < n; k++) {
    ...
    /*  print_psisqr_gnuplot( d, psisq, k+n_offset+1); */
    /* print_isosurf_pdb( d, psisq,
     * d->opt.ExecParam.ElCalc.ElOut.PsiSqrCutoffPdb , k+n_offset+1);  */
  }
...
void write_displacement(qd_struct d) {
  ...
  if ((!d->opt.ExecParam.Strain.StrainOut.DisplacementData &&
       !d->opt.ExecParam.Strain.StrainOut.DisplacementData3D) ||
      mpi_n3d_id != mpi_n3d_masterid)
    return;
#else
  if ((!d->opt.ExecParam.Strain.StrainOut.DisplacementData &&
    ...
    for (i = 0; i < d->geo.N_Cell; i++) {
      for (j = 0; j < d->geo.AtomsPerCellMax(); j++) {
        if (d->geo.AtomType[i][j]) {
          ...
          fprintf(fp2, " %e",
                  sqrt(d->geo.l_m_2_atomdsp[i][j][0] *
                           d->geo.l_m_2_atomdsp[i][j][0] +
                       d->geo.l_m_2_atomdsp[i][j][1] *
                           d->geo.l_m_2_atomdsp[i][j][1] +
                       d->geo.l_m_2_atomdsp[i][j][2] *
...
void read_displacement(qd_struct d, rvectr p) {
  ...
  if (mpi_n3d_id == mpi_n3d_masterid) {
    ...
    if (!(fp = fopen(d->opt.ExecParam.Strain.Keating.StartFileRead.c_str(),
                     "r"))) {
    ...
    MPI_Recv(p, 3 * d->geo.AtomsPerCellMax() * d->cell_ln[mpi_n3d_id],
             MPI_DOUBLE, mpi_n3d_masterid, MPI_ANY_TAG, MPI_COMM_WORLD,
             &mpi_n3d_status);
...
// The variable first_or_second tells whether this function is called the first
// or second time.
{
  ...
  if (!d->opt.ExecParam.Strain.StrainOut.Neighborhood)
    return;
...

  {
    ...
    if (first_or_second == 1)
      sprintf(label, "nd_bondLen_first_%d", mpi_n3d_id);
    else
      sprintf(label, "nd_bondLen_second_%d", mpi_n3d_id);
    filename = n3d_strdup_n(d->inputfile);
    n3d_FileTypeSet(&filename, label, TRUE);
    fp = fopen(filename, "w");
    str_free(filename);
  ...
  for (l = isx; l < iex; l++) {
    ...
    for (m = 0; m < d->geo.AtomsPerCellMax(); m++) {
      ...
      if (x < locbs_xmin || x > locbs_xmax)
        continue;
      if (y < locbs_ymin || y > locbs_ymax)
        continue;
      if (z < locbs_zmin || z > locbs_zmax)
        continue;
      ...
      if (flag)
        continue; /* incomplete set of neighbors found; throw out */
      ...
      for (b = 0; b < d->geo.Neighbors(m); b++) {
        ...
        alat = d->strain.amat[(int)d->geo.AtomType[l][m]]
      ...
      fprintf(fp,
              "%3d   %11.8f %11.8f %11.8f   %11.8f %11.8f %11.8f   %11.8f "
              "%11.8f %11.8f   %11.8f %11.8f %11.8f   %11.8f %11.8f %11.8f   "
              "%11.8f %11.8f %11.8f   %11.8f %11.8f %11.8f   %11.8f %11.8f "
              "%11.8f   %11.8f %11.8f %11.8f\n",
              (int)d->geo.AtomType[l][m], x + disp_this[0], y + disp_this[1],
              z + disp_this[2], dr[0][0], dr[0][1], dr[0][2], dr[1][0],
              dr[1][1], dr[1][2], dr[2][0], dr[2][1], dr[2][2], dr[3][0],
              dr[3][1], dr[3][2], dr0[0][0], dr0[0][1], dr0[0][2], dr0[1][0],
              dr0[1][1], dr0[1][2], dr0[2][0], dr0[2][1], dr0[2][2], dr0[3][0],
              dr0[3][1], dr0[3][2]);
    }
  }
  fclose(fp);
...
/* This is primarily intended for the simple singel band case where the
   overall structure may be a free-standing pyramid and some atoms might have
   been out of the original cubic domain.  We need to fill in these missing
   atoms for visualization on a regular mesh */
int write_cubic_struct_wavefunctions(qd_struct d) {
  ...
  geo_xi_yi_zi_2_cell =
      I3tensor(0, d->geo.Nx_Cell, 0, d->geo.Ny_Cell, 0, d->geo.Nz_Cell);
  geo_xi_yi_zi_inside =
      B3tensor(0, d->geo.Nx_Cell, 0, d->geo.Ny_Cell, 0, d->geo.Nz_Cell);

  /* for the app-qdot application on nanoHUB it would be nice to outline the
     shapes of the embedded quantum dots.  In the simplest of all QD
     calculations there is only one material and the confinement is "air",
     therefore the outer edge of all included atoms / sites is really forming
     the confinement area. However if it is an embedded quantum dot, then we are
     looking to put the shape into the transition of the confining material to
     the confinement material.  Here at the output level we have really lost all
     shape information already.  Really this processing should happen at the
     input scanning routine!
  */
