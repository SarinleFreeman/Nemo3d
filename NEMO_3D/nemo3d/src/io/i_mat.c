...
int str_to_atomID(const char *atomname) {
  ...
  if ((strcasecmp(atomname, "NULL") == 0) || (strcasecmp(atomname, "XX") == 0))
    result = EL_None;
...
static Material_struct *find_Material_struct(vector<Material_struct> &m,
                                             string material, string band_model,
                                             string comp1_mat, real comp1_val,
                                             string comp2_mat, real comp2_val,
                                             real tkel, bool get_strain) {
  for (vector<Material_struct>::iterator mPtr = m.begin(); mPtr != m.end();
       mPtr++) {
...
// #define _PRINT_DEBUG
    ...
    if (material != mPtr->material)
      continue;
    if (tkel != mPtr->tkel && tkel >= 0)
      continue;
    if (comp1_mat != mPtr->comp1_mat)
      continue;
    if ((comp1_val != mPtr->xval) && comp1_val >= 0)
      continue;
    if (comp2_mat != mPtr->comp2_mat)
      continue;
    if ((comp2_val != mPtr->yval) && comp2_val >= 0)
      continue;
    if (band_model != mPtr->band_model)
      continue;
...
static void printMaterialList(vector<MaterialHandle> &sml) {
  ...
  for (vector<MaterialHandle>::iterator p = sml.begin(); p != sml.end(); p++) {
    ...
    for (vector<int>::iterator c = p->cation_id.begin();
      ...
      printf("%d ", *c);
    printf("}  anions={ ");
    for (vector<int>::iterator a = p->anion_id.begin(); a != p->anion_id.end();
         a++)
      printf("%d ", *a);
    printf("}\n");
  }
...
void QD_struct::set_sMatList() {
  ...
  for (vector<Shape_struct>::iterator forme = this->opt.Dev.ShapeList.begin();
    ...
    int Npolar = 0;
    for (vector<int>::iterator c = forme->pol.begin(); c != forme->pol.end();
         c++) {
      if (*c != 0)
        Npolar++;
    }
    ...
    if (is_monoatomic) {
      ...
      Material_struct *m = find_Material_struct(
          this->opt.material_list, forme->mat[0], bandModel, string("NULL"),
          0.0, string("NULL"), 0.0, tkel,
          this->opt.ExecParam.ElCalc.include_strain_in_Hamiltonian);

      if (!m)
        die("Could not find requested material %s with band model %s.",
            forme->mat[0].c_str(), bandModel.c_str());
    ...
    //   case:  binary material or alloy
      ...
      if (is_alloy) {
        ...
        Material_struct *m = find_Material_struct(
            this->opt.material_list, material, bandModel, comp1_mat, comp1_val,
            comp2_mat, comp2_val, tkel,
            this->opt.ExecParam.ElCalc.include_strain_in_Hamiltonian);
        ...
        if (!m || h == this->sMatList.end()) {
          ...
          cout_master
              << "      Shape material is an alloy (previously found):  "
              << h->cation << " + " << h->anion << "   (matid3d=" << h->matid3d
              << ")\n";
        }
      }
      ...
      if (!(is_vca && is_alloy)) {
        ...
        for (unsigned int cation = 0; cation < forme->pol.size(); cation++) {
          if (forme->pol[cation] < 0)
            continue; // ignore anions

          // Find a corresponding anion.
          for (unsigned int anion = 0; anion < forme->pol.size(); anion++) {
            if (forme->pol[anion] > 0)
              continue; // ignore cations
            ...
            /* leave out any VCA treatment and look for the
              simple pairs only. */
            Material_struct *m = find_Material_struct(
                this->opt.material_list, material, bandModel, string("NULL"),
                0.0, string("NULL"), 0.0, tkel,
                this->opt.ExecParam.ElCalc.include_strain_in_Hamiltonian);

            if (!m)
              die("Could not find requested material %s with band model %s.",
                  material.c_str(), bandModel.c_str());
            ...
            if (h == this->sMatList.end()) { // no match; must be new
              ...
              cout_master << "      Adding a new binary material:  "
                          << m->material << "  (matid3d=" << mh.matid3d
                          << ")\n";
            }
