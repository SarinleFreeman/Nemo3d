...
*****************************************************************************
$Header: /repo/nemo3d/src/io/fileformat.c,v 1.17 2009/04/27 12:19:29 nkharche Exp $
*****************************************************************************/
...
void writeHeader(const char* filename, const char* recFmt, 
...
void displayHeader(const char* filename) 
{
   ...
   parseRecordFormat(label, &Nint, &Nreal, &Nfloat, Ncol_var, sz_var, &Nvar, fieldType);
...
void fmtdat(int argc, char* argv[])
{
   ...
   info.append("        -b:   convert from ascii format to double precision binary");
   ...
   info.append("        -dx:  convert binary data to dx data for 3D visualization");
   info.append("        -Wf2Pdb wf_file pdb_file:  merge binary wavefunction file with pdb file and output a pdb format file");
...
#ifdef DONTBUILDFORMATCONVERTER
...
void writeDXfile(char* filename) {
	...
	FILE *fp;
 	...
 	char c, tension[5], shape[7], ev[10], lattice[10], xmax[10], ymax[10], zmax[10];
 	char evalue[20], xbuffer[19], ybuffer[19], zbuffer[19], databuffer[20], newfilename[100];
 	int count = 0;
 	int RNum = 0;
 	...
 	strcpy(xbuffer,"");strcpy(ybuffer,"");strcpy(zbuffer,"");strcpy(databuffer,"");
 	strcpy(ev,"");strcpy(lattice,"");strcpy(xmax,"");strcpy(ymax,"");strcpy(zmax,"");
 	strcpy(evalue,"");strcpy(newfilename,"");strcpy(tension,"");strcpy(shape,"");

 	evN =0; xi = 0; yi = 0; zi = 0; index = 0; cellxmax = 0; cellymax = 0; cellzmax = 0; index1d = 0;
 	value = 0; eLvalue = 0; lattice_x = 0; lattice_y = 0; lattice_z = 0;
 	initpos = 0; datainitpos = 0;
	...
	fgetc(fp);fgetc(fp);fread(xmax,9,1,fp);xmax[9]='\0';
 	fgetc(fp);fgetc(fp);fread(ymax,9,1,fp);ymax[9]='\0';
 	fgetc(fp);fgetc(fp);fread(zmax,9,1,fp);zmax[9]='\0';
 	...
 	if (strstr(filename,".nd_dx_e")!=NULL) {
	 	...
	 	fgetc(fp);fgetc(fp);fread(tension,4,1,fp);tension[4]='\0';
 	}
	 	...
	 	fgetc(fp);fgetc(fp);fread(shape,6,1,fp);shape[6]='\0';
 	}
		...
		while(1) { 
	 	...
	 	fgetc(fp);fgetc(fp);fread(ev,9,1,fp);ev[9]='\0';
	 	fgetc(fp);fgetc(fp);fread(evalue,19,1,fp);evalue[19]='\0';	 
 	}
 	...
 	fgetc(fp);fgetc(fp);fread(lattice,9,1,fp);lattice[9]='\0';
 	...
 	double *Lvalue = (double *) malloc(sizeof(double)*cellxmax*cellymax*cellzmax);
 	fseek(fp,initpos, SEEK_SET);
 	count = 0;
	...
	for (index = 1; index <= RNum; index++)  {
   		read_oneLine(fp, xbuffer, ybuffer, zbuffer, databuffer, &xi, &yi, &zi, &value);
 		//printf("\n%s %s %s %s %d %d %d %f\n", xbuffer, ybuffer, zbuffer, databuffer, xi, yi, zi, value);
   		...
   		if (index == 1) { 
     			...
     			for (count = index1d + 1; count <  (xi - 1)*cellymax*cellzmax + (yi-1)*cellzmax + (zi-1) ; count++) {
          			Lvalue[count] = 0.0;
     			}
   		}
   		...
   		if (index == RNum) { 
     			for (count = index1d+1;count <= cellxmax*cellymax*cellzmax -1; count++) {
          			Lvalue[count] = 0.0;
     			}
   		}
 	}       
 	...
 	if(strstr(filename,".nd_dx_shape") != NULL) {
		writeDXHeader_For_Tension_Profile(fp, cellxmax, cellymax, cellzmax, lattice_x, lattice_y, lattice_z, shape);
	}

 	else if (strstr(filename,".nd_dx_e") !=NULL) {
	 	writeDXHeader_For_Tension_Profile(fp, cellxmax, cellymax, cellzmax, lattice_x, lattice_y, lattice_z, tension);
 	}

	else 
	 	writeDXHeader_For_WF_Profile(fp, cellxmax, cellymax, cellzmax, evN, eLvalue, lattice_x, lattice_y, lattice_z);
...
void read_oneLine(FILE *fp, char* xbuffer, char* ybuffer, char* zbuffer, char* databuffer, int* xi, int* yi, int* zi, double* value) { 
 
 	fread(xbuffer,18,1,fp);xbuffer[18]='\0';fgetc(fp);fgetc(fp);fgetc(fp);
 	fread(ybuffer,18,1,fp);ybuffer[18]='\0';fgetc(fp);fgetc(fp);fgetc(fp);
 	fread(zbuffer,18,1,fp);zbuffer[18]='\0';fgetc(fp);fgetc(fp);
 	fread(databuffer,19,1,fp);databuffer[19]='\0';fgetc(fp);fgetc(fp);fgetc(fp);fgetc(fp);
 	*xi = atof(xbuffer);
 	*yi = atof(ybuffer);
	*zi = atof(zbuffer);
...
void  reformatBinaryData(char* filename, const char* flags, int prec)
{
   ...
   if (Nfloat) die("Can't handle floats yet") ;
   ...
   if ( (_convertNET || _convertHW) && !_convertSNGL) {
      ...
      dat2 = (unsigned long*) 
         malloc(sizeof(unsigned long)*szReal_long*sz_chunk*Nreal);
...
void convertWfToPdb(char* filename,char* filename2)
{
   ...
   for (k=0; k<N_chunk; k++) {
	 ...
	 for (j=0; j<Nreal+Nint+Nfloat; j++) {
		       ...
		       fprintf(fp_new, "ATOM "FORMATOUT" %s%12d    %8.3f%8.3f%8.3f%14.7e", na,str,num,x,y,z,*((float*) dat));
		  fflush(fp_new);
                  dat += rel_sz_real;
               }
	 }
   ...
   for (i=0; i<sz_rem; i++) {
      for (j=0; j<Nreal+Nint+Nfloat; j++) {
		  ...
		  /* fscanf(fp_pdb, "%s  "FORMATOUT"%s%d%f%f%f"COORDINATEREAD"\n",str2,&na,str,&num,&x,&y,&z,&r1,&r2); */
		  fscanf(fp_pdb, "%s  "FORMATOUT"%s%d%f%f%f"COORDINATEREAD"\n",str2,&na,str,&num,&x,&y,&z);
		  fprintf(fp_new, "ATOM "FORMATOUT" %s%12d    %8.3f%8.3f%8.3f%14.7e", na,str,num,x,y,z,*((double*) dat));
		  fflush(fp_new);
                  dat += rel_sz_real;
               }
      }
