...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/Dipole.cpp,v 1.1 2004/08/26 21:28:07
swlee Exp $
*****************************************************************************/
...
int Dipole::pair(int g1, int g2) {
  ...
  for (int p = 0; p < num_atom_pair; p++) {
    int a1 = _atom[g1];
    int a2 = _atom[g2];
    if (a1 == atom_pair[p * num_atom_pair] &&
        a2 == atom_pair[p * num_atom_pair + 1]) {
      pair_index = p;
      break;
    }
  }
...
void Dipole::get_data(complex *coef_e, complex *coef_h, int *atomid,
                      int *shapeid, int *nn, double *lat) {
  _cf_e = coef_e;
  _cf_h = coef_h;
  _atom = atomid;
  _shape = shapeid;
...
void Dipole::compute_dipole_approximation(void) {
  ...
  for (int n1 = 0; n1 < e_max; n1++) {   // electron state
    for (int n2 = 0; n2 < h_max; n2++) { // hole state
      ...
      for (int atom = g_min; atom < g_max; atom++) { // atom index
        ...
        for (int spin = 0; spin < 2; spin++) {                // spin index
          for (int orbital = 0; orbital < o_max; orbital++) { // orbital index
            complex tmp = dot_product(cf_e(n1, atom, spin, orbital),
                                      cf_h(n2, atom, spin, orbital));
            overlap.r += tmp.r;
            overlap.i += tmp.i; // overlap between two wave functions
          }
        }
      ...
      cout_master << "n1, n2, <n1|X|n2>: " << n1 << " " << n2 << " " << "("
                  << X.r << ", " << X.i << ")" << endl;
      cout_master << "n1, n2, <n1|Y|n2>: " << n1 << " " << n2 << " " << "("
                  << Y.r << ", " << Y.i << ")" << endl;
      cout_master << "n1, n2, <n1|Z|n2>: " << n1 << " " << n2 << " " << "("
                  << Y.r << ", " << Y.i << ")" << endl;
    }
  }
}
...
void Dipole::compute_dipole(void) {
  for (int n1 = 0; n1 < e_max; n1++) {
    for (int n2 = 0; n2 < h_max; n2++) {
      ...
      for (int g1 = g_min; g1 < g_max; g1++) {
        ...
        for (int s = 0; s < 2; s++) {
          for (int orbital = 0; orbital < o_max; orbital++) {
            complex tmp =
                dot_product(cf_e(n1, g1, s, orbital), cf_h(n2, g1, s, orbital));
            overlap.r += tmp.r;
            overlap.i += tmp.i;
          }
      ...
      cout_master << "n1, n2 electron+hole, X Oscillator Strength: " << n1
                  << " " << n2 << " " << T_XX << endl;
      cout_master << "n1, n2 electron+hole, Y Oscillator Strength: " << n1
                  << " " << n2 << " " << T_YY << endl;
      cout_master << "n1, n2 electron+hole, Z Oscillator Strength: " << n1
                  << " " << n2 << " " << T_ZZ << endl
                  << endl;
    }
  }
  ...
  for (int n1 = 0; n1 < e_max; n1++) {
    for (int n2 = 0; n2 < h_max; n2++) {
      ...
      for (int g1 = g_min; g1 < g_max; g1++) {
        ...
        for (int orbital = 0; orbital < o_max; orbital++)
          overlap +=
              amplitude_e(n1, g1, orbital) * amplitude_h(n2, g1, orbital);
      ...
      MPI_Allreduce(&Overlap, &T_Overlap, 1, MPI_DOUBLE, MPI_SUM,
                    MPI_COMM_WORLD);

      cout_master << "n1, n2 electron+hole, X Oscillator Strength Approx: "
                  << n1 << " " << n2 << " " << T_XX << endl;
      cout_master << "n1, n2 electron+hole, Y Oscillator Strength Approx: "
                  << n1 << " " << n2 << " " << T_YY << endl;
      cout_master << "n1, n2 electron+hole, Z Oscillator Strength Approx: "
                  << n1 << " " << n2 << " " << T_ZZ << endl;
      cout_master << "n1, n2 electron+hole, X Envelope Oscillator: " << n1
                  << " " << n2 << " " << T_X6 << endl;
      cout_master << "n1, n2 electron+hole, Y Envelope Oscillator: " << n1
                  << " " << n2 << " " << T_Y6 << endl;
      cout_master << "n1, n2 electron+hole, Z Envelope Oscillator: " << n1
                  << " " << n2 << " " << T_Z5 << endl;
      cout_master << "n1, n2 electron+hole, Overlap Approx: " << n1 << " " << n2
                  << " " << T_Overlap << endl
                  << endl;
    }
  }
