...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/Projection.cpp,v 1.2 2004/10/27 19:43:10 gekco Exp $
*****************************************************************************/
...
void Projection::get_data(complex* coef_e, complex* coef_h,
...
void Projection::compute_weight_on_shape(int n1)
{
  ...
  double weight[6];
  weight[0]=0.0; weight[1]=0.0; weight[2]=0.0; weight[3]=0.0; 
  weight[4]=0.0; weight[5]=0.0; 
  ...
  for(int g=g_min; g<g_max; g++){
    ...
    if(_shape[g]==0)  weight[0]+=tmp_weight;
    else if(_shape[g]==1)  weight[1]+=tmp_weight;
    else if(_shape[g]==2)  weight[2]+=tmp_weight;
    else if(_shape[g]==3)  weight[3]+=tmp_weight;
    else if(_shape[g]==4)  weight[4]+=tmp_weight;
    else if(_shape[g]==5)  weight[5]+=tmp_weight;
  }
  ...
  cout_master<<n1<<" Wavefunction,  Weight on each shape 0, 1, 2, 3, ... "
             ...
             <<Weight[3]<<" "<<Weight[4]<<" "<<Weight[5]<<" "<<endl;
...
void Projection::compute_electron_hole_alignment(int n1, int n2)
{
  ...
  cout_master<<"Electron state, center of mass X, Y, Z: "<<n1<<" "<<X<<" "<<Y<<" "<<Z<<endl;  
  ...
  cout_master<<"Electron state, diviation R, X, Y, Z "
             <<n1<<" "<<sqrt(R_div)<<" "<<sqrt(X_div)<<" "<<sqrt(Y_div)<< " "<<sqrt(Z_div)<<endl;
  ...
  MPI_Allreduce(&y, &Y, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&z, &Z, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  cout_master<<"Hole state, center of mass X, Y, Z: "<<n1<<" "<<X<<" "<<Y<<" "<<Z<<endl;  
  ...
  MPI_Allreduce(&z_div, &Z_div, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&r_div, &R_div, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  cout_master<<"Hole state, diviation R, X, Y, Z "
             <<n2<<" "<<sqrt(R_div)<<" "<<sqrt(X_div)<<" "<<sqrt(Y_div)<< " "<<sqrt(Z_div)<<endl;
...
void Projection::compute_qd_wl_extent(int n1)
{
  ...
  cout<<"My id, maximum atom, local maximum wf "<<my_id<<" "<<max_atom[my_id]
      <<" "<<max_weight[my_id]<<endl; 
  ...
  cout_master<<"Center atom location: "
      <<lattice(global_max_atom, 0)
  ...
  MPI_Allreduce(&xy_WL_extent, &XY_WL_extent, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&xy_QD_extent, &XY_QD_extent, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  MPI_Allreduce(&atom_WL, &Total_atom_WL, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&atom_QD, &Total_atom_QD, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  cout_master<<"R, XY, Z: " <<R_extent<<" "<<XY_extent<<" "<<Z_extent<<endl;
...
void Projection::compute_overlap_without_spin(int n1, int n2)
{
  ...
  MPI_Allreduce(&fluctuation, &Fluctuation, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  cout<<"Overlap between wave functions without spin "
      << n1 <<" and "<< n2 <<": "<< Overlap <<endl;  
  cout<<"Fluctuation between wave functions without spin "
      << n1 <<" and "<< n2 <<": "<< Fluctuation/(numAtoms*o_max)<<endl;  
}
...
void Projection::print_amplitude(int n)
{
  ...
  for(int g=g_min; g<g_max; g++){
    ...
    for(int g=g_min;g<g_max;g++){
      if ( lattice(g,0) > 17.5 && lattice(g,0)<18.0 && lattice(g,1)>17.5 && lattice(g,1)<18.0 )
        DensityOut << lattice(g,1) << "\t" << density[g-g_min]<< "\t" << _atom[g]<< endl; 
    }
...
void Projection::compute_projection(void)
{
  ...
  for(int n=0;n<e_max+h_max;n++){
    ...
    for(int g=g_min;g<g_max;g++){
      ...
      Tmp0+= coef(1,0).r*coef(1,0).r + coef(1,0).i*coef(1,0).i; // s*-band
    
      Tmp1+= coef(0,1).r*coef(0,1).r + coef(0,1).i*coef(0,1).i;
      Tmp1+= coef(1,1).r*coef(1,1).r + coef(1,1).i*coef(1,1).i; // s-band
      ...
      lh_u.r = sqrt_two_over_sqrt_three*coef(0,4).r 
             - one_over_sqrt_six*( coef(1,2).r + coef(1,3).i ); //light-hole J=3/2, Jz=1/2
      lh_u.i = sqrt_two_over_sqrt_three*coef(0,4).i 
             - one_over_sqrt_six*( coef(1,2).i - coef(1,3).r  ); //light-hole J=3/2, Jz=1/2
      lh_d.r = sqrt_two_over_sqrt_three*coef(1,4).r 
             + one_over_sqrt_six*( coef(0,2).r - coef(0,3).i ); //light-hole J=3/2, Jz=-1/2
      lh_d.i = sqrt_two_over_sqrt_three*coef(1,4).i 
             + one_over_sqrt_six*( coef(0,2).i + coef(0,3).r ); //light-hole J=3/2, Jz=-1/2
      ss_u.r = one_over_sqrt_three*( coef(0,4).r + coef(1,2).r + coef(1,3).i ); //split-off J=1/2, Jz=1/2
      ss_u.i = one_over_sqrt_three*( coef(0,4).i + coef(1,2).i - coef(1,3).r ); //split-off J=1/2, Jz=1/2
      ss_d.r = one_over_sqrt_three*( coef(0,2).r - coef(0,3).i - coef(1,4).r ); //split-off J=1/2, Jz=-1/2   
      ss_d.i = one_over_sqrt_three*( coef(0,2).i + coef(0,3).r - coef(1,4).i ); //split-off J=1/2, Jz=-1/2   
      
      Tmp2+= hh_u.r*hh_u.r + hh_u.i*hh_u.i + hh_d.r*hh_d.r + hh_d.i*hh_d.i;
      Tmp3+= lh_u.r*lh_u.r + lh_u.i*lh_u.i + lh_d.r*lh_d.r + lh_d.i*lh_d.i;
      Tmp4+= ss_u.r*ss_u.r + ss_u.i*ss_u.i + ss_d.r*ss_d.r + ss_d.i*ss_d.i;

      Tmp8+=coef(0,2).r*coef(0,2).r + coef(0,2).i*coef(0,2).i;
      Tmp8+=coef(1,2).r*coef(1,2).r + coef(1,2).i*coef(1,2).i; //  px band
      Tmp9+=coef(0,3).r*coef(0,3).r + coef(0,3).i*coef(0,3).i;
      Tmp9+=coef(1,3).r*coef(1,3).r + coef(1,3).i*coef(1,3).i; //  py band
      Tmp10+=coef(0,4).r*coef(0,4).r + coef(0,4).i*coef(0,4).i;
      Tmp10+=coef(1,4).r*coef(1,4).r + coef(1,4).i*coef(1,4).i; // pz band
      ...
      for(int orb=0; orb<o_max; orb++){
        double amplitude_up= coef(0,orb).r*coef(0,orb).r + coef(0,orb).i*coef(0,orb).i;  // spin up
        double amplitude_down= coef(1,orb).r*coef(1,orb).r + coef(1,orb).i*coef(1,orb).i;  // spin down 
        Tmp6+= amplitude_up; 
        Tmp7+= amplitude_down;
    ...
    projection[2]=Tmp2*0.5; // total amplitude of heavy-hole Bloch function (J=3/2, Jz=+/-3/2)
    projection[3]=Tmp3; // total amplitude of light-hole Bloch function (J=3/2, Jz=+/-1/2)
    projection[4]=Tmp4; // total amplitude of spin-split Bloch function (J=1/2, Jz=+/-1/2)
    ...
    MPI_Allreduce(projection, total_projection, 11, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
 
    if (mpi_n3d_id==0) {
        if(n<e_max) cout<<"Electron Level: "<<setw(5)<<n+1<<endl;
        else        cout<<"Hole Level: "<<setw(5)<<n-e_max+1<<endl;
        cout<<"Projection onto s* band:          " <<setw(10)<<total_projection[0]<<endl;
        cout<<"Projection onto s  band:          " <<setw(10)<<total_projection[1]<<endl;
        cout<<"Projection onto heavy-hole band:  " <<setw(10)<<total_projection[2]<<endl;
        cout<<"Projection onto light-hole band:  " <<setw(10)<<total_projection[3]<<endl;
        cout<<"Projection onto split-off  band:  " <<setw(10)<<total_projection[4]<<endl;
        cout<<"Projection onto d  band:          " <<setw(10)<<total_projection[5]<<endl;
        cout<<"Projection onto spin up in z:     " <<setw(10)<<total_projection[6]<<endl;
        cout<<"Projection onto spin down in z:   " <<setw(10)<<total_projection[7]<<endl;
        cout<<"Projection onto px band:          " <<setw(10)<<total_projection[8]<<endl;
        cout<<"Projection onto py band:          " <<setw(10)<<total_projection[9]<<endl;
        cout<<"Projection onto pz band:          " <<setw(10)<<total_projection[10]<<endl;
        cout<<endl;
    }
  }
}
