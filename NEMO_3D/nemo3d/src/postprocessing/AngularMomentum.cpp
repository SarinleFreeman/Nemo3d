...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/AngularMomentum.cpp,v 1.1 2004/08/26
21:28:07 swlee Exp $
*****************************************************************************/

#include "AngularMomentum.h"

void AngularMomentum::get_data(complex *coef_e, complex *coef_h, int *atom_id,
...
void AngularMomentum::set_origin(double X, double Y, double Z) {
  ...
  Y0 = Y;
  Z0 = Z;
  cout << "origin for angular momentum operator: " << X0 << " " << Y0 << " "
       << Z0 << endl;
}
...
complex AngularMomentum::compute_global_angular_momentum_square(
    int n1, const char &carrier, const char &direction) {
...
complex
AngularMomentum::compute_global_angular_momentum(int n1, const char &carrier,
  ...
  if (carrier != 'e' && carrier != 'h') {
    ...
    for (int at = g_min; at < g_max; at++) {
      for (int spin = 0; spin < 2; spin++) {
        for (int iorb = 0; iorb < o_max; iorb++) {
          if (carrier == 'e')
            c1 = cf_e(n1, at, spin, iorb);
          else
            c1 = cf_h(n1, at, spin, iorb);
          for (int jorb = 0; jorb < o_max; jorb++) {
            if (carrier == 'e')
              c2 = cf_e(n2, at, spin, jorb);
            else
              c2 = cf_h(n2, at, spin, jorb);
            if (direction == 'x')
              l = MatrixLx[iorb * 10 + jorb];
            else if (direction == 'y')
              l = MatrixLy[iorb * 10 + jorb];
            else
              l = MatrixLz[iorb * 10 + jorb];
            if (l.i != 0.0) {
...
complex AngularMomentum::compute_spin(int n1, int n2, const char &carrier,
                                      const char &direction) {
