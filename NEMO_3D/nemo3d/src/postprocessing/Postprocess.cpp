...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/Postprocess.cpp,v 1.1 2004/08/26 21:28:07 swlee Exp $
*****************************************************************************/
...
//Processing command options =======================================
  ...
  while(iopt < options.size()) {
    if(options[iopt] == "--data") {
      ...
      if(NumElectrons>0) eList = new int[NumElectrons];
      ...
      if(NumHoles>0) hList = new int[NumHoles];
   ...
   if(ComputeTimeReversal && ! HasEvecFile) 
     die("Option '--time_reversal' should be used with option '--evec_file filename'.");  
...
// Prepare Hamiltonian =============================================
   ...
   if(ComputeAngularMomentum) setup_hamiltonian_for_postprocessing(d);
   else setup_crystal_structure_for_postprocessing(d);
...
//Read NEMO3D Output Files ==========================================
  ...
  complex* wf_h; wf_h=0;
  complex* wf; wf=0;
  int* atom_id; atom_id=0;
  int* shape_rank; shape_rank=0;
  int* neighbor; neighbor=0;
  double* lattice; lattice=0;

  read_natoms_norbitals(data, &NumAtoms, &NumOrbitals);
  cout<<"NumAtoms = "<<NumAtoms<<endl;
  cout<<"NumOrbitals = "<<NumOrbitals<<endl<<endl;

  if(ComputeTimeReversal) read_wavefunction_only(evec_file, NumAtoms, NumOrbitals, &wf); 
  else                    read_data_for_postprocessing(data, NumElectrons, eList, 
                                                       NumHoles, hList,
                                                       &wf_e, &wf_h, 
                                                       &atom_id, &shape_rank, 
                                                       &neighbor, &lattice);
...
//Compute Angular Momentum ==============================================
  if(ComputeAngularMomentum) {
    ...
    angularmomentum.get_data(wf_e, wf_h, atom_id, shape_rank, neighbor, lattice);
...
//Compute Dipole Momentum ===============================================
  if(ComputeDipole) {
    ...
    if(!file_input.good()) die("File 'dipole.table' does not exist.");
...
//Compute Coulomb and Exchange  ===============================================
  if(ComputeCoulomb) {
    ...
    if(!file_input.good()) die("File 'coulomb.table' does not exist.");
...
//Compute Decoherence Time ===========================================
  if(ComputeDecoherence) {
    Decoherence decoherence = Decoherence(d, NumElectrons, NumHoles, mpi_n3d_id);
...
// Deleting allocated memory =========================================
   if(wf_e) rm_cvectr(&wf_e);
   if(wf_h) rm_cvectr(&wf_h);
   if(wf)   rm_cvectr(&wf);
   if(lattice)    delete []lattice;
   if(atom_id)    delete []atom_id;
   if(shape_rank) delete []shape_rank;
