...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/ReadData.cpp,v 1.2 2007/02/23 18:59:38 hoonryu Exp $
*****************************************************************************/
...
void read_wavefunction_only(const char* filename, int Natoms, int Norbitals, complex** psi)
{
   FILE* fp;
   char label[100], *dim_lbl[MAX_VARS], *usr_comment[MAX_VARS];
   ...
   if(Nrow != Natoms*Norbitals*2){
      cout_master<<"Nrows from XML and Evec file "<<Nrow<<" "<<Natoms*Norbitals*2<<endl; 
      die("XML file and Evec file do not have the same dimension of the wave function.");
   }
   *psi = Cvectr(Nrow);
                                 ...
                                 ) 
{
   ...
   *lattice = new double[3*Natoms];
   ...
   for(int ia=0; ia<Natoms; ia++) {
     if((*idAtom)[ia]==12) (*idAtom)[ia]=0; //As atom
     else if((*idAtom)[ia]==10) (*idAtom)[ia]=1; //Ga atom
     else if((*idAtom)[ia]==14) (*idAtom)[ia]=2; //In atom
     else if((*idAtom)[ia]==7) (*idAtom)[ia]=3; //Si atom
     else if((*idAtom)[ia]==11) (*idAtom)[ia]=4; //Ge atom
   }
   ...
   if (!fp_tmp) {
     sprintf(filename, "%s.nd_aType", base);
     cout_master << "Reading fake shape rank from atom_type file:  " << filename << endl;
   } 
   else cout_master << "Reading shape rank:  " << filename << endl;
   fp=readHeader(filename, label, &Ndim, dim, dim_lbl, &Nuser, usr_comment);
   ...
   if (!fp_tmp2) {
     cout_master<< "No neighborhood file available. Neighborhood info will be not read!!!!!"<< filename<<endl;
   }
   else {
