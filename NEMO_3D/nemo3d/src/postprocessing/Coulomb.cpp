...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/Coulomb.cpp,v 1.1 2004/08/26 21:28:07
swlee Exp $
*****************************************************************************/
...
const double Coulomb::CONSTANT1 =
    0.482085578,                      // the factor to convert nm^2 to 1/eV^2.
...
void Coulomb::get_data(complex *coef_e, complex *coef_h, int *atomid, int *nn,
                       double *lat) {
  _cf_e = coef_e;
  _cf_h = coef_h;
  _atom = atomid;
...
double Coulomb::w_exch(int g1, int g2, int o1, int o2) {
  double final;
  if (g1 == g2)
    final = exch_on(o1, o2, _atom[g1]);
  else
    final = exch_off(o1, o2, pair(g1, g2));
    ...
    */
    for (int atom = 0; atom < d->geo.AtomsPerCellMax(); atom++) {
      ...
      /*
               ...
               double ratio1 =
         (wf1.r*wf1.r+wf1.i*wf1.i)/(wf4.r*wf4.r+wf4.i*wf4.i); double ratio2 =
         (wf2.r*wf2.r+wf2.i*wf2.i)/(wf5.r*wf5.r+wf5.i*wf5.i); double ratio3 =
         (wf3.r*wf3.r+wf3.i*wf3.i)/(wf6.r*wf6.r+wf6.i*wf6.i);
      */
      ...
      if (wf3.i < 0.0)
        theta3 = -theta3;
      double theta4 = acos(cos4);
      if (wf4.i < 0.0)
        theta4 = -theta4;
      double theta5 = acos(cos5);
      if (wf5.i < 0.0)
        theta5 = -theta5;
      double theta6 = acos(cos6);
      if (wf6.i < 0.0)
        theta6 = -theta6;
      double theta7 = acos(cos7);
      if (wf7.i < 0.0)
        theta7 = -theta7;
      double theta8 = acos(cos8);
      if (wf8.i < 0.0)
        theta8 = -theta8;
...
void Coulomb::compute_overlap_vs_dot_separation(int separation, int direction) {
  ...
  complex T_Overlap;
  MPI_Allreduce(&Overlap.r, &T_Overlap.r, 1, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&Overlap.i, &T_Overlap.i, 1, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
...
// over which the two wave functions overlap.
{
  ...
  H_exch.r = 0.0e0;
  H_exch.i = 0.0e0;
  t1.r = 0.0e0, t1.i = 0.0e0, t2.r = 0.0e0, t2.i = 0.0e0, t3.r = 0.0e0,
  t3.i = 0.0e0;
  ...
  for (int cell1 = cell_start; cell1 < cell_end; cell1++) {
    ...
    for (int atom1 = 0; atom1 < d->geo.AtomsPerCellMax(); atom1++) {
      ...
      for (int orbital1 = 0; orbital1 < o_max; orbital1++) {
        ...
        } // orbital2 for loop exchange interaction between orbitals from the
          // same atom
        ...
        } // atom2 for loop exchange interaction between atoms from the same
          // cell
  ...
  MPI_Allreduce(&H_exch.r, &Total_exch_real, 1, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&H_exch.i, &Total_exch_imag, 1, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  ...
  cout_master << "overlap, exch : " << separation << " " << Total_exch_real
              << " " << Total_exch_imag << endl;
  cout_master << "overlap, t1   : " << separation << " " << T1[0] * inv_dielect
              << " " << T1[1] * inv_dielect << endl;
  cout_master << "overlap, t2   : " << separation << " " << T2[0] * inv_dielect
              << " " << T2[1] * inv_dielect << endl;
  cout_master << "overlap, t3   : " << separation << " " << T3[0] * inv_dielect
              << " " << T3[1] * inv_dielect << endl;
  cout_master << "overlap, t4   : " << separation << " " << T4[0] * inv_dielect
              << " " << T4[1] * inv_dielect << endl
              << endl;
}

complex Coulomb::compute_distant_exchange_interaction(int separation,
                                                      int direction) {
  complex t4;
  t4.r = 0.0e0;
  t4.i = 0.0e0;
...
bool Coulomb::AreCellsClose(int cell1, int cell2) {
  ...
  bool answer;
  if (abs(cell1_x - cell2_x) <= 1 && abs(cell1_y - cell2_y) <= 1 &&
      abs(cell1_z - cell2_z) <= 1)
    answer = true;
  else
    answer = false;
  return answer;
...
int Coulomb::find_cell_overlap(int cell, int separation, int direction) {
  ...
  */
  ...
  int cell_overlap;
  if (cell_overlap_index >= cell_overlap_min &&
      cell_overlap_index <= cell_overlap_max) {
    ...
    cell_overlap = -1;
  /*
       if(cell_overlap!=-1) cout_master<<"cell_y, cell_overlap_y,
     num_overlap_cell: "
                                       <<cell_y<<" "<<cell_overlap_y<<"
     "<<separation<<endl;
  */
  return cell_overlap;
}
