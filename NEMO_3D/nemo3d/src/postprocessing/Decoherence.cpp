...
*****************************************************************************
$Header: /repo/nemo3d/src/postprocessing/Decoherence.cpp,v 1.3 2007/02/23
19:00:36 hoonryu Exp $
*****************************************************************************/
...
#define hf_coupling_coefficient                                                \
  7.849e-18 // in unit of erg with electron density 10e25 /cm3
...
void Decoherence::compute_decoherence_time(void) {
  ...
  double AI_alloy_Variance = Composition * (1.0 - Composition) *
                             (A[2] * I[2] - A[1] * I[1]) *
                             (A[2] * I[2] - A[1] * I[1]);
  ...
  for (int n = 0; n < n_max; n++) {
    ...
    AA_sum[n] = 0.0;
    AAII_sum[n] = 0.0;
    AAII_alloy_sum[n] = 0.0;
    AAII_interface_sum[n] = 0.0;
    ...
    for (int g = g_min; g < g_max; g++) {
      ...
      if (HF_coupling[g - g_min] > A_max)
        A_max = HF_coupling[g - g_min];
    ...
    if (my_id == 0)
      cout << "A_max is " << Total_A_max * tmp_convertor << " eV" << endl;
    ...
    for (int g = g_min; g < g_max; g++)
      if (HF_coupling[g - g_min] > A_min) {
        count_nuclei++;
        fout << I[_atom[g]] << "\t" << HF_coupling[g - g_min] * tmp_convertor
             << endl;
      }
    MPI_Allreduce(&count_nuclei, &total_nuclei, 1, MPI_INT, MPI_SUM,
                  MPI_COMM_WORLD);
    if (my_id == 0)
      cout << "Number of Nuclei interacting with electron " << total_nuclei
           << endl;
    ...
    /*
        ...
        for(int g=g_min;g<g_max;g++){
          if ( lattice(g,0) > 10.2 && lattice(g,0)<10.8 && lattice(g,1)>10.2 &&
       lattice(g,1)<10.8 ) HFzout<< lattice(g,2) << "\t" <<
       HF_coupling[g-g_min]*tmp_convertor<<"\t"<<_atom[g]<< "\t"<<
       HF_coupling[g-g_min]/A[_atom[g]]<<endl;
        }
        ...
        for(int g=g_min;g<g_max;g++){
          if ( lattice(g,2) > 5 && lattice(g,2)<5.4 && lattice(g,1)>10.3 &&
       lattice(g,1)<10.7 ) HFxout<< lattice(g,0) << "\t" <<
       HF_coupling[g-g_min]*tmp_convertor<<"\t"<<_atom[g]<<"\t"<<
       HF_coupling[g-g_min]/A[_atom[g]]<<endl;
        }
    */
  ...
  MPI_Allreduce(&A_sum, &total_A_sum, n_max, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&AI_sum, &total_AI_sum, n_max, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&AA_sum, &total_AA_sum, n_max, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&AAII_sum, &total_AAII_sum, n_max, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&AAII_alloy_sum, &total_AAII_alloy_sum, n_max, MPI_DOUBLE,
                MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&AAII_interface_sum, &total_AAII_interface_sum, n_max,
                MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&count_interface_atom, &total_interface_atom, n_max, MPI_INT,
                MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&count_dot_atom, &total_dot_atom, n_max, MPI_INT, MPI_SUM,
                MPI_COMM_WORLD);
  ...
  if (my_id == 0) {
    ...
    for (int n = 0; n < n_max; n++) {
      ...
      double denominator =
          (total_AA_sum[n] / numAtoms) -
          (total_A_sum[n] * total_A_sum[n]) / numAtoms / numAtoms;
      decoherence_time[n] =
          2.0 * hbar_over_hf_coupling_coefficient / sqrt(denominator);
      if (n < e_max)
        cout << "Electron Level: " << n << endl;
      else
        cout << "Hole Level: " << n - e_max << endl;
      cout << "Number of Interface Atoms: " << total_interface_atom[n] << endl;
      cout << "Number of Dot Atoms: " << total_dot_atom[n] << endl;
      cout << "A_sum (eV): "
           << total_A_sum[n] * hf_coupling_coefficient * convertor_erg_to_eV
           << endl;
      cout << "AI_sum (eV) with polarized nuclear spins: "
           << total_AI_sum[n] * hf_coupling_coefficient * convertor_erg_to_eV
           << endl;
      cout << "AA_sum (eV_square): "
           << total_AA_sum[n] * hf_coupling_coefficient * convertor_erg_to_eV *
                  hf_coupling_coefficient * convertor_erg_to_eV
           << endl;
      cout << "Dephasing_time due to nuclear spin configuration (second):  "
           << dephasing_time[n] << endl;
      cout << "Dephasing_time due to alloy disorder (second):  "
           << alloy_dephasing_time[n] << endl;
      cout << "Dephasing_time due to interface disorder (second):  "
           << interface_dephasing_time[n] << endl;
      cout << "Decoherence_time (second): " << decoherence_time[n] << endl
           << endl;
    }
  }
}

void Decoherence::get_data(complex *coef_e, complex *coef_h, int *atom_id,
...
void Decoherence::get_geometry(double x0, double y0, double z0, double xL,
                               double yL, double zL, double composition) {
  Xorigin = x0;
  Yorigin = y0;
  Zorigin = z0;
  Xdim = xL;
...
bool Decoherence::CationInsideDot(int g) {
  if (_atom[g] == 0)
    return false; // anion atom
  else {          // cation atom
    double x = lattice(g, 0), y = lattice(g, 1), z = lattice(g, 2);
    if (z < Zorigin || z > Zorigin + Zdim || x < Xorigin ||
        x > Xorigin + Xdim || y < Yorigin || y > Yorigin + Ydim)
...
bool Decoherence::CationNearInterface(int g) {
  ...
  if (_atom[g] == 0)
    ...
    return false; // cation atom at the bottom of the dot
  else if (lattice(g, 2) < (Zorigin + Zdim * f1) &&
           lattice(g, 0) > (Xorigin - Xdim * f1) &&
           lattice(g, 0) < (Xorigin + Xdim * f2) &&
           lattice(g, 1) > (Yorigin - Ydim * f1) &&
           lattice(g, 1) > (Yorigin - Ydim * f2))
    return true; // cation atom near the substrate
  else {
    double x = lattice(g, 0), y = lattice(g, 1), z = lattice(g, 2);
    double sum = 0.0;
    sum += (x - Xcenter) * (x - Xcenter) / Xradius2;
    ...
    sum += (z - Zcenter) * (z - Zcenter) / Zradius2;
    if (sum > f3 || sum < f4)
      return false; // cation atom away from the interface
    else
      return true; // cation atom near the interface
  }
}
...
void Decoherence::compute_spin_correlator(const char &carrier, int n) {
  ...
  for (int g = g_min; g < g_max; g++) {
    ...
    double hyperfine_coupling =
        amplitude * A[_atom[g]] * hf_coupling_coefficient; // in unit of erg
    ...
    VV[g - g_min] =
        hyperfine_coupling * hyperfine_coupling * 0.25; // in unit of erg*erg
  ...
  double two_external_magnetic_field_omega =
      2 * bohr_magneton * MagneticField / hbar_in_eV_sec;
  // in unit of 1/second,  g*bohr_magneton*B/hbar
  double constant_omega =
      total_AI_sum_over_hbar + two_external_magnetic_field_omega;
  ...
  MPI_Allreduce(&V_over_hbar_omega, &total_V_over_hbar_omega, 1, MPI_DOUBLE,
                MPI_SUM, MPI_COMM_WORLD);

  if (my_id == 0) {
    cout.setf(ios::scientific);
    cout << setprecision(10);
    cout << "Sum of AI: " << total_AI_sum * convertor_erg_to_eV << " eV"
         << endl;
    cout << "Zeeman Energy: "
         << 2 * bohr_magneton * MagneticField +
                total_AI_sum * convertor_erg_to_eV
         << " eV" << endl;
    cout << "Electron precession time: " << 2 * M_PI / fabs(constant_omega)
         << " second" << endl;
    cout << "The sum of the amplitude of cosine function : "
         << total_V_over_hbar_omega << endl;
  }
  ...
  if (my_id == 0) {
    if (RandomOrientation)
      fout = new ofstream("Sz.unpolarized.coarse.dat");
    else
      fout = new ofstream("Sz.polarized.coarse.dat");
  ...
  for (int long it = 0; it < num_time_step; it++) {
    double Sz =
        0.0; // expectation value of electron spin z component, unitless.
    double time = it * time_step; // in unit of second;
    for (int g = 0; g < g_num; g++) {
      //      Sz += VV[g] / (omega[g]*omega[g]) *  ( cos(omega[g]*time) - 1.0 );
      //      // unitless
  ...
  /*
    ...
    for(int long it=0; it<num_time_step; it++) {
      double Sz=0.0;  // expectation value of electron spin z component,
    unitless. double time = it*time_step; // in unit of second; for(int g=0;
    g<g_num; g++){ Sz += VV[g] / (omega[g]*omega[g]) *  ( cos(omega[g]*time)
    - 1.0 ); // unitless
      }
      double total_Sz;
      MPI_Allreduce(&Sz, &total_Sz, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
