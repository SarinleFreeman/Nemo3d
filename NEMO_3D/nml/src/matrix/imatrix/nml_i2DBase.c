...

int
(nml_i2D_fprintf)(FILE* stream, const nml_i2DBase* pM, 
  int width, unsigned int precision, nml_fmtflags flags, unsigned int columns) {
  int		characters;
  int		total = 0;
  const
  nml_extent	m = nml_i2D_extent2(pM);
  nml_offset	i = 0;
  columns = (0 < columns)? columns: 4;
  for (i = 0; i < m; ++i) {
    const
    nml_extent	n = nml_i2D_extent1(pM);
    nml_offset	j = 0;
    for (j = 0; j < n; ++j) {
      nml_index k = (nml_index)j + nml_i2D_lower1(pM);
      if (0 < j) {
	characters
