...
int
(nml_fc3D_fprintf)(FILE* stream, const nml_fc3DBase* pT, 
  int width, unsigned int precision, nml_fmtflags flags, unsigned int columns) {
  int		characters;
  int		total = 0;
  const
  nml_extent	l = nml_fc3D_extent3(pT);
  nml_offset	h = 0;
  columns = (0 < columns)? columns: 4;
  for (h = 0; h < l; ++h) {
    const
    nml_extent	m = nml_fc3D_extent2(pT);
    nml_offset	i = 0;
    for (i = 0; i < m; ++i) {
      nml_index k2 = (nml_index)i + nml_fc3D_lower2(pT);
      const
      nml_extent	n = nml_fc3D_extent1(pT);
      nml_offset	j = 0;
      for (j = 0; j < n; ++j) {
	nml_index k1 = (nml_index)j + nml_fc3D_lower1(pT);
	if (0 < j) {
	  characters
