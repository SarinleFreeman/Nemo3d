...
//Read Options =================================================
  ...
  bool Compute_gaussian_projection=false;
  bool generate_test_wfs=false;
  ...
  while(iopt < options.size()) {
   ...
   else if(options[iopt] == "--wf_index") {
      wf_indx = atoi(options[++iopt].c_str());
    }	
    if(options[iopt] == "--hl") {
      ComputeCoulomb = true;
    }
    if(options[iopt] == "--hl_new") {
     ...
     else if(options[iopt] == "--gauss") {
      Compute_gaussian_projection = true;
    }	
     else if(options[iopt] == "--test_wfs") {
      generate_test_wfs = true;
    }
...
if(generate_test_wfs){

double x_coor=0.0;
double y_coor=0.0;
double z_coor=0.0;
double R2=0.0;
int wf_index=wf_indx;
double sum=0.0;
double tmp=0.0;
double a0=0.543095;
double a_bohr=3.0;
...
double* wf_data=new double[NumAtoms];
cout<<"wf_index : "<<wf_index<<endl;

for(int i=0;i<NumAtoms;i++){
	x_coor=lattice[3*i]-xc;
	y_coor=lattice[3*i+1]-yc;
	z_coor=lattice[3*i+2]-zc;
	R2=x_coor*x_coor+y_coor*y_coor+z_coor*z_coor;
	//R=sqrt(R2);
	//Gaussian wfs
	if(wf_index==1)
		wf_data[i]=exp(-R2/a_bohr);
	else if(wf_index==3)
		wf_data[i]=x_coor*exp(-R2/a_bohr);
	else if(wf_index==5)
                wf_data[i]=y_coor*exp(-R2/a_bohr);
	else if(wf_index==7)
                wf_data[i]=z_coor*exp(-R2/a_bohr);
	else if(wf_index==9)
                wf_data[i]=x_coor*x_coor*exp(-R2/a_bohr);
	else if(wf_index==11)
                wf_data[i]=x_coor*y_coor*exp(-R2/a_bohr);

	//Gaussian
	sum+=wf_data[i]*wf_data[i];
}
for(int i=0;i<NumAtoms;i++){
  	wf_data[i]=wf_data[i]/sum;	
}
write_test_wfs(data, wf_index, wf_data, NumAtoms, eList, 1);
}

if(Compute_gaussian_projection){
...
complex<double>* wf_data=new complex<double>[NumAtoms];
cout<<"wf_index : "<<wf_index<<endl;
...
for(int i=0;i<NumAtoms;i++){
        ...
        if(wf_index==1)
                ...
                tmp=x_coor*exp(-R2/a_bohr);
        else if(wf_index==5)
                tmp=y_coor*exp(-R2/a_bohr);
        else if(wf_index==7)
                tmp=z_coor*exp(-R2/a_bohr);
        else if(wf_index==9)
                tmp=x_coor*x_coor*exp(-R2/a_bohr);
        else if(wf_index==11)
                tmp=x_coor*y_coor*exp(-R2/a_bohr);

	double k=0.85*PI/a0;
        double re = cos(k*x_coor);
	double im = sin(k*y_coor);	
	wf_data[i]._Val[0]=tmp*re;
	wf_data[i]._Val[1]=tmp*im;	

        //Gaussian
        sum+=wf_data[i]._Val[0]*wf_data[i]._Val[0]+wf_data[i]._Val[1]*wf_data[i]._Val[1];
}
for(int i=0;i<NumAtoms;i++){
        wf_data[i]._Val[0]=wf_data[i]._Val[0]/sum;
	wf_data[i]._Val[1]=wf_data[i]._Val[1]/sum;
}
//write_Gaussian*plane wave wf for test
write_Gaussian_plus_wfs(data, wf_index, wf_data, NumAtoms, eList, 1);
//dot product

/*
 	complex<double>* wf=new complex<double>[NumAtoms];
	double k=0.85*PI/a0;

	double param=30;
	double result = sin(param*PI/180);
	cout<<"Result 30 deg sin : "<<endl;
	result = cos(param*PI/180);
        cout<<"Result 30 deg cos : "<<endl;
	...
	double tmp1=0.0;
		...
		//cout<<R2<<endl;
                ...
                //s-type
		tmp1=x_coor*exp(-R2*a_bohr);
                wf[i]=complex<double> (tmp1*cos(k*z_coor), tmp1*sin(k*z_coor));
		//cout<<wf[i]<<endl;
		//cout<<"exp : "<<exp(-R2/a_bohr)<<endl;

                //px-type
                //wf2[i]=x_coor*exp(-R2/a_bohr);

                //py-type
                //wf3[i]=y_coor*exp(-R2/a_bohr);

                //pz-type
                //wf4[i]=z_coor*exp(-R2/a_bohr);
        }
        //Normalize wf
 	//cout<<"exp : "<<exp(0.0)<<endl;
 	//cout<<"exp : "<<exp(-1.0)<<endl;
	...
	norm=sqrt(sum.real());
        ...
        double param=30;
        double result = sin(param*PI/180);
        cout<<"Result 30 deg sin : "<<result<<endl;
        result = cos(param*PI/180);
        cout<<"Result 30 deg cos : "<<result<<endl;

        if(rank==0){
                //save wf
                FILE*fp=fopen("wf_1","w");
		FILE*fp1=fopen("wf_1_prob","w");
                for(int i=0;i<NumAtoms;i++){
