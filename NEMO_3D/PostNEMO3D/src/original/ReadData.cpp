   ...
   complex<double> *psi_e_up = new complex<double>[numAtoms*20];
    complex<double> *psi_e_down = new complex<double>[numAtoms*20];
        for(int k=0;k<numAtoms;k++){
                for(int j=0;j<20;j++){
                        if(j==0){
                                psi_e_up[k*20+j]._Val[0]=wf_data[k]._Val[0];
                                psi_e_up[k*20+j]._Val[1]=wf_data[k]._Val[1];
                                psi_e_down[k*20+j]._Val[0]=0.0;
                                psi_e_down[k*20+j]._Val[1]=0.0;
                        }else if(j==10){
                                psi_e_down[k*20+j]._Val[0]=wf_data[k]._Val[0];
                                psi_e_down[k*20+j]._Val[1]=wf_data[k]._Val[1];
                                psi_e_up[k*20+j]._Val[0]=0.0;
                                psi_e_up[k*20+j]._Val[1]=0.0;
                        }else{
                                psi_e_down[k*20+j]._Val[0]=0.0;
                                psi_e_down[k*20+j]._Val[1]=0.0;
                                psi_e_up[k*20+j]._Val[0]=0.0;
...
for(int i=0;i<numAtoms;i++)
        wf_prob[i]=wf_data[i]._Val[0]*wf_data[i]._Val[0]+wf_data[i]._Val[1]*wf_data[i]._Val[1];
...
//save Hydrogenic/Gaussian wfs
   ...
   for (int i=0; i<MAX_VARS; i++) {
	...
	for(int k=0;k<numAtoms;k++){
		for(int j=0;j<20;j++){
			if(j==0){
				psi_e_up[k*20+j]._Val[0]=wf_data[k];
				psi_e_up[k*20+j]._Val[1]=0.0;
				psi_e_down[k*20+j]._Val[0]=0.0;
                                psi_e_down[k*20+j]._Val[1]=0.0;   	
			}else if(j==10){
				psi_e_down[k*20+j]._Val[0]=wf_data[k];
                                psi_e_down[k*20+j]._Val[1]=0.0;   
                                psi_e_up[k*20+j]._Val[0]=0.0;
                                psi_e_up[k*20+j]._Val[1]=0.0;
			}else{
