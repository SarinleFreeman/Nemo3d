...
//Read NEMO3D Output Files ==========================================  
  ...
  for(int iatom=0; iatom<NumAtoms; iatom++) {
    ...
    else cout<<"atomid does not match with As, Ga, In atom"<<endl;
  }
...
//===================================================================
  ...
  if(ComputeCoulomb) {
    ...
    Coulomb data_coulomb = 
	  Coulomb(NumElectrons, NumHoles, NumAtoms, myAtoms_begin, myAtoms_end, NumOrbitals);
    ifstream coulomb_input("coulomb.table");
    data_coulomb.assign_table(coulomb_input);
    data_coulomb.get_data(wf_e, wf_h, atomid, neighbor, lattice);
    data_coulomb.compute_coulomb_exchange();
    complex<double>* local_coul = data_coulomb.get_H_coul();
    complex<double>* local_exch = data_coulomb.get_H_exch();
    int csize = NumElectrons*NumHoles;
    ...
    if (rank==0) {
      ...
      for(int ne=0; ne<NumElectrons; ne++){
        for(int nh=0; nh<NumHoles; nh++){ 
          ...
          cout<<"Coulomb:  "<<setw(10)<<total_coul[NumHoles*ne+nh]<<endl;
          cout<<"Exchange: "<<setw(10)<<total_exch[NumHoles*ne+nh]<<endl;
          cout<<endl;
        }
      }
    }
