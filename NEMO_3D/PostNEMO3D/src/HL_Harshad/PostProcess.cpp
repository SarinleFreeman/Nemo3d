...
for(int i=0;i<NumAtoms;i++){
	...
	wf_data[i].real()=tmp*re;
	wf_data[i].imag()=tmp*im;	

        //Gaussian
        sum+=wf_data[i].real()*wf_data[i].real()+wf_data[i].imag()*wf_data[i].imag();
}
for(int i=0;i<NumAtoms;i++){
        wf_data[i].real()=wf_data[i].real()/sum;
	wf_data[i].imag()=wf_data[i].imag()/sum;
}
