...
*****************************************************************************
$Header: /repo/nml/src/matrix/imatrix/i2tensor.c,v 1.3 2003/10/08 16:19:45 hook Exp $
*****************************************************************************/
...
#undef NML_INLINE
#undef NML_MACROS
/* Include i2tensor type definition.					*/
#include "i2tensor.h"

/* Constructor								*/
i2tensor	I2tensor(int lb2, int ub2, int lb1, int ub1) {
  nml_i2DBase*	pp = nml_i2D_new(lb2, ub2, lb1, ub1);
  if (NULL != pp)
    return pp - lb2;
  else
    return NULL;
...
i2tensor	check_i2tensor(i2tensor pM,
  ...
  if (NULL != pM) {
    ...
    if (nml_i2D_lower2(p2) == lb2) {
      if ((nml_i2D_upper2(p2) != ub2)
       || (nml_i2D_lower1(p2) != lb1)
       || (nml_i2D_upper1(p2) != ub1)) {
	p2 = nml_i2D_resize(&p2, lb2, ub2, lb1, ub1);
