module sctracemin_functions
    use tracemin_types
    !use minimum_residual
    implicit none 
    contains
 

    subroutine test_zgtracemin(M,ai,Y,E,R,rank,np,comm,info) 
      type(zmatrix)                                 :: M
      ...
      !simple variables
      integer                                       :: neig,n,i,j,mcsinfo,ix,iy
      double precision                              :: alfa, gamma, invDii, dmiu      
      double complex                                :: zdotc 
      !qmr
      double complex,  dimension(:,:),allocatable   :: Vecs
      integer,         dimension(:),  allocatable   :: infoqmr
      ...
      allocate(IWork(neig))   
      !allocate temporary variables needed by map_chebyshev
      allocate(S    (n,neig))
      allocate(Vn   (n,neig))
      allocate(Vp   (n,neig))
      allocate(T    (n,neig)) 

      !write(*,*) 'perform the map_chebyshev_shift multiply'
      call zcopy(neig*n,Y,1,Q,1) 
      call zcopy(neig*n,Q,1,S,1)
      call cshmv(M,Q,Vn,neig)
      do j=1,4*ai%k
         call map_chebyshev_shift(n,0,neig,Vn,Q,R,Vp,S,ai%k,gamma,ai%a,ai%b,j,mcsinfo)
         if (mcsinfo == 0) then
            !write(*,*) '--- map chebyshev shift done ---'
            exit
         end if
         if ((mcsinfo == 1) .OR. (mcsinfo == 2)) then
            call cshmv(M,Vn,T,neig)
            call zcopy(neig*n,T,1,Vn,1)
         end if
         if (mcsinfo < 0) then
      ...
      !write(*,*) 'start zgtracemin'
      do i=1,4*ai%maxi !every iteration requires two matrix-vector multiplies (the algorithm will stop when needed by itself)
         call zgtracemin(n,neig,Y,E,Q,Qt,H,Hloc,G,ZWork,RWork,IWork,R,ai%tol,ai%maxi,i,0,rank,np,comm,info)
         if (info == 0) then
            write(*,*) '--- Algorithm Converged --- (iteration =',i,')'
            exit
         end if         
         if ((info == 1) .OR. (info == 2)) then
            !perform map_chebyshev_shift multiply 
            call zcopy(neig*n,Q,1,S,1)
            call cshmv(M,Q,Vn,neig)
            do j=1,4*ai%k
               call map_chebyshev_shift(n,0,neig,Vn,Q,R,Vp,S,ai%k,gamma,ai%a,ai%b,j,mcsinfo)
               if (mcsinfo == 0) then   
                  write(*,*) '--- map chebyshev shift done ---',i
                  exit
               end if
               if ((mcsinfo == 1) .OR. (mcsinfo == 2)) then
                  call cshmv(M,Vn,T,neig)
      ...
      do i=1,neig
         ...
         call zscal(n,dcmplx(alfa,ZERO),Y(1,i),1) 
      end do
      call cshmv(M,Y,T,neig)
      call zgemm('C','N',neig,neig,n,ZONE,Y,n,T,n,ZZERO,H,neig)
      !find eigenvectors of H
      call zheev('V','L',neig,H,neig,E,ZWork,max(2*neig-1,1),RWork,info)
      !write(*,*) 'eigenvalues of H are (should match matrix eigs)', E 
      ...
      !perform refinement of eigenpairs
      !FIRST ATTEMPT
      !write(*,*) 'E',E
      !write(*,*) 'Y',Y
      !do j=1,1
      !call cshmv(M,Y,T,neig)                                     !T is Z=A*Y
      !call zcopy(n*neig,T,1,Q,1)
      !call zcopy(n*neig,T,1,S,1)      
      !do i=1,neig
      !   invDii = E(i)/(E(i)-ONE)
      !   call zscal(n,dcmplx(invDii,ZERO),S(1,i),1)              !S is inv(D)*Z where dii= 1 - 1/lambda
      !   call zscal(n,dcmplx(MONE/E(i),ZERO),Q(1,i),1)
      !   call zaxpy(n,ZONE,Y(1,i),1,Q(1,i),1)                    !Q is resiual
      !   write(*,*) 'R=Q:', Q(:,i)

      !   dmiu = MONE*(dble(zdotc(n,S(1,i),1,Q(1,i),1))/dble(zdotc(n,S(1,i),1,T(1,i),1)))         
      !   write(*,*) 'dmiu', dmiu


      !   call zscal(n,dcmplx(MONE*invDii*dmiu,ZERO),T(1,i),1)    !T is deltamu*Z
      !   call zaxpy(n,dcmplx(MONE*invDii),Q(1,i),1,T(1,i),1)
      !   write(*,*) 'T', T(:,i)

      !   E(i) = ONE/((ONE/E(i)) + dmiu)
      !   call zaxpy(n,ZONE,T(1,i),1,Y(1,i),1)
      !end do
      !end do
      ...
      !write(*,*) 'T',T
      !do i=1,neig
         !call zcopy(n,T(1,i),1,Vecs(1,2),1)
         !call zaxpy(n,dcmplx(MONE*E(i),ZERO),Y(1,i),1,Vecs(1,2),1)
         !call zgemm('C','N',neig,1,n,ZONE,Y,n,Vecs(1,2),n,ZZERO,H(1,1),neig)  !only first vector of H is used
         !call zgemm('N','N',n,1,neig,ZMONE,Y,n,H(1,1),neig,ZZERO,Q(1,1),n)    !only first vector of Q/H is used
         !call zaxpy(n,ZONE,Q(1,1),1,Vecs(1,2),1)                              !Vecs(:,2) contains the residual
         !infoqmr(2)=0         
         !do j=1,11 
         !   write(*,*) '---->it',j            
         !   call zucpx(n,n,10,Vecs,ai%tol,infoqmr)
         !   write(*,*) 'infoqmr', infoqmr
         !   if ((infoqmr(2) == 1) .OR. (infoqmr(2) == 2)) then               
         !      ix = infoqmr(3)
         !      iy = infoqmr(4)
               !write(*,*) 'ix',ix,'iy',iy
               !write(*,*) 'Vix',Vecs(:,ix)

         !      call zgemm('C','N',neig,1,n,ZONE,Y,n,Vecs(1,ix),n,ZZERO,H(1,1),neig) !only first vector of H is used
         !      call zgemm('N','N',n,1,neig,ZONE,Y,n,H(1,1),neig,ZZERO,Q(1,1),n)     !only first vector of Q/H is used
         !      call zaxpy(n,ZMONE,Vecs(1,ix),1,Q(1,1),1)                            !Vecs(:,iy) contains the residual
         !      write(*,*) 'Q',Q(:,1)

         !      if (infoqmr(2) == 1) then
         !         write(*,*) 'multiply'
         !         call cshmv(M,Q,Vecs(:,iy:iy),1)
         !      else
         !         write(*,*) 'multiply transpose'
         !         call cshmvt(M,Q,Vecs(:,iy:iy),1)
         !      end if
         !      write(*,*) 'a Viy',Vecs(:,iy)
         !      call zaxpy(n,dcmplx(MONE*E(i),ZERO),Q(1,1),1,Vecs(1,iy),1)
               !write(*,*) 'a Viy',Vecs(:,iy)

         !      call zgemm('C','N',neig,1,n,ZONE,Y,n,Vecs(1,iy),n,ZZERO,H(1,1),neig) !only first vector of H is used
         !      call zgemm('N','N',n,1,neig,ZMONE,Y,n,H(1,1),neig,ZZERO,Q(1,1),n)    !only first vector of Q/H is used
         !      call zaxpy(n,ZONE,Q(1,1),1,Vecs(1,iy),1)                             !Vecs(:,iy) contains the residual
               !write(*,*) 'd Viy',Vecs(:,iy)
         !   else
         !      if (infoqmr(1) == 0) then
         !         write(*,*) 'converged'
         ...
         !end do
         !write(*,*) 'Vecs', Vecs(:,1)
         !call zaxpy(n,ZONE,Vecs(1,1),1,Y(1,i),1) !!!update the eigenvector i
      !end do
      !deallocate(Vecs)
      !deallocate(infoqmr)
      !call cshmv(M,Y,T,neig)
    ...
    subroutine zgtracemin(n,neig,Y,E,Q,Qt,H,Hloc,G,ZWork,RWork,IWork,R,tol,maxi,ncall,nloc,rank,np,comm,info)
      ...
      if ((info == 1) .OR. (ncall == 1)) then
         ...
         converged = TRUE
         do i=1,neig 
            call zscal(n,dcmplx(E(i),ZERO),R(1,i),1)   
            call zaxpy(n,ZMONE,Y(1,i),1,R(1,i),1)
            call zscal(n,ZMONE,R(1,i),1) 
...
!!         if ((converged == TRUE) .OR. (ncall > 2*maxi)) then
         if ((converged == TRUE) .OR. (ncall > maxi)) then !!new
            ...
            !Step 2 - B-orthonormalization (the assumption is that the first time routine is called Q=B*Y)   
            !write(*,*) '--- info1 ---'
!!            call zgemm('C','N',neig,neig,n,ZONE,Y,n,Q,n,ZZERO,H,neig)
!!            call zheev('V','L',neig,H,neig,E,ZWork,max(2*neig-1,1),RWork,locinfo)
            call zgemm('C','N',neig,neig,n,ZONE,Y,n,Q,n,ZZERO,G,neig)             !!new
...
!!            call zgemm('N','N',n,neig,neig,ZONE,Y,n,H,neig,ZZERO,Q,n)
!!            call zcopy(n*neig,Q,1,Y,1)
            call zgemm('N','N',n,neig,neig,ZONE,Y,n,G,neig,ZZERO,R,n) !!new
            call zcopy(n*neig,R,1,Y,1)                                !!new
            !Step 3 - form H
!!            call zgemm('C','N',neig,neig,n,ZONE,Y,n,Q,n,ZZERO,H,neig)
            call zgemm('C','N',neig,neig,n,ZONE,Y,n,R,n,ZZERO,H,neig) !!new
            ...
            !Step 5 - form Y that is section of A*x=lambda*B*x       
!!            call zgemm('N','N',n,neig,neig,ZONE,Y,n,H,neig,ZZERO,Q,n)
            !write(*,*) 'extra steps to for BY=V Omega^-1/2 W'        !!new
            call zgemm('N','N',n,neig,neig,ZONE,Q,n,G,neig,ZZERO,R,n) !!new
            call zgemm('N','N',n,neig,neig,ZONE,R,n,H,neig,ZZERO,Q,n) !!new
...
!!      if (info == 2) then
         ...
         call zgetrs('T',neig,n,H,neig,IWork,Qt,neig,locinfo)
         if (locinfo /= 0) then
    ...
    subroutine map_chebyshev_shift(n,nloc,nrhs,Vn,Vc,Vch,Vp,S,k,gamma,a,b,ncall,info)
      ...
      double complex, dimension(:,:), intent(inout) :: Vn        !set of p new vectors             (input: vector B*U)
      ...
      double complex, dimension(:,:), intent(inout) :: Vch       !B times set of p current vectors 
      ...
      if (ncall > 2*k) then
         info=0
         call zaxpy(n*nrhs,ZTWO,S,1,Vc,1)
         return 
      else
         if ((info ==  1) .OR. (ncall == 1)) then 
            call zaxpy(n*nrhs,dcmplx(MONE*b,ZERO),Vc,1,Vn,1)
            call zcopy(n*nrhs,Vn,1,Vch,1)
            info=2 
            return  
         end if
         if (info == 2) then
            call zaxpy(n*nrhs,dcmplx(MONE*a,ZERO),Vch,1,Vn,1)
            if (ncall == 2) then 
               call zscal(n*nrhs,dcmplx(MONE*gamma,ZERO),Vn,1)
               call zaxpy(n*nrhs,ZONE,Vc,1,Vn,1)
            else
               call zscal(n*nrhs,dcmplx(MTWO*gamma,ZERO),Vn,1)
               call zaxpy(n*nrhs,ZTWO,Vc,1,Vn,1)
               call zaxpy(n*nrhs,ZMONE,Vp,1,Vn,1)
            end if
           
            call zcopy(n*nrhs,Vc,1,Vp,1)
            call zcopy(n*nrhs,Vn,1,Vc,1)
            info=1
            return 
         end if
      end if
    ...
    subroutine extract_eigenpairs(Y,E,neig,nloc,H,Hloc,ZWork,RWork,T,rank,comm,info)
      ...
      if (info == 1) then
         !compute Omega   
         call zgemm('C','N',neig,neig,nloc,ZONE,Y,nloc,Y,nloc,ZZERO,H,neig)
      ...
      if (info == 2) then
         call zgemm('C','N',neig,neig,nloc,ZONE,Y,nloc,T,nloc,ZZERO,H,neig)
         !call MPI_Allreduce (Hloc(1:neig*neig,1), H(1:neig*neig,1), neig*neig, MPI_DOUBLE_COMPLEX, MPI_SUM, MPI_COMM_WORLD, mpierror)
         !find eigenvectors of H
         call zheev('V','L',neig,H,neig,E,ZWork,max(2*neig-1,1),RWork,info)
         !form new Y by multiplying scaled Y by eigenvectors of H   
    ...
    subroutine read_matrix_from_file(M, ai)
      ...
      read(fdescriptorA,*) M%rsize, M%sizem, M%bsize, shift, ai%neig, ai%a, ai%b, ai%c, ai%d, ai%k, ai%maxi, ai%tol
      !write(*,*) M%rsize, M%sizem, M%bsize, shift, ai%neig, ai%a, ai%b, ai%c, ai%d, ai%k, ai%maxi, ai%tol
      ai%a = ai%a + shift
      ai%b = ai%b + shift
      ai%c = ai%c + shift
      ai%d = ai%d + shift
...
end module sctracemin_functions
