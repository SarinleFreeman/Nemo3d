program pqtracemin
      ...
      double complex,dimension(:,:), allocatable :: result_v, R, ig, result_v_t, ig_t       !resulting eigenvectors, residual and initial guess 
      ...
      !call MPI_scatterv(transpose(ig),ne,disp,MPI_DOUBLE_COMPLEX,result_v_t,ne(rank+1),MPI_DOUBLE_COMPLEX,root,MPI_COMM_WORLD,mpierror) 
      ig_t = transpose(ig)
      ...
      t= MPI_wtime()      
      call test_ztracemin(Mp,ai,result_v,result_e,R, ne,rank,np,MPI_COMM_WORLD,info)  
      t2=MPI_wtime()

      write(*,*) rank,'Print results'
      allocate(Rnrm(ai%neig)) 
      do i=1,ai%neig 
         alpha  =dble(zdotc(ne(rank+1)/ai%neig,R(1,i),1,R(1,i),1))
         call MPI_Allreduce(alpha, Rnrm(i), 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, mpierror)                  
         Rnrm(i)=sqrt(Rnrm(i))           
      end do


      write(*,*) rank,'compute norm of residual after refinement of real A*x=lambda*x eigenvalue problem'
      ...
      do i=1,ai%neig
         ...
         iRnrm(i)=sqrt(iRnrm(i))         
      end do


      !write(*,*) rank,'Smallest eigenpairs are:'
      if (rank == root) then
         do i=1,ai%neig
